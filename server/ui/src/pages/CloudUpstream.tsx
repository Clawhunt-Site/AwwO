      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Push {selectedCompany.name} into a cloud stack. Automations stay paused until activation.
          </p>
        </div>
          ...
          {connection ? (
            <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
              <div>
                <div className="text-sm font-medium">
                  {displayCloudTargetLabel(
                    connection.target.stackDisplayName ?? connection.target.stackSlug ?? connection.target.stackId,
                    "Cloud stack",
                  )}
                </div>
                <div className="mt-1 text-xs text-muted-foreground">
                  {displayCloudTargetLabel(connection.target.product, "Cloud")} ·{" "}
                  {displayCloudTargetLabel(connection.target.origin, "Configured origin")} · token {connection.tokenStatus}
                </div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <Input
                ...
                placeholder="https://cloud.example.com/PC521D/dashboard"
                aria-label="Cloud stack URL"
              />
...
function displayCloudTargetLabel(value: string | null | undefined, fallback: string) {
  const trimmed = value?.trim();
  if (!trimmed) return fallback;
  return trimmed.replace(/paperclip/gi, "workspace");
}
