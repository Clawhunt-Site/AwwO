...
import { type LocalizedText, useLocalizedRelativeTime, useLocalizedText } from "../i18n/localized";
import {
  applyIssueFilters,
...
const inboxTabLabels: Record<InboxTab, LocalizedText> = {
  mine: { en: "Mine", zh: "我的" },
...
const inboxGroupLabels: Record<InboxWorkItemGroupBy, LocalizedText> = {
  none: { en: "None", zh: "无" },
  type: { en: "Type", zh: "类型" },
...
const blockedGroupLabels: Record<BlockedInboxGroupBy, LocalizedText> = {
  blocker_type: { en: "Blocker type", zh: "阻塞类型" },
  none: { en: "None", zh: "无" },
};

const blockedSortLabels: Record<BlockedInboxSort, LocalizedText> = {
  urgency: { en: "Most urgent", zh: "最紧急" },
  most_recent: { en: "Most recent", zh: "最近" },
  longest_stopped: { en: "Longest stopped", zh: "停滞最久" },
};

const inboxCategoryLabels: Record<InboxCategoryFilter, LocalizedText> = {
  everything: { en: "All categories", zh: "全部类别" },
  issues_i_touched: { en: "My recent tasks", zh: "我最近处理的任务" },
  join_requests: { en: "Join requests", zh: "加入请求" },
  approvals: { en: "Approvals", zh: "审批" },
  failed_runs: { en: "Failed runs", zh: "失败运行" },
  alerts: { en: "Alerts", zh: "提醒" },
};

const inboxApprovalFilterLabels: Record<InboxApprovalFilter, LocalizedText> = {
  all: { en: "All approval statuses", zh: "全部审批状态" },
  actionable: { en: "Needs action", zh: "需要处理" },
  resolved: { en: "Resolved", zh: "已解决" },
};

const generatedInboxGroupLabelCopy: Record<string, LocalizedText> = {
  "No workspace": { en: "No workspace", zh: "无工作区" },
  "Default workspace": { en: "Default workspace", zh: "默认工作区" },
  User: { en: "User", zh: "用户" },
  Unassigned: { en: "Unassigned", zh: "未分配" },
  "No project": { en: "No project", zh: "无项目" },
  Tasks: { en: "Tasks", zh: "任务" },
  Approvals: { en: "Approvals", zh: "审批" },
  "Failed runs": { en: "Failed runs", zh: "失败运行" },
  "Join requests": { en: "Join requests", zh: "加入请求" },
};

function localizeGeneratedInboxGroupLabel(
  label: string,
  localize: (copy: LocalizedText) => string,
): string {
  const defaultWorkspaceMatch = label.match(/^(.*) \(default\)$/);
  if (defaultWorkspaceMatch?.[1]) {
    return localize({ en: label, zh: `${defaultWorkspaceMatch[1]}（默认）` });
  }
  return localize(generatedInboxGroupLabelCopy[label] ?? { en: label, zh: label });
}
...
function approvalStatusLabel(status: Approval["status"], localize?: (copy: LocalizedText) => string): string {
  const fallback = status.replaceAll("_", " ");
  const copy: Record<string, LocalizedText> = {
    pending: { en: "pending", zh: "待处理" },
    approved: { en: "approved", zh: "已批准" },
    rejected: { en: "rejected", zh: "已拒绝" },
    revision_requested: { en: "revision requested", zh: "要求修改" },
  };
  return localize ? localize(copy[status] ?? { en: fallback, zh: fallback }) : fallback;
}
...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    const prefix = localize ? localize({ en: "Agent join request", zh: "Agent 加入请求" }) : "Agent join request";
    return `${prefix}${joinRequest.agentName ? `: ${joinRequest.agentName}` : ""}`;
  }
  ...
  return localize ? localize({ en: "Human join request", zh: "人工加入请求" }) : "Human join request";
}
...
export function FailedRunInboxRow({
  ...
  const displayError =
    ...
    ?? localize({ en: "Run exited with an error.", zh: "运行异常退出。" });
  const retryLabel = isRetrying
    ? localize({ en: "Retrying...", zh: "正在重试..." })
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{localize({ en: "Failed run", zh: "失败运行" })}{linkedAgentName ? ` - ${linkedAgentName}` : ""}</>
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          {!showUnreadSlot && (
            <button
              ...
              aria-label={localize({ en: "Dismiss", zh: "关闭" })}
            >
              <X className="h-4 w-4" />
            </button>
          )}
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span className="capitalize">{approvalStatusLabel(approval.status, localize)}</span>
              {requesterName ? <span>{localize({ en: `requested by ${requesterName}`, zh: `由 ${requesterName} 请求` })}</span> : null}
              <span>{localize({ en: `updated ${relativeTime(approval.updatedAt)}`, zh: `更新于 ${relativeTime(approval.updatedAt)}` })}</span>
            </span>
          </span>
        </Link>
...
function JoinRequestInboxRow({
  ...
  const localize = useLocalizedText();
  const relativeTime = useLocalizedRelativeTime();
  const label = formatJoinRequestInboxLabel(joinRequest, localize);
  const showUnreadSlot = unreadState !== null;
  const showUnreadDot = unreadState === "visible" || unreadState === "fading";
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{localize({
                en: `requested ${relativeTime(joinRequest.createdAt)} from IP ${joinRequest.requestIp}`,
                zh: `${relativeTime(joinRequest.createdAt)}从 IP ${joinRequest.requestIp} 请求`,
              })}</span>
              {joinRequest.adapterType && <span>{localize({ en: `adapter: ${joinRequest.adapterType}`, zh: `适配器：${joinRequest.adapterType}` })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  const issueLinkState = useMemo(
    ...
    [localize, location.pathname, location.search, location.hash],
  );
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: localize({ en: "Inbox", zh: "收件箱" }) }]);
  ...
  }, [agents, currentUserId, localize, mineIssues, touchedIssues]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : localize({ en: "Failed to approve", zh: "批准失败" }));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : localize({ en: "Failed to reject", zh: "拒绝失败" }));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : localize({ en: "Failed to approve join request", zh: "批准加入请求失败" }));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : localize({ en: "Failed to reject join request", zh: "拒绝加入请求失败" }));
    },
  });
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error(result.message ?? localize({ en: "Retry was skipped.", zh: "已跳过重试。" }));
      }
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : localize({ en: "Failed to archive task", zh: "归档任务失败" }));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : localize({ en: "Failed to undo inbox archive", zh: "撤销收件箱归档失败" }));
    },
    onSuccess: (_data, id) => {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={InboxIcon} message={localize({ en: "Select a company to view inbox.", zh: "选择一个公司查看收件箱。" })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="relative sm:hidden">
          ...
          <Input
            type="search"
            placeholder={localize({ en: "Search inbox…", zh: "搜索收件箱..." })}
            value={searchQuery}
        ...
        <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
          <PageTabBar
            items={[
              {
                value: "mine",
                label: localize(inboxTabLabels.mine),
              },
              {
                value: "recent",
                label: localize(inboxTabLabels.recent),
              },
              { value: "unread", label: localize(inboxTabLabels.unread) },
              { value: "blocked", label: localize(inboxTabLabels.blocked) },
              { value: "all", label: localize(inboxTabLabels.all) },
            ]}
          />
        </Tabs>
          ...
          {tab === "blocked" ? (
            <>
                ...
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="space-y-0.5 p-2">
                    {BLOCKED_GROUP_OPTIONS.map(([value, label]) => (
                      ...
                      >
                        <span>{localize(blockedGroupLabels[value] ?? { en: label, zh: label })}</span>
              ...
              <IssueColumnPicker
                ...
                title={localize({ en: "Choose which inbox columns stay visible", zh: "选择显示哪些收件箱列" })}
                iconOnly
              />
              <Popover>
                <PopoverTrigger asChild>
                ...
                <PopoverContent align="end" className="w-48 p-0">
                  <div className="space-y-0.5 p-2">
                    {BLOCKED_SORT_OPTIONS.map(([value, label]) => (
                      ...
                      >
                        <span>{localize(blockedSortLabels[value] ?? { en: label, zh: label })}</span>
            ...
            <>
              <Button
                ...
                title={nestingEnabled
                  ? localize({ en: "Disable parent-child nesting", zh: "关闭父子任务嵌套" })
                  : localize({ en: "Enable parent-child nesting", zh: "开启父子任务嵌套" })}
              >
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-40 p-2">
                  <div className="space-y-0.5">
                    {([
                      ["none", localize(inboxGroupLabels.none)],
                      ["type", localize(inboxGroupLabels.type)],
                      ["assignee", localize(inboxGroupLabels.assignee)],
                      ["project", localize(inboxGroupLabels.project)],
                      ...(isolatedWorkspacesEnabled ? ([["workspace", localize(inboxGroupLabels.workspace)]] as const) : []),
                ...
                <>
                  ...
                  >
                    {markAllReadMutation.isPending
                      ? localize({ en: "Marking...", zh: "正在标记..." })
                      : localize({ en: "Mark all as read", zh: "全部标记为已读" })}
                  </Button>
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        <DialogTitle>{localize({ en: "Mark all as read?", zh: "全部标记为已读？" })}</DialogTitle>
                        <DialogDescription>
                          {localize({
                            en: `This will mark ${unreadIssueIds.length} unread ${unreadIssueIds.length === 1 ? "item" : "items"} as read.`,
                            zh: `这会将 ${unreadIssueIds.length} 个未读项标记为已读。`,
                          })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={localize({ en: "Category", zh: "类别" })} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{localize(inboxCategoryLabels.everything)}</SelectItem>
              <SelectItem value="issues_i_touched">{localize(inboxCategoryLabels.issues_i_touched)}</SelectItem>
              <SelectItem value="join_requests">{localize(inboxCategoryLabels.join_requests)}</SelectItem>
              <SelectItem value="approvals">{localize(inboxCategoryLabels.approvals)}</SelectItem>
              <SelectItem value="failed_runs">{localize(inboxCategoryLabels.failed_runs)}</SelectItem>
              <SelectItem value="alerts">{localize(inboxCategoryLabels.alerts)}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={localize({ en: "Approval status", zh: "审批状态" })} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{localize(inboxApprovalFilterLabels.all)}</SelectItem>
                <SelectItem value="actionable">{localize(inboxApprovalFilterLabels.actionable)}</SelectItem>
                <SelectItem value="resolved">{localize(inboxApprovalFilterLabels.resolved)}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      {tab !== "blocked" && allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? localize({ en: "No inbox items match your search.", zh: "没有收件箱项目匹配搜索。" })
              : tab === "mine"
              ? localize({ en: "Inbox zero.", zh: "收件箱已清空。" })
              : tab === "unread"
              ? localize({ en: "No new inbox items.", zh: "没有新的收件箱项目。" })
              : tab === "recent"
                ? localize({ en: "No recent inbox items.", zh: "没有最近的收件箱项目。" })
                : localize({ en: "No inbox items match these filters.", zh: "没有收件箱项目匹配这些筛选。" })
          }
        />
      )}
              ...
              {(() => {
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {localize({
                            en: `(${childCount} sub-task${childCount !== 1 ? "s" : ""})`,
                            zh: `（${childCount} 个子任务）`,
                          })}
                        </span>
                      ) : undefined}
                      mobileMeta={issueActivityText(issue, language).toLowerCase()}
                      mobileLeading={
                        depth === 0 && hasChildren && collapseParentId ? (
                          <button
                            type="button"
                ...
                return groupedSections.flatMap((group, groupIndex) => {
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived"
                            ...
                            : localize({ en: "Other results", zh: "其他结果" })}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                  ...
                  if (group.label) {
                    ...
                    const displayGroupLabel = localizeGeneratedInboxGroupLabel(group.label, localize);
                    elements.push(
                      <div
                        key={`group-${group.key}`}
                        data-inbox-item
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={localize({ en: `New task in ${displayGroupLabel}`, zh: `在 ${displayGroupLabel} 中新建任务` })}
                              aria-label={localize({ en: `New task in ${displayGroupLabel}`, zh: `在 ${displayGroupLabel} 中新建任务` })}
                              onClick={(event) => {
                                event.stopPropagation();
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {localize({
                        en: `${dashboard!.agents.error === 1 ? "agent has" : "agents have"} errors`,
                        zh: `个 agent 出现错误`,
                      })}
                    </span>
                  </Link>
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {localize({ en: "Budget at", zh: "本月预算已使用" })}{" "}
                      <span className="font-medium">{dashboard!.costs.monthUtilizationPercent}%</span>
                      {localize({ en: " utilization this month", zh: "" })}
                    </span>
                  </Link>
                  <button
                    type="button"
