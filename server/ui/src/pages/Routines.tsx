...
const routineSortFieldCopy: Record<RoutineSortField, LocalizedText> = {
...
const routineGroupByCopy: Record<RoutineGroupBy, LocalizedText> = {
  project: { en: "Project", zh: "项目" },
  assignee: { en: "Agent", zh: "Agent" },
  none: { en: "None", zh: "无" },
};

const routineGroupLabelCopy: Record<string, LocalizedText> = {
  "No project": { en: "No project", zh: "无项目" },
  "Unknown project": { en: "Unknown project", zh: "未知项目" },
  Unassigned: { en: "Unassigned", zh: "未分配" },
  "Unknown agent": { en: "Unknown agent", zh: "未知 Agent" },
};

const routinePolicyLabelCopy: Record<string, LocalizedText> = {
  coalesce_if_active: { en: "Coalesce if active", zh: "运行中则合并" },
  always_enqueue: { en: "Always enqueue", zh: "始终入队" },
  skip_if_active: { en: "Skip if active", zh: "运行中则跳过" },
  skip_missed: { en: "Skip missed", zh: "跳过错过窗口" },
  enqueue_missed_with_cap: { en: "Enqueue missed with cap", zh: "按上限补加入队" },
};
...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: localize({ en: "Routines", zh: "定时任务" }) }]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: localize({ en: "Routine created", zh: "定时任务已创建" }),
        body: routine.assigneeAgentId
          ? localize({ en: "Add the first trigger to turn it into a live workflow.", zh: "添加第一个触发器后，它会成为可运行的工作流。" })
          : localize({ en: "Draft saved. Add a default agent before enabling automation.", zh: "草稿已保存。启用自动化前请添加默认 Agent。" }),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: localize({ en: "Failed to update routine", zh: "更新定时任务失败" }),
        body: mutationError instanceof Error
          ? mutationError.message
          : localize({ en: "The system could not update the routine.", zh: "系统无法更新该定时任务。" }),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: localize({ en: "Routine run failed", zh: "定时任务运行失败" }),
        body: mutationError instanceof Error
          ? mutationError.message
          : localize({ en: "The system could not start the routine run.", zh: "系统无法启动该定时任务运行。" }),
        tone: "error",
      });
    },
  });
  ...
  const recentRunsIssueLinkState = useMemo(
    () =>
      createIssueDetailLocationState(
        localize({ en: "Recent Runs", zh: "最近运行" }),
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: localize({ en: "Default agent required", zh: "需要默认 Agent" }),
        body: localize({ en: "Set a default agent before enabling routine automation.", zh: "启用定时任务自动化前请设置默认 Agent。" }),
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={localize({ en: "Select a company to view routines.", zh: "选择一个公司查看定时任务。" })} />;
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: localize({ en: "Routines", zh: "定时任务" }) },
            { value: "runs", label: localize({ en: "Recent Runs", zh: "最近运行" }) },
          ]}
        />
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {localize({
                en: `${visibleRoutines.length} routine${visibleRoutines.length === 1 ? "" : "s"}`,
                zh: `${visibleRoutines.length} 个定时任务`,
              })}
            </p>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{localize({ en: "Sort", zh: "排序" })}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      >
                        <span>{localize(routineSortFieldCopy[field])}</span>
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc"
                              ? localize({ en: "Asc", zh: "升序" })
                              : localize({ en: "Desc", zh: "降序" })}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{localize({ en: "Group", zh: "分组" })}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      >
                        <span>{localize(routineGroupByCopy[value])}</span>
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">
                {localize({ en: "New routine", zh: "新建定时任务" })}
              </p>
              <p className="text-sm text-muted-foreground">
                {localize({
                  en: "Define the recurring work first. Default project and agent are optional for draft routines.",
                  zh: "先定义周期性工作。草稿定时任务可以暂不设置默认项目和 Agent。",
                })}
              </p>
            </div>
            <Button
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={localize({ en: "Routine title", zh: "定时任务标题" })}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{localize({ en: "For", zh: "给" })}</span>
                  <span className="sr-only">{localize({ en: "Assignee", zh: "负责人" })}</span>
                  <InlineEntitySelector
                    ref={assigneeSelectorRef}
                    value={draft.assigneeAgentId}
                    options={assigneeOptions}
                    recentOptionIds={recentAssigneeIds}
                    placeholder={localize({ en: "Assignee", zh: "负责人" })}
                    noneLabel={localize({ en: "No assignee", zh: "无负责人" })}
                    searchPlaceholder={localize({ en: "Search assignees...", zh: "搜索负责人..." })}
                    emptyMessage={localize({ en: "No assignees found.", zh: "未找到负责人。" })}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{localize({ en: "Assignee", zh: "负责人" })}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  />
                  <span>{localize({ en: "in", zh: "在" })}</span>
                  <InlineEntitySelector
                    ref={projectSelectorRef}
                    value={draft.projectId}
                    options={projectOptions}
                    recentOptionIds={recentProjectIds}
                    placeholder={localize({ en: "Project", zh: "项目" })}
                    noneLabel={localize({ en: "No project", zh: "无项目" })}
                    searchPlaceholder={localize({ en: "Search projects...", zh: "搜索项目..." })}
                    emptyMessage={localize({ en: "No projects found.", zh: "未找到项目。" })}
                    onChange={(projectId) => {
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{localize({ en: "Project", zh: "项目" })}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={localize({ en: "Add instructions...", zh: "添加说明..." })}
                bordered={false}
                ...
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">
                      {localize({ en: "Advanced delivery settings", zh: "高级投递设置" })}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {localize({
                        en: "Keep policy controls secondary to the work definition.",
                        zh: "策略控制作为工作定义的次级设置保留。",
                      })}
                    </p>
                  </div>
                        ...
                        <SelectContent>
                          {concurrencyPolicies.map((value) => (
                            <SelectItem key={value} value={value}>
                              {localize(routinePolicyLabelCopy[value])}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">
                        {localize(concurrencyPolicyDescriptions[draft.concurrencyPolicy])}
                      </p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
                        {localize({ en: "Catch-up", zh: "补跑" })}
                      </p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">
                        {localize(catchUpPolicyDescriptions[draft.catchUpPolicy])}
                      </p>
                    </div>
                  </div>
                </CollapsibleContent>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {localize({
                en: "After creation, the workflow opens trigger setup. Draft routines stay paused until you add a default agent.",
                zh: "创建后，工作流会直接进入触发器设置。草稿定时任务会保持暂停，直到你添加默认 Agent。",
              })}
            </div>
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error
                    ...
                    : localize({ en: "Failed to create routine", zh: "创建定时任务失败" })}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : localize({ en: "Failed to load routines", zh: "加载定时任务失败" })}
          </CardContent>
        </Card>
      ) : null}
          ...
          {visibleRoutines.length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={localize({
                  en: "No active routines. Use Create routine to define the first recurring workflow.",
                  zh: "没有活跃定时任务。使用“创建定时任务”定义第一个周期性工作流。",
                })}
              />
            </div>
          ) : (
                      ...
                      >
                        <CollapsibleTrigger className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm font-semibold uppercase tracking-wide">
                            {routineGroupLabelCopy[group.label]
                              ? localize(routineGroupLabelCopy[group.label])
                              : group.label}
                          </span>
                        </CollapsibleTrigger>
                        <span className="text-xs text-muted-foreground">
