...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Workspace", managedLabel: "Workspace managed" };
    default:
...
// Reserved key namespace the backend canonicalizes app-shipped bundled skills into
// (deriveCanonicalSkillKey → `${UPSTREAM_BUNDLED_KEY_PREFIX}<slug>`). The list DTO
// drops `metadata` and only fills `catalogKind` for catalog-sourced rows, so the
// reserved key prefix is the one signal that survives to the client for the
// disk-bundled read-only skills (which otherwise look like an editable company
// skill: sourceBadge "paperclip", catalogKind null).
const UPSTREAM_BUNDLED_KEY_PREFIX = "paperclipai/paperclip/";

// SuperClaw local-only store: a skill is "upstream / official-source" when it came
// from the curated install catalog or the app-shipped bundled set. Detected from
// list-DTO-available signals only:
//   - reserved bundled key namespace (app-shipped read-only skills), or
//   - a "catalog" source badge, or
//   - a bundled / optional catalog kind (carried directly, or via readonly metadata
//     when the full detail object is available).
// User-authored company skills (editable, key NOT in the reserved namespace) and
// user-imported github / url / skills.sh / local skills are NOT upstream and stay.
export function isUpstreamCatalogSkill(skill: {
  key: string;
  sourceBadge: CompanySkillSourceBadge;
  catalogKind?: "bundled" | "optional" | null;
  metadata?: Record<string, unknown> | null;
}): boolean {
  if (skill.key.startsWith(UPSTREAM_BUNDLED_KEY_PREFIX)) return true;
  if (skill.sourceBadge === "catalog") return true;
  const kind = skill.catalogKind ?? readonlyMetadataKind(skill.metadata);
  return kind === "bundled" || kind === "optional";
}
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      ...
      tooltip: "This skill could not be validated as Agent Skills markdown. Install at your own risk.",
...
// SuperClaw: local-only skills store — the upstream "catalog" / "bundled"
// (official-source) tabs are removed; only locally managed skills show.
...
const DISCOVERY_SORT_LABELS: Record<DiscoverySort, LocalizedText> = {
  agents: { en: "Most agents", zh: "最多 Agent" },
  stars: { en: "Most stars", zh: "最多收藏" },
  forks: { en: "Most forks", zh: "最多 Fork" },
  recent: { en: "Recently updated", zh: "最近更新" },
  alphabetical: { en: "Alphabetical", zh: "按字母" },
};
...
function SkillCardIcon({ card, size = 36 }: { card: DiscoveryCard; size?: number }) {
  if (card.iconUrl) {
    return (
      <img
        src={resolveContentUrl(card.iconUrl)}
        alt=""
...
function buildDiscoveryCards(
  ...
  for (const entry of catalog) {
    ...
    cards.push({
      ...
      name: entry.name,
      slug: entry.slug,
      author: entry.packageName ?? "Workspace",
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-muted-foreground">
            {localize({ en: `by ${card.author}`, zh: `作者：${card.author}` })}{card.version ? ` · ${card.version}` : ""}
          </div>
        </div>
        ...
        {(() => {
          ...
          return (
            <span className="shrink-0 text-muted-foreground" title={localize({ en: `From ${meta.label}`, zh: `来源：${meta.label}` })} aria-label={localize({ en: `From ${meta.label}`, zh: `来源：${meta.label}` })}>
      ...
      <div className="mt-auto pt-3">
        ...
        <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
          <span>{card.agentCount} {localize({ en: card.agentCount === 1 ? "agent" : "agents", zh: "个 Agent" })}</span>
          {card.starCount > 0 ? (
            <>
...
export function DiscoveryGrid({
  ...
  return (
    ...
    // SuperClaw: local-only skills surface — the upstream "Skills Store"
    // category sidebar and external catalog browsing are removed; this page only
    // manages company-owned skills.
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
          <div className="flex h-9 min-w-[12rem] flex-1 items-center gap-2 rounded-md border border-border px-2.5">
            ...
            <input
              ...
              placeholder={localize({ en: "Search skills, authors, categories…", zh: "搜索技能、作者、分类…" })}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm">
                <span className="text-muted-foreground">{localize({ en: "Sort", zh: "排序" })}</span>
                <span className="ml-1.5">{localize(DISCOVERY_SORT_LABELS[sort])}</span>
              ...
              <DropdownMenuRadioGroup value={sort} onValueChange={(value) => onSortChange(value as DiscoverySort)}>
                {DISCOVERY_SORTS.map((option) => (
                  <DropdownMenuRadioItem key={option} value={option}>
                    {localize(DISCOVERY_SORT_LABELS[option])}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
            </DropdownMenuContent>
          ...
          {availableSources.length > 1 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm">
                  <span className="text-muted-foreground">{localize({ en: "Source", zh: "来源" })}</span>
                  <span className="ml-1.5 capitalize">
                    {sourceBadgeFilter === "all" ? localize({ en: "All", zh: "全部" }) : sourceMeta(sourceBadgeFilter as CompanySkillSourceBadge, null).label}
                  </span>
                  <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuRadioGroup value={sourceBadgeFilter} onValueChange={setSourceBadgeFilter}>
                  <DropdownMenuRadioItem value="all">{localize({ en: "All sources", zh: "全部来源" })}</DropdownMenuRadioItem>
          ...
          <Button
            ...
            title={localize({ en: "Scan project workspaces for skills", zh: "扫描项目工作区中的技能" })}
          >
          ...
          {loading ? (
            ...
            <div className="py-12">
              <EmptyState
                ...
                message={
                  totalCount === 0
                    ? localize({ en: "No skills yet. Create one to get started.", zh: "暂无技能。创建一个开始吧。" })
                    : search || activeCategory || sourceFilterActive
                      ? localize({ en: "No skills match your filters.", zh: "没有符合筛选条件的技能。" })
                      : localize({ en: "No skills in this tab yet.", zh: "该标签下暂无技能。" })
                }
              />
              {totalCount === 0 ? (
                <div className="mt-3 flex flex-col items-center gap-2">
                  <Button size="sm" onClick={onCreate}>
                    <Pencil className="mr-1.5 h-3.5 w-3.5" /> {localize({ en: "Create a skill", zh: "创建技能" })}
                  </Button>
                </div>
              ) : (search || activeCategory || sourceFilterActive) ? (
                <div className="mt-3 flex justify-center">
                  ...
                  >
                    {localize({ en: "Clear filters", zh: "清除筛选" })}
                  </Button>
                </div>
              ) : null}
            </div>
            ...
            <>
              <p className="mb-3 text-xs text-muted-foreground">
                {sourceFilteredCards.length}{" "}
                {localize({
                  en: sourceFilteredCards.length === 1 ? "skill" : "skills",
                  zh: "个技能",
                })}
...
export function CompanySkills() {
  ...
  const sourceFilter: SourceFilter = (["all", "company", "bundled", "optional", "external"] as SourceFilter[]).includes(sourceFilterParam as SourceFilter)
    ...
    : "all";
  // SuperClaw local-only store: the catalog detail / install surface does not exist
  // here. We derive selectedCatalogRef as null from the very first render (NOT via a
  // post-render effect), so a stale `?catalog=<ref>` deep link can never flash the
  // CatalogDetailPane or kick off catalog fetches before the URL is cleaned up. The
  // legacy `view`/`catalog` params are still stripped from the URL by the effect
  // below for tidiness.
  const selectedCatalogRef: string | null = null;
  ...
  // Discovery grid owns `/skills` whenever no specific skill is selected. Catalog
  // refs are ignored entirely in the local-only store (selectedCatalogRef is always
  // null), so they can never drop out of discovery into the catalog detail surface.
  const isDiscovery = !routeSkillToken;
  ...
  // SuperClaw local-only store: strip both the legacy `view` param and any
  // `catalog` deep link. The local management surface never browses, installs, or
  // updates from the upstream curated catalog, so a stale `?catalog=<ref>` (or
  // `?view=catalog`) link must fall back to the local grid instead of exposing the
  // catalog install/update surface.
  useEffect(() => {
  ...
  // SuperClaw local-only store: filter out upstream / official-source skills
  // (app-shipped bundled set + catalog-sourced) up front. This single derived list
  // backs BOTH the discovery grid AND the detail-route resolution below, so an
  // official skill cannot be surfaced through the grid OR through a direct
  // `/skills/<id|slug|key>` deep link, bookmark, or typed token.
  const localSkills = useMemo(
    () => installedSkills.filter((skill) => !isUpstreamCatalogSkill(skill)),
    [installedSkills],
  );
  const routeResolution = useMemo(
    () => resolveSkillRouteToken(routeSkillToken, localSkills),
    [routeSkillToken, localSkills],
  );
  // A deep link that resolves to a real company skill which we deliberately hide
  // (present in installedSkills but not in localSkills → upstream/official) is
  // redirected back to the grid rather than left on a "not found" page. The detail
  // never renders regardless (selectedSkillId stays null since the token does not
  // resolve against localSkills), so this is UX polish, not the security boundary.
  const routeHitsHiddenUpstreamSkill = useMemo(
    () =>
      Boolean(routeSkillToken)
      && !routeResolution.skill
      && Boolean(resolveSkillRouteToken(routeSkillToken, installedSkills).skill),
    [routeSkillToken, routeResolution.skill, installedSkills],
  );

  useEffect(() => {
    if (routeHitsHiddenUpstreamSkill) navigate("/skills", { replace: true });
  }, [routeHitsHiddenUpstreamSkill, navigate]);
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: `${skill.name} is now editable in the workspace.`,
      });
    },
    onError: (error) => {
  ...
  // Local-only store never browses the upstream curated catalog, so the catalog
  // list is never fetched (selectedCatalogRef is always null → the detail/file
  // queries below also stay disabled). Kept declared so the now-unreachable catalog
  // detail components type-check until they are excised in a follow-up.
  ...
  // SuperClaw: the Skills store is a LOCAL-ONLY surface. Two things keep
  // upstream / official-source skills out of discovery:
  //   1. We never merge the upstream curated install catalog into the grid —
  //      buildDiscoveryCards receives an empty catalog, so catalog-only cards
  //      never appear.
  //   2. Installed skills with an upstream provenance (app-shipped bundled set, or
  //      a catalog source badge / bundled-optional kind) are filtered out via
  //      isUpstreamCatalogSkill (see `localSkills`, derived above next to the route
  //      resolver so the grid and direct deep links share one filtered list), so an
  //      already-installed official skill cannot leak back in. Only company-authored
  //      and user-imported (github / url / skills.sh / local path) skills remain.
  const discoveryCards = useMemo(
    () => buildDiscoveryCards(localSkills, []),
    [localSkills],
  );
  ...
  const visibleDiscoveryCards = useMemo(() => {
    ...
    // Local-only store has no bundled tab, so required skills are always demoted.
    return sortDiscoveryCards(filtered, discoverySort, true);
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {createDraft.forkedFromSkillId
                ...
                : "Create an editable company skill in the workspace."}
            </DialogDescription>
          </DialogHeader>
          <NewSkillWizard
