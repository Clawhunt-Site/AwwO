...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes workspace config", async () => {
    ...
    expect(document.body.textContent).toContain("from this workspace only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from workspace"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
  ...
  it("shows AWS discovery errors without replacing manual vault form values", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Workspace is not authorized";
    mockSecretsApi.providerConfigDiscoveryPreview.mockRejectedValueOnce(
      new ApiError("AWS Secrets Manager denied the request. Check IAM permissions for this provider vault.", 403, {
        details: {
          ...
          region: "us-west-2",
          credentialPath: "Server runtime/provider credential path",
          requiredCapability: "secretsmanager:ListSecrets",
          actionableMessage:
            "AWS discovery preview needs secretsmanager:ListSecrets in the selected region for the server runtime/provider credential path.",
          safeAlternative:
    ...
    expect(errorBanner?.textContent).toContain("server runtime/provider credential path");
