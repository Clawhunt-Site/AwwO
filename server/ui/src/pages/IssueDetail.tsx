...
import { isImageAttachment, matchImageAttachmentIndex } from "../lib/issue-attachments";
...
const TREE_CONTROL_MODE_LABEL_ZH: Record<IssueTreeControlMode, string> = {
  pause: "暂停子树",
  resume: "恢复子树",
  cancel: "取消子树",
  restore: "还原子树",
...
const LEAF_WORK_CONTROL_MODE_LABEL_ZH: Partial<Record<IssueTreeControlMode, string>> = {
  pause: "暂停任务",
  resume: "恢复任务",
};
...
const TREE_CONTROL_MODE_HELP_TEXT_ZH: Record<IssueTreeControlMode, string> = {
  pause: "暂停此任务子树中的活跃执行，直到明确恢复。",
  resume: "释放活跃的子树暂停，让已暂停的工作继续。",
  cancel: "取消此子树中的非终态任务，并尽可能中断排队或运行中的工作。",
  restore: "还原此子树操作取消的任务，让工作可以继续。",
};
...
const LEAF_WORK_CONTROL_MODE_HELP_TEXT_ZH: Partial<Record<IssueTreeControlMode, string>> = {
  pause: "暂停此任务上的活跃执行，直到明确恢复。",
  resume: "释放活跃的暂停，让此任务继续。",
};
...
function issueTreeControlLabel(mode: IssueTreeControlMode, scope: "leaf" | "subtree", localize?: (copy: LocalizedText) => string) {
  const en = scope === "leaf"
    ? LEAF_WORK_CONTROL_MODE_LABEL[mode] ?? TREE_CONTROL_MODE_LABEL[mode]
    : TREE_CONTROL_MODE_LABEL[mode];
  const zh = scope === "leaf"
    ? LEAF_WORK_CONTROL_MODE_LABEL_ZH[mode] ?? TREE_CONTROL_MODE_LABEL_ZH[mode]
    : TREE_CONTROL_MODE_LABEL_ZH[mode];
  return pickLocalized(en, zh, localize);
}

function issueTreeControlHelpText(mode: IssueTreeControlMode, scope: "leaf" | "subtree", localize?: (copy: LocalizedText) => string) {
  const en = scope === "leaf"
    ? LEAF_WORK_CONTROL_MODE_HELP_TEXT[mode] ?? TREE_CONTROL_MODE_HELP_TEXT[mode]
    : TREE_CONTROL_MODE_HELP_TEXT[mode];
  const zh = scope === "leaf"
    ? LEAF_WORK_CONTROL_MODE_HELP_TEXT_ZH[mode] ?? TREE_CONTROL_MODE_HELP_TEXT_ZH[mode]
    : TREE_CONTROL_MODE_HELP_TEXT_ZH[mode];
  return pickLocalized(en, zh, localize);
}

function treeControlPreviewErrorCopy(error: unknown, localize?: (copy: LocalizedText) => string): string {
  if (error instanceof ApiError) {
    if (error.status === 403) {
      return pickLocalized("Only board users can preview subtree controls.", "只有看板用户可以预览子树控制。", localize);
    }
    if (error.status === 409) {
      return pickLocalized("Preview is stale because subtree hold state changed. Retry to refresh.", "预览已过期，因为子树暂停状态已变化。请重试刷新。", localize);
    }
    if (error.status === 422) {
      return pickLocalized("This subtree action is currently invalid for the selected tasks.", "此子树操作当前不适用于选中的任务。", localize);
    }
  }
  return error instanceof Error ? error.message : pickLocalized("Unable to load preview.", "无法加载预览。", localize);
}
...
function ActorIdentity({ evt, agentMap, userProfileMap }: { evt: ActivityEvent; agentMap: Map<string, Agent>; userProfileMap?: Map<string, import("../lib/company-members").CompanyUserProfile> }) {
  ...
  if (evt.actorType === "system") return <Identity name={localize({ en: "System", zh: "系统" })} size="sm" />;
  if (evt.actorType === "user") {
    const profile = userProfileMap?.get(id);
    return <Identity name={profile?.label ?? localize({ en: "Board", zh: "看板" })} avatarUrl={profile?.image} size="sm" />;
  }
  return <Identity name={id || localize({ en: "Unknown", zh: "未知" })} size="sm" />;
}
  ...
  return (
    <div className="flex items-center w-full">
      <Button
        ...
        aria-label={localize({ en: "Back to inbox", zh: "返回收件箱" })}
      >
      ...
      <div className="ml-auto flex items-center gap-0.5">
        {issueIdProp && !issueHidden && (
          <Button
            ...
            aria-label={localize({ en: "Archive from inbox", zh: "从收件箱归档" })}
          >
        ...
        <Popover open={menuOpen} onOpenChange={setMenuOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={localize({ en: "More actions", zh: "更多操作" })}>
          ...
          <PopoverContent className="w-44 p-1" align="end">
            ...
            >
              ...
              {localize({ en: "Copy as markdown", zh: "复制为 Markdown" })}
            </button>
            <button
              className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
            ...
            {issueIdProp && (
              ...
              >
                ...
                {localize({ en: "Hide this task", zh: "隐藏此任务" })}
              </button>
            )}
          </PopoverContent>
        </Popover>
  ...
  return (
    <div className="space-y-3">
      ...
      <ThreadComponent
        ...
        stopRunLabel={localize({ en: "Pause work", zh: "暂停任务" })}
        stoppingRunLabel={localize({ en: "Pausing...", zh: "正在暂停..." })}
            ...
            <div className="space-y-1 text-xs text-muted-foreground tabular-nums">
              <div className="flex flex-wrap gap-3">
                ...
                {issueCostSummary.hasTokens ? (
                  <span>
                    {localize({ en: "Tokens", zh: "Token" })} {formatTokens(issueCostSummary.totalTokens)}
                    {issueCostSummary.cached > 0
                      ? ` (${localize({ en: "in", zh: "输入" })} ${formatTokens(issueCostSummary.input)}, ${localize({ en: "out", zh: "输出" })} ${formatTokens(issueCostSummary.output)}, ${localize({ en: "cached", zh: "缓存" })} ${formatTokens(issueCostSummary.cached)})`
                      : ` (${localize({ en: "in", zh: "输入" })} ${formatTokens(issueCostSummary.input)}, ${localize({ en: "out", zh: "输出" })} ${formatTokens(issueCostSummary.output)})`}
                  </span>
                ) : null}
                {issueCostSummary.hasRuntime ? (
                  <span>
                    {localize({ en: "Runtime", zh: "运行时长" })} {formatDurationMs(issueCostSummary.runtimeMs)}
                    {` (${localize({
                      en: `${issueCostSummary.runCount} run${issueCostSummary.runCount === 1 ? "" : "s"}`,
                      zh: `${issueCostSummary.runCount} 次运行`,
                    })})`}
                  </span>
                ) : null}
                {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !issueCostSummary.hasRuntime ? (
                  <span>{localize({ en: "No direct cost data.", zh: "无直接成本数据。" })}</span>
                ) : null}
              </div>
              {hasIssueTreeCost && issueTreeCostSummary ? (
                <div className="flex flex-wrap gap-3">
                  <span className="font-medium text-foreground">
                    {localize({ en: "Including sub-tasks", zh: "包含子任务" })} {(issueTreeCostSummary.costCents / 100).toLocaleString(undefined, {
                      style: "currency",
                      currency: "USD",
                  ...
                  <span>
                    {localize({ en: "Tokens", zh: "Token" })} {formatTokens(issueTreeCostTokens)}
                    {issueTreeCostSummary.cachedInputTokens > 0
                      ? ` (${localize({ en: "in", zh: "输入" })} ${formatTokens(issueTreeCostSummary.inputTokens)}, ${localize({ en: "out", zh: "输出" })} ${formatTokens(issueTreeCostSummary.outputTokens)}, ${localize({ en: "cached", zh: "缓存" })} ${formatTokens(issueTreeCostSummary.cachedInputTokens)})`
                      : ` (${localize({ en: "in", zh: "输入" })} ${formatTokens(issueTreeCostSummary.inputTokens)}, ${localize({ en: "out", zh: "输出" })} ${formatTokens(issueTreeCostSummary.outputTokens)})`}
                  </span>
                  {issueTreeCostSummary.runCount > 0 ? (
                    <span>
                      {localize({ en: "Runtime", zh: "运行时长" })} {formatDurationMs(issueTreeCostSummary.runtimeMs)}
                      {` (${localize({
                        en: `${issueTreeCostSummary.runCount} run${issueTreeCostSummary.runCount === 1 ? "" : "s"}`,
                        zh: `${issueTreeCostSummary.runCount} 次运行`,
                      })})`}
                    </span>
                  ) : null}
                  <span>
                    {localize({
                      en: `${issueTreeCostSummary.issueCount} task${issueTreeCostSummary.issueCount === 1 ? "" : "s"}`,
                      zh: `${issueTreeCostSummary.issueCount} 个任务`,
                    })}
                  </span>
                </div>
              ) : null}
            ...
            return (
              <div className={cn("space-y-1.5 rounded-lg border px-3 py-2 text-xs", tone.className)}>
                <div className="flex items-center gap-1.5">
                  ...
                  <span>{formatIssueActivityAction(evt.action, evt.details, { agentMap, userProfileMap, currentUserId, localize })}</span>
                  <span className="ml-auto shrink-0">{localizedRelativeTime(evt.createdAt)}</span>
                </div>
...
export function IssueDetail() {
  ...
  const executeTreeControl = useMutation({
    ...
    onSuccess: async (result) => {
      const modeLabel = issueTreeControlLabel(result.hold.mode, treeControlScope, localize);
      const cancelCount = result.preview?.totals.activeRuns ?? 0;
      pushToast({
        title: result.kind === "release"
          ? treeControlScope === "leaf"
            ? localize({ en: "Work resumed", zh: "任务已恢复" })
            : localize({ en: "Subtree resumed", zh: "子树已恢复" })
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf"
              ? localize({ en: "Work paused", zh: "任务已暂停" })
              : localize({ en: "Subtree paused", zh: "子树已暂停" })
            : localize({ en: `${modeLabel} applied`, zh: `${modeLabel}已应用` }),
        body: result.kind === "release"
          ? (result.hold.releaseReason?.trim() || (treeControlScope === "leaf"
            ? localize({ en: "Active task pause released.", zh: "活跃任务暂停已释放。" })
            : localize({ en: "Active subtree pause released.", zh: "活跃子树暂停已释放。" })))
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf"
              ? localize({
                en: `Work paused. ${cancelCount} run${cancelCount === 1 ? "" : "s"} cancelled.`,
                zh: `任务已暂停。已取消 ${cancelCount} 次运行。`,
              })
              : localize({
                en: `Subtree paused. ${cancelCount} run${cancelCount === 1 ? "" : "s"} cancelled.`,
                zh: `子树已暂停。已取消 ${cancelCount} 次运行。`,
              })
            : result.hold.reason?.trim()
              ? result.hold.reason
              : localize({ en: "Subtree control applied.", zh: "子树控制已应用。" }),
      });
  ...
  const handleChatImageClick = useCallback(
    (src: string) => {
      // matchImageAttachmentIndex canonicalizes both the click src and each attachment's
      // contentPath so a click resolves to the gallery whether src arrives raw "/api/..."
      // or embedding-rewritten "/paperclip-api/..." (see issue-attachments.ts).
      const idx = matchImageAttachmentIndex(imageAttachments, src);
      if (idx >= 0) {
        setGalleryIndex(idx);
        setGalleryOpen(true);
      } else {
        // Image not in attachment list — open in a new tab, embedding-aware so a raw
        // "/api/..." opens through the Paperclip control plane when embedded.
        window.open(resolveContentUrl(src), "_blank");
      }
    },
    [imageAttachments],
  );
  ...
  const treeControlPrimaryButtonLabel =
    treeControlMode === "pause"
      ? treeControlScope === "leaf"
        ? localize({ en: "Pause work", zh: "暂停任务" })
        : localize({ en: "Pause and stop work", zh: "暂停并停止工作" })
      : treeControlMode === "cancel"
        ? localize({ en: `Cancel ${previewAffectedIssueCount} tasks`, zh: `取消 ${previewAffectedIssueCount} 个任务` })
      : treeControlMode === "restore"
          ? localize({ en: `Restore ${previewAffectedIssueCount} tasks`, zh: `还原 ${previewAffectedIssueCount} 个任务` })
          : treeControlScope === "leaf"
            ? localize({ en: "Resume work", zh: "恢复任务" })
            : localize({ en: "Resume subtree", zh: "恢复子树" });
    ...
    <div className="max-w-3xl space-y-6">
      ...
      {issue.hiddenAt && (
        <div className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          ...
          {localize({ en: "This task is hidden", zh: "此任务已隐藏" })}
        </div>
      )}
      {activePauseHold && (
        <div className="rounded-md border border-amber-500/35 bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200">
          {activePauseHold.isRoot ? (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium">
                  {childIssues.length === 0
                    ? localize({ en: "Paused by board.", zh: "已由看板暂停。" })
                    : localize({ en: "Subtree pause is active.", zh: "子树暂停已生效。" })}
                </span>
                <span className="text-xs text-amber-900/80 dark:text-amber-100/80">
                  {childIssues.length === 0
                    ? localize({
                      en: "Task execution is held until resume. Human comments can still wake the assignee for triage.",
                      zh: "任务执行已暂停，直到恢复。人工评论仍可唤醒负责人进行分流。",
                    })
                    : localize({
                      en: "Root and descendant execution is held until resume. Human comments can still wake assignees for triage.",
                      zh: "根任务和子任务执行已暂停，直到恢复。人工评论仍可唤醒负责人进行分流。",
                    })}
                </span>
              </div>
              <div className="text-xs text-amber-900/80 dark:text-amber-100/80">
                {childIssues.length === 0
                  ? localize({ en: "1 task held", zh: "已暂停 1 个任务" })
                  : localize({
                    en: `${heldDescendantCount} descendant${heldDescendantCount === 1 ? "" : "s"} held`,
                    zh: `已暂停 ${heldDescendantCount} 个子任务`,
                  })}
                {activeRootPauseHold?.createdAt
                  ? ` · ${localize({ en: "started", zh: "开始于" })} ${localizedRelativeTime(activeRootPauseHold.createdAt)}`
                  : ""}
              </div>
              {canShowSubtreeControls || canResumeLeafWork ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {localize({ en: "View affected", zh: "查看受影响项" })} ({childIssues.length === 0 ? 1 : heldDescendantCount})
                  </Button>
                  ...
                  >
                      {localize({ en: "Cancel subtree...", zh: "取消子树..." })}
                    </Button>
                  ) : null}
                </div>
              ) : null}
            ...
            <div className="text-xs">
              {localize({ en: "This task is paused by ancestor", zh: "此任务因上级任务暂停" })}{" "}
              ...
              . {localize({
                en: "Resume from the root task to deliver deferred work.",
                zh: "请从根任务恢复，以交付延后的工作。",
              })}
            </div>
          )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {issue.originKind === "issue_productivity_review" ? (
            <span
              ...
              title={localize({ en: "This task is a productivity review.", zh: "此任务是生产力审查。" })}
            >
              <Eye className="h-3 w-3" />
              {localize({ en: "Productivity review", zh: "生产力审查" })}
            </span>
          ) : null}
          ...
          {issue.originKind === "task_watchdog" ? (
            <span
              ...
              title={localize({
                en: "This task is a generated watchdog task. It verifies whether stopped work in the watched task tree is legitimate.",
                zh: "此任务是生成的看门狗任务，用于验证被监控任务树中的停止工作是否合理。",
              })}
            >
              <ScanEye className="h-3 w-3" />
              {localize({ en: "Watchdog", zh: "看门狗" })}
            </span>
          ) : null}
          ...
          {hasAssignedBacklogBlocker(issue.blockedBy) ? (
            <span
              ...
              title={localize({
                ...
                zh: "被停放工作阻塞 — 至少一个已分配阻塞项在 backlog 中，不会唤醒负责人。",
              })}
            >
              <Flag className="h-3 w-3" />
              {localize({ en: "Blocked by parked work", zh: "被停放工作阻塞" })}
            </span>
          ) : null}
          ...
          {!(isMobile && isFromInbox) && (
            <div className="ml-auto flex items-center gap-0.5 md:hidden shrink-0">
              <Button
                ...
                title={localize({ en: "Copy task as markdown", zh: "复制任务为 Markdown" })}
              >
              ...
              title={localize({ en: "Properties", zh: "属性" })}
            >
          ...
          <div className="hidden md:flex items-center md:ml-auto shrink-0">
            ...
            <Button
              ...
              title={localize({ en: "Show properties", zh: "显示属性" })}
            >
              <SlidersHorizontal className="h-4 w-4" />
            </Button>
            ...
            <Popover open={moreOpen} onOpenChange={setMoreOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon-xs"
                  className="shrink-0"
                  aria-label={localize({ en: "More task actions", zh: "更多任务操作" })}
                  title={localize({ en: "More task actions", zh: "更多任务操作" })}
                  onKeyDown={(event) => {
            ...
            <PopoverContent className="w-52 p-1" align="end">
              ...
              {canShowSubtreeControls ? (
                <>
                  ...
                  >
                    <PauseCircle className="h-3 w-3" />
                    {localize({ en: "Pause subtree...", zh: "暂停子树..." })}
                  </button>
                  {canResumeSubtree ? (
                    <button
                      className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
                  ...
                  {canRestoreSubtree ? (
                    ...
                    >
                      <Repeat className="h-3 w-3" />
                      {localize({ en: "Restore subtree...", zh: "还原子树..." })}
                    </button>
                  ) : null}
                </>
              ) : null}
        ...
        <InlineEditor
          ...
          placeholder={localize({ en: "Add a description...", zh: "添加描述..." })}
          multiline
          foldable
      ...
      <Dialog open={treeControlOpen} onOpenChange={setTreeControlOpen}>
        <DialogContent className="flex max-h-[calc(100dvh-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[560px]">
          <DialogHeader className="border-b border-border/60 px-6 pb-4 pr-12 pt-6">
            <DialogTitle>{issueTreeControlLabel(treeControlMode, treeControlScope, localize)}</DialogTitle>
            <DialogDescription>
              {issueTreeControlHelpText(treeControlMode, treeControlScope, localize)}
            </DialogDescription>
          </DialogHeader>
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
            {treeControlMode === "cancel" ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                {localize({
                  en: "Cancelling a subtree is destructive. Non-terminal tasks will be marked cancelled, and running or queued work will be interrupted where possible.",
                  zh: "取消子树是破坏性操作。非终态任务会被标记为已取消，运行中或排队中的工作会尽可能被中断。",
                })}
              </div>
            ) : null}
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {localize({ en: "Reason (optional)", zh: "原因（可选）" })}
              </label>
              <Textarea
                value={treeControlReason}
                onChange={(event) => setTreeControlReason(event.target.value)}
                placeholder={localize({
                  en: "Explain why this subtree control is being applied...",
                  zh: "说明为什么要应用此子树控制...",
                })}
                ...
                <label className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    <span className="block font-medium">
                      {localize({ en: "Wake affected agents", zh: "唤醒受影响的 agent" })} ({previewAffectedAgentCount})
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {previewAffectedAgentCount === 0
                        ? localize({
                          en: "No assigned agents are eligible to wake from this preview.",
                          zh: "此预览中没有可唤醒的已分配 agent。",
                        })
                        : localize({
                          en: "Wake assigned agents after this operation completes.",
                          zh: "操作完成后唤醒已分配的 agent。",
                        })}
                    </span>
                  </span>
                </label>
            ...
            {treeControlMode === "cancel" ? (
              <label className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-2 text-sm">
                ...
                <span>
                  {localize({
                    en: `I understand this will cancel ${previewAffectedIssueCount} tasks.`,
                    zh: `我理解这将取消 ${previewAffectedIssueCount} 个任务。`,
                  })}
                </span>
              </label>
            ) : null}
            ...
            <div className="space-y-2">
              {treeControlPreviewLoading ? (
                ...
                <div className="space-y-2">
                  <p className="text-xs text-destructive">{treeControlPreviewErrorCopy(treeControlPreviewError, localize)}</p>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                  ...
                  >
                    {localize({ en: "Retry preview", zh: "重试预览" })}
                  </Button>
                </div>
              ) : treeControlPreview ? (
                <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {localize({ en: "Preview unavailable.", zh: "预览不可用。" })}
                </p>
              )}
            </div>
          </div>
          <DialogFooter className="border-t border-border/60 bg-background px-6 py-4">
            ...
            >
              {executeTreeControl.isPending ? localize({ en: "Applying...", zh: "正在应用..." }) : treeControlPrimaryButtonLabel}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Sheet open={mobilePropsOpen} onOpenChange={setMobilePropsOpen}>
        <SheetContent side="bottom" className="max-h-[85dvh] pb-[env(safe-area-inset-bottom)]">
          <SheetHeader>
            <SheetTitle className="text-sm">{localize({ en: "Properties", zh: "属性" })}</SheetTitle>
          </SheetHeader>
