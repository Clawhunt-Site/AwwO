...
import { DiscoveryGrid, SkillDetailPage, getSkillVersionDiffSelection, isUpstreamCatalogSkill, type DiscoveryCard } from "./CompanySkills";
import { resolveSkillRouteToken } from "../lib/company-skill-routes";
...
async function renderDiscovery(props: Partial<ComponentProps<typeof DiscoveryGrid>> = {}) {
  container = document.createElement("div");
  document.body.appendChild(container);
  root = createRoot(container);
  await act(async () => {
...
describe("DiscoveryGrid local-only store", () => {
  it("renders only the All and Installed tabs (upstream Catalog/Bundled removed)", async () => {
    const node = await renderDiscovery({ tabCounts: { all: 3, installed: 3 } });
  ...
  it("never offers Browse catalog and the empty state only creates a local skill", async () => {
    const node = await renderDiscovery({ totalCount: 0, cards: [] });
    expect((node.textContent ?? "").toLowerCase()).not.toContain("browse catalog");
    expect(buttonsNamed(node, "Create a skill").length).toBeGreaterThan(0);
  });
});

describe("isUpstreamCatalogSkill", () => {
  it("flags catalog-sourced skills (catalog badge / bundled / optional kind)", () => {
    expect(isUpstreamCatalogSkill({ key: "anthropic/claude-api", sourceBadge: "catalog", catalogKind: null })).toBe(true);
    expect(isUpstreamCatalogSkill({ key: "anthropic/claude-api", sourceBadge: "catalog", catalogKind: "bundled" })).toBe(true);
    expect(isUpstreamCatalogSkill({ key: "yuki/commit-perfect", sourceBadge: "catalog", catalogKind: "optional" })).toBe(true);
    expect(
      isUpstreamCatalogSkill({ key: "x/y", sourceBadge: "url", catalogKind: null, metadata: { catalogKind: "bundled" } }),
    ).toBe(true);
  });

  it("flags app-shipped bundled skills by their reserved key namespace", () => {
    // The list DTO drops metadata and leaves catalogKind null for disk-bundled
    // skills (they look like an editable company skill with a "paperclip" badge),
    // so the reserved key prefix is the only client-visible upstream signal.
    expect(isUpstreamCatalogSkill({ key: "paperclipai/paperclip/release-announcement", sourceBadge: "paperclip", catalogKind: null })).toBe(true);
  });

  it("keeps company-authored and user-imported local skills", () => {
    // User-authored managed company skill: "paperclip" badge but NOT in the
    // reserved namespace — must stay.
    expect(isUpstreamCatalogSkill({ key: "company-acme/my-skill", sourceBadge: "paperclip", catalogKind: null })).toBe(false);
    expect(isUpstreamCatalogSkill({ key: "owner/repo", sourceBadge: "github", catalogKind: null })).toBe(false);
    expect(isUpstreamCatalogSkill({ key: "https://x/y", sourceBadge: "url", catalogKind: null })).toBe(false);
    expect(isUpstreamCatalogSkill({ key: "scan/local", sourceBadge: "local", catalogKind: null })).toBe(false);
  });
});

describe("local-only skill route resolution", () => {
  // The detail route resolves against the SAME upstream-filtered list as the grid,
  // so an official/bundled skill cannot be surfaced through a direct deep link.
  const upstream = { id: "11111111-1111-1111-1111-111111111111", slug: "release-announcement", key: "paperclipai/paperclip/release-announcement", sourceBadge: "paperclip" as const, catalogKind: null };
  const local = { id: "22222222-2222-2222-2222-222222222222", slug: "my-skill", key: "company-acme/my-skill", sourceBadge: "paperclip" as const, catalogKind: null };
  const installed = [upstream, local];
  const localSkills = installed.filter((skill) => !isUpstreamCatalogSkill(skill));

  it("resolves a local skill token but never an upstream one", () => {
    expect(resolveSkillRouteToken("my-skill", localSkills).skill?.id).toBe(local.id);
    // The upstream skill is filtered out of the resolver's list → unreachable.
    expect(resolveSkillRouteToken("release-announcement", localSkills).skill).toBeNull();
    expect(resolveSkillRouteToken(upstream.id, localSkills).skill).toBeNull();
  });

  it("would have been reachable without the filter (guards against regressions)", () => {
    expect(resolveSkillRouteToken("release-announcement", installed).skill?.id).toBe(upstream.id);
  });
});
