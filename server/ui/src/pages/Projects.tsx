...
const PROJECT_SORT_OPTIONS: Array<{ field: ProjectSortField; label: LocalizedText }> = [
  { field: "name", label: { en: "Name", zh: "名称" } },
  { field: "updated", label: { en: "Updated", zh: "更新时间" } },
  { field: "created", label: { en: "Created", zh: "创建时间" } },
  { field: "targetDate", label: { en: "Target date", zh: "目标日期" } },
];
...
export function Projects() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: localize({ en: "Projects", zh: "项目" }) }]);
  ...
  const sortLabel = localize(PROJECT_SORT_OPTIONS.find((option) => option.field === sortField)?.label ?? { en: "Name", zh: "名称" });

  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={localize({ en: "Select a company to view projects.", zh: "选择一个公司查看项目。" })} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="w-fit text-xs" title={localize({ en: "Sort", zh: "排序" })}>
              <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
              <span>{localize({ en: `Sort: ${sortLabel}`, zh: `排序：${sortLabel}` })}</span>
            </Button>
          </PopoverTrigger>
                ...
                >
                  ...
                  {sortField === option.field ? (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {sortDir === "asc" ? localize({ en: "Asc", zh: "升序" }) : localize({ en: "Desc", zh: "降序" })}
                    </span>
                  ) : null}
                </button>
              ))}
        ...
        <Button size="sm" variant="outline" onClick={openNewProject}>
          ...
          {localize({ en: "Add Project", zh: "添加项目" })}
        </Button>
      </div>
      ...
      {!isLoading && projects.length === 0 && (
        <EmptyState
          icon={Hexagon}
          message={localize({ en: "No projects yet.", zh: "还没有项目。" })}
          action={localize({ en: "Add Project", zh: "添加项目" })}
      ...
      {projects.length > 0 && (
        <div className="space-y-6">
          {([
            [localize({ en: "My Projects", zh: "我的项目" }), groupedProjects.mine],
            [localize({ en: "Other Projects", zh: "其他项目" }), groupedProjects.other],
              ...
              <section key={label} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {localize({
                      en: `${sectionProjects.length} project${sectionProjects.length === 1 ? "" : "s"}`,
                      zh: `${sectionProjects.length} 个项目`,
                    })}
                  </span>
                </div>
                        ...
                        trailing={
                          <div className="flex items-center gap-3">
                            <span
                              ...
                              title={localize({
                                en: `${formatNumber(project.taskCount ?? 0)} task${(project.taskCount ?? 0) === 1 ? "" : "s"}`,
                                zh: `${formatNumber(project.taskCount ?? 0)} 个任务`,
                              })}
                            >
                              {localize({
                                en: `${formatNumber(project.taskCount ?? 0)} task${(project.taskCount ?? 0) === 1 ? "" : "s"}`,
