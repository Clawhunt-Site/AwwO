...
const activityEntityTypeCopy: Record<string, LocalizedText> = {
  issue: { en: "Issue", zh: "任务" },
  project: { en: "Project", zh: "项目" },
  goal: { en: "Goal", zh: "目标" },
  agent: { en: "Agent", zh: "Agent" },
  routine: { en: "Routine", zh: "定时任务" },
...
export function Activity() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: localize({ en: "Activity", zh: "活动" }) }]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={History} message={localize({ en: "Select a company to view activity.", zh: "选择一个公司查看活动。" })} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Select value={filter} onValueChange={setFilter}>
          <SelectTrigger className="w-[140px] h-8 text-xs">
            <SelectValue placeholder={localize({ en: "Filter by type", zh: "按类型筛选" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{localize({ en: "All types", zh: "全部类型" })}</SelectItem>
            {entityTypes.map((type) => (
              <SelectItem key={type} value={type}>
                {localize(activityEntityTypeCopy[type] ?? { en: type.charAt(0).toUpperCase() + type.slice(1), zh: type })}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      {filtered && filtered.length === 0 && (
        <EmptyState icon={History} message={localize({ en: "No activity yet.", zh: "还没有活动。" })} />
      )}
