...
function mapInviteAuthFeedback(
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message:
        "That email and password did not match an existing account. Check both fields, or create an account first if you are new here.",
    };
  }
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || "Workspace";
          ...
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo" ? "Set up this instance" : `Join ${companyDisplayName}`}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ...
                    : requiresHumanAccount
                      ? "Create your account first. If you already have one, switch to sign in and continue the invite with the same email."
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? "Company board"}</div>
              </div>
              <div className="border border-zinc-800 p-3">
            ...
            {showsAgentForm ? (
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? `Start with an account. After that, you'll come right back here to accept the invite for ${companyDisplayName}.`
                      : "Use the account that already matches this invite. If you do not have one yet, switch back to create account."}
                  </p>
                </div>
                ...
                <form
                  className="space-y-4"
                  method="post"
                  action={authMode === "sign_up" ? `${apiBase}/auth/sign-up/email` : `${apiBase}/auth/sign-in/email`}
                  onSubmit={(event) => {
                ...
                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                    ? "Already signed up before? Use the existing-account option instead so the invite lands on the right user."
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ...
                      : `This will ${
                          invite.inviteType === "bootstrap_ceo" ? "finish setting up this instance" : `grant or complete your access to ${companyDisplayName}`
                        }.`}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
