...
export function Agents() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: localize({ en: "Agents", zh: "Agent" }) }]);
  }, [localize, setBreadcrumbs]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={localize({ en: "Select a company to view agents.", zh: "选择一个公司查看 Agent。" })} />;
  }
      ...
      <EntityRow
        ...
        leading={hasInvalidOrgChain ? (
          <AlertTriangle
            ...
            aria-label={localize({ en: "Invalid reporting chain", zh: "汇报链无效" })}
          />
        ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <AgentActionButtons
                ...
                runLabel={localize({ en: "Run Heartbeat", zh: "运行心跳" })}
                showStatus={false}
              />
            </div>
            <MembershipAction
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Tabs value={tab} onValueChange={(v) => navigate(`/agents/${v}`)}>
          <PageTabBar
            items={[
              { value: "all", label: localize({ en: "All", zh: "全部" }) },
              { value: "active", label: localize({ en: "Active", zh: "活跃" }) },
              { value: "paused", label: localize({ en: "Paused", zh: "已暂停" }) },
              { value: "error", label: localize({ en: "Error", zh: "错误" }) },
            ]}
            value={tab}
        ...
        <div className="flex items-center gap-2">
          ...
          {!forceListView && (
            <div className="flex items-center border border-border">
              <button
                ...
                aria-label={localize({ en: "List view", zh: "列表视图" })}
              >
              ...
              <button
                ...
                onClick={() => setView("org")}
                title={localize({ en: "Org view", zh: "组织视图" })}
                aria-label={localize({ en: "Org view", zh: "组织视图" })}
              >
          ...
          <Button size="sm" variant="outline" onClick={openNewAgent}>
            ...
            {localize({ en: "New Agent", zh: "新建 Agent" })}
          </Button>
        </div>
      </div>
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {localize({
            en: `${filtered.length} agent${filtered.length !== 1 ? "s" : ""}`,
            zh: `${filtered.length} 个 Agent`,
          })}
        </p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message={localize({ en: "Create your first agent to get started.", zh: "创建第一个 Agent 开始使用。" })}
          action={localize({ en: "New Agent", zh: "新建 Agent" })}
      ...
      {effectiveView === "list" && agents && agents.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {localize({ en: "No agents match the selected status.", zh: "没有 Agent 匹配所选状态。" })}
        </p>
      )}
      ...
      {effectiveView === "org" && orgTree && orgTree.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {localize({ en: "No organizational hierarchy defined.", zh: "尚未定义组织层级。" })}
        </p>
      )}
    </div>
  );
...
function AgentMetaColumns({
  ...
  const environmentLabel = localizedEnvironmentLabel(environment.label, localize);
  const environmentDetail = localizedEnvironmentDetail(environment.detail, localize);
  return (
    <>
      ...
      {showEnvironment && (
        <div className="w-44 min-w-0 leading-tight">
          <div className="truncate text-xs text-muted-foreground" title={`${environmentLabel} - ${environmentDetail}`}>
            {environmentLabel}
          </div>
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {localize({ en: "Live", zh: "实时" })}{liveCount > 1 ? ` (${liveCount})` : ""}
      </span>
    </Link>
  );
}

function localizedEnvironmentLabel(
  label: string,
  localize: (copy: { en: string; zh: string }) => string,
): string {
  if (label === "Local") return localize({ en: "Local", zh: "本地" });
  if (label === "Unknown environment") return localize({ en: "Unknown environment", zh: "未知环境" });
  if (label === "—") return label;
  return label;
}

function localizedEnvironmentDetail(
  detail: string,
  localize: (copy: { en: string; zh: string }) => string,
): string {
  if (detail === "Local host") return localize({ en: "Local host", zh: "本地主机" });
  if (detail === "Loading environment") return localize({ en: "Loading environment", zh: "正在加载环境" });
  if (detail.endsWith(" sandbox provider")) {
    const provider = detail.slice(0, -" sandbox provider".length);
    return localize({ en: detail, zh: `${provider} 沙盒提供方` });
  }
  return detail;
}
