        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              ...
              {event.actorAgent ? ` ${localize({ en: "by", zh: "由" })} ${event.actorAgent.name}` : null}
            </p>
          </div>
        </div>
      ))}
  ...
  const saveStage = useMutation({
    ...
    onError: async (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "The system could not save the stage.",
        tone: "error",
      });
    },
  });
      ...
      pushToast({
        ...
        body: error instanceof ApiError
          ...
          : error instanceof Error
            ? error.message
            : "The system could not save the stage secrets.",
        tone: "error",
      });
    },
  });
  ...
  const deleteStage = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "The system could not delete the stage.",
        tone: "error",
      });
    },
  });
  ...
  const saveStrictTransitions = useMutation({
    ...
    onError: (error) => {
      ...
      pushToast({
        ...
        body: error instanceof Error ? error.message : "The system could not update transition rules.",
        tone: "error",
      });
    },
  });
      ...
      <div className="flex items-start justify-between gap-4 border-b border-border p-4">
        <div className="space-y-1">
          ...
          <p className="max-w-md text-sm text-muted-foreground">
            The agent decides what the pieces are. The workflow creates and tracks them.
          </p>
        </div>
        <ToggleSwitch
                            ...
                            <FieldRow label="Project context">
                              ...
                              {selectedAutomationProject && !selectedAutomationProjectWorkspace ? (
                                <p className="mt-2 text-xs text-muted-foreground">
                                  This project has no saved workspace default. Automation will use the project fallback when it runs.
                                </p>
                              ) : null}
                            </FieldRow>
