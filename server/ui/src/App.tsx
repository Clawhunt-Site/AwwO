...
import { EmbeddedHostProvider, useEmbeddedHost, type EmbeddedHostIntegration } from "./context/EmbeddedHostContext";
...
// Company directory (Team home) — full-page, no per-company sidebar/breadcrumb. It is a
// top-level (unprefixed) route, so the per-company Layout never wraps it; the embedding
// host (apps/web) provides the only chrome ("Back to main menu"). Its own empty state
// invites creating the first company when there are none.
function CompanyDirectoryHome() {
  return (
...
function NoCompaniesStartPage() {
  ...
  const embeddedHost = useEmbeddedHost();
  const createCompany = embeddedHost?.onCreateCompany ?? openOnboarding;
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {embeddedHost
            ? t("app.noCompanies.embeddedDescription", { defaultValue: "Create a company to start working here." })
...
export function App({ embeddedHost = null }: { embeddedHost?: EmbeddedHostIntegration | null } = {}) {
  return (
    <EmbeddedHostProvider value={embeddedHost}>
      <Routes>
        ...
        <Route element={<CloudAccessGate />}>
          ...
          <Route path="instance/settings/*" element={<LegacySettingsRedirect />} />
          <Route path="companies" element={<CompanyDirectoryHome />} />
      ...
      {embeddedHost ? null : <OnboardingWizardVariant />}
