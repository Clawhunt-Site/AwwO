...
function createTimelineEventMessage(args: {
  ...
  const localize = args.localize ?? ((copy: LocalizedText) => copy.en);
  const noneLabel = localize({ en: "none", zh: "无" });
  const unassignedLabel = localize({ en: "Unassigned", zh: "未分配" });
  const actorName = event.actorType === "agent"
    ? (agentMap?.get(event.actorId)?.name ?? event.actorId.slice(0, 8))
    : event.actorType === "system"
      ? localize({ en: "System", zh: "系统" })
      : (formatAssigneeUserLabel(event.actorId, currentUserId, userLabelMap) ?? localize({ en: "Board", zh: "看板" }));

  const lines: string[] = [
    event.followUpRequested
      ? `${actorName} ${localize({ en: "requested follow-up", zh: "请求跟进" })}`
      : `${actorName} ${localize({ en: "updated this issue", zh: "更新了此任务" })}`,
  ];
  if (event.statusChange) {
    lines.push(
      `${localize({ en: "Status", zh: "状态" })}: ${event.statusChange.from ?? noneLabel} -> ${event.statusChange.to ?? noneLabel}`,
    );
  }
  if (event.assigneeChange) {
    const from = event.assigneeChange.from.agentId
      ...
      : (formatAssigneeUserLabel(event.assigneeChange.from.userId, currentUserId, userLabelMap) ?? unassignedLabel);
    const to = event.assigneeChange.to.agentId
      ...
      : (formatAssigneeUserLabel(event.assigneeChange.to.userId, currentUserId, userLabelMap) ?? unassignedLabel);
    lines.push(`${localize({ en: "Assignee", zh: "负责人" })}: ${from} -> ${to}`);
  }
  if (event.workspaceChange) {
    lines.push(
      `${localize({ en: "Workspace", zh: "工作区" })}: ${event.workspaceChange.from.label ?? noneLabel} -> ${event.workspaceChange.to.label ?? noneLabel}`,
    );
  }
...
export function formatDurationWords(ms: number | null, locale: "en" | "zh" = "en") {
  if (ms === null || !Number.isFinite(ms) || ms <= 0) return null;
  const totalSeconds = Math.max(1, Math.round(ms / 1000));
  if (totalSeconds < 60) {
    if (locale === "zh") return `${totalSeconds}秒`;
    return `${totalSeconds} second${totalSeconds === 1 ? "" : "s"}`;
  }
  const totalMinutes = Math.round(totalSeconds / 60);
  if (totalMinutes < 60) {
    if (locale === "zh") return `${totalMinutes}分钟`;
...
function createHistoricalTranscriptMessage(args: {
  ...
  const message: ThreadAssistantMessage = {
    ...
    metadata: createAssistantMetadata({
      ...
      notices,
      waitingText,
      waitingTextZh: waitingText ? "运行已完成" : "",
      chainOfThoughtLabel: runDurationLabel(run),
      chainOfThoughtLabelZh: runDurationLabel(run, "zh"),
...
export function buildIssueChatMessages(args: {
  ...
  for (const event of sortByCreated(timelineEvents)) {
    orderedMessages.push({
      createdAtMs: toTimestamp(event.createdAt),
      order: 0,
      message: createTimelineEventMessage({ event, agentMap, currentUserId, userLabelMap, localize }),
    });
  }
