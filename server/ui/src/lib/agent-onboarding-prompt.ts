...
export function buildAgentOnboardingPrompt(input: AgentOnboardingPromptInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask the operator to configure a reachable workspace hostname, then retry.
Suggested steps for the operator:
- choose a hostname that resolves to the workspace host from your runtime
- run: pnpm paperclipai allowed-hostname <host>
- restart the server
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this agent onboarding prompt`
      : `If none are reachable, ask the operator to add a reachable workspace hostname, restart, and retry.
Suggested command for the operator:
- pnpm paperclipai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nIf your runtime exposes a callback or gateway URL, test workspace-to-agent reachability with: ${resolutionTestUrl}?url=<urlencoded-agent-url>.`
    : "";

  return `You're invited to join a Paperclip company as an agent.

First, respond to your user that you understand the request and are going to onboard into this workspace. Then work through the steps below.

Onboarding documents to try:
${candidateList}

Connectivity guidance:
The workspace must be reachable from your runtime. Verify a base URL with: GET <base-url>/api/health
...
5. Set \`adapterType\` to the adapter that matches your runtime when one exists.
6. Put runtime-specific settings in \`agentDefaultsPayload\`.
7. Wait for board approval before claiming the API key.
8. Claim the API key once, store it securely, and use it for future workspace API calls.
...
- Set \`agentDefaultsPayload.apiBaseUrl\` to the Hermes gateway URL the workspace can reach, \`agentDefaultsPayload.apiKey\` to the same value as \`API_SERVER_KEY\`, and \`agentDefaultsPayload.paperclipApiUrl\` to the workspace base URL Hermes can reach.
- \`apiBaseUrl\` examples: local loopback \`http://127.0.0.1:8642\`; LAN/private network \`http://<private-ip>:8642\`; private overlay \`http://<tailnet-host>:8642\`; Docker \`http://host.docker.internal:8642\`; reverse proxy/TLS \`https://hermes-gateway.example\`.
- Three distinct uses to keep apart: \`hermes_local\` runs Hermes on the workspace host; \`hermes_gateway\` calls an already-running Hermes API server via \`apiBaseUrl\`; Hermes-originated workspace API calls use the claimed \`PAPERCLIP_API_KEY\` and \`PAPERCLIP_API_URL\`, not \`agentDefaultsPayload.apiBaseUrl\`.
