...
export interface AgentConfigOverlay {
  ...
  /** Pending edits to `runtimeConfig.pluginTools` (the persistent plugin-bridge opt-in). */
...
export function buildAgentUpdatePatch(agent: Agent, overlay: AgentConfigOverlay) {
  ...
  if (
    ...
    Object.keys(overlay.pluginTools).length > 0 ||
    ...
    // Sanitize pluginTools UNCONDITIONALLY whenever runtimeConfig is rebuilt —
    // not only when the user touched the toggle. nextRuntimeConfig is spread
    // from the existing runtimeConfig, so a legacy typo (e.g. `{ enable: true }`)
    // would otherwise ride along on an unrelated change (e.g. a heartbeat edit)
    // and the strict backend schema would reject the whole save. Write ONLY the
    // schema-known `enabled`, preferring the overlay edit, else the existing
    // valid value, dropping any stray keys.
    if (existingRc.pluginTools !== undefined || Object.keys(overlay.pluginTools).length > 0) {
      const existingPt = (existingRc.pluginTools ?? {}) as Record<string, unknown>;
      const enabled = overlay.pluginTools.enabled ?? existingPt.enabled;
      nextRuntimeConfig.pluginTools = typeof enabled === "boolean" ? { enabled } : {};
    }
