...
describe("company routes", () => {
  ...
  // Regression: top-level global routes must NOT be read as a company prefix. The host
  // (apps/web) team-page topbar derives "inside a company" from
  // extractCompanyPrefixFromPath; before `companies`/`onboarding` were added to
  // GLOBAL_ROUTE_ROOTS, "/onboarding" resolved to prefix "ONBOARDING" and the topbar
  // wrongly showed the in-company back-hierarchy on the full-page directory / onboarding.
  it("treats top-level global routes as non-company (no prefix)", () => {
    for (const p of ["/", "/companies", "/onboarding", "/instance", "/docs", "/auth", "/invite/tok"]) {
      expect(extractCompanyPrefixFromPath(p)).toBeNull();
      expect(isGlobalPath(p)).toBe(true);
    }
    // A real company-prefixed route is still classified as in-company.
    expect(extractCompanyPrefixFromPath("/PAP/dashboard")).toBe("PAP");
    expect(isGlobalPath("/PAP/dashboard")).toBe(false);
  });
});
