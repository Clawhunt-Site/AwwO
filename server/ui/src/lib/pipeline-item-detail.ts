...
function actorName(event: PipelineCaseEvent, localize: Localize) {
  if (event.actorAgent?.name) return event.actorAgent.name;
  if (event.actorType === "user") return localize({ en: "Board", zh: "看板" });
  if (event.actorType === "system") return localize({ en: "System", zh: "系统" });
  return null;
}

function movementReason(payload: Record<string, unknown>, localize: Localize) {
  const reason = readString(payload.reason);
  if (!reason) return null;
  if (reason === "children_terminal") return localize({ en: "all child items done", zh: "所有子条目已完成" });
  return reason;
}
...
export function formatPipelineItemEvent(event: PipelineCaseEvent, stages?: StageLookup, localizeText?: Localize) {
  const localize = localizeText ?? defaultLocalize;
  const kind = event.type.startsWith("case.") ? event.type.slice("case.".length) : event.type;
  const payload = event.payload ?? {};
  if (kind === "ingested") return localize({ en: "Item added.", zh: "条目已添加。" });
  if (kind === "updated") {
    if (payload.action === "stage_automation_rerun_requested") {
      return localize({ en: "Stage automation re-run requested.", zh: "已请求重新运行阶段自动化。" });
    }
    return localize({ en: "Item details updated.", zh: "条目详情已更新。" });
  }
  if (kind === "transitioned") {
    const from = stageName(event, stages, "from");
    const to = stageName(event, stages, "to");
    const movement = from && to
      ? localize({ en: `Moved from ${from} to ${to}`, zh: `已从 ${from} 移动到 ${to}` })
      : to
        ? localize({ en: `Moved to ${to}`, zh: `已移动到 ${to}` })
        : localize({ en: "Moved to another stage", zh: "已移动到另一阶段" });
    const reason = movementReason(payload, localize);
    const transitionClass = movementClass(event, payload);
    if (transitionClass === "automatic") {
      return `${movement} — ${localize({ en: "automatic", zh: "自动" })}${reason ? ` (${reason})` : ""}.`;
    }
    const actor = actorName(event, localize);
  ...
  if (kind === "suggested" || kind === "transition_suggested") {
    ...
    const to = stageNameFromLookup(stages, toStageKey) ?? localize({ en: "the next stage", zh: "下一阶段" });
    return localize({ en: `Suggested moving to ${to}.`, zh: `建议移动到 ${to}。` });
  }
  if (kind === "suggestion_resolved") {
    const decision = readDecision(payload);
    if (decision === "accept") return localize({ en: "Suggestion approved.", zh: "建议已通过。" });
    if (decision === "dismiss") return localize({ en: "Suggestion dismissed.", zh: "建议已忽略。" });
    return localize({ en: "Suggestion resolved.", zh: "建议已处理。" });
  }
  if (kind === "reviewed" || kind === "review_decided") {
    const decision = readDecision(payload);
    if (decision === "request_changes") return localize({ en: "Review requested changes.", zh: "审核要求修改。" });
    if (decision === "drop" || decision === "reject") return localize({ en: "Review removed this item.", zh: "审核已移除此条目。" });
    if (decision === "approve") return localize({ en: "Review approved this item.", zh: "审核已通过此条目。" });
    return localize({ en: "Review completed.", zh: "审核已完成。" });
  }
  if (kind === "conversation_opened") return localize({ en: "Conversation started.", zh: "会话已开始。" });
  if (kind === "issue_linked") return localize({ en: "Linked to work.", zh: "已关联任务。" });
  if (kind === "issue_unlinked") return localize({ en: "Work link removed.", zh: "任务关联已移除。" });
  if (kind === "blockers_set") return localize({ en: "Waiting items updated.", zh: "等待项已更新。" });
  if (kind === "blockers_resolved") return localize({ en: "Waiting items cleared.", zh: "等待项已清除。" });
  if (kind === "children_terminal") return localize({ en: "Built-from items completed.", zh: "构建来源条目已完成。" });
  if (kind === "upstream_drift") {
    const upstreamCaseKey = readString(payload.upstreamCaseKey);
    if (upstreamCaseKey) {
      return localize({ en: `Upstream change detected from ${upstreamCaseKey}.`, zh: `检测到来自 ${upstreamCaseKey} 的上游变更。` });
    }
    return localize({ en: "Upstream change detected.", zh: "检测到上游变更。" });
  }
  if (kind === "drift_acknowledged") return localize({ en: "Upstream change acknowledged.", zh: "上游变更已确认。" });
  if (kind === "automation_executed") {
    ...
    return localize({
      en: `Automation completed — ran ${routineName}${issueLabel ? ` -> ${issueLabel}` : ""}.`,
      zh: `自动化已完成 — 已运行 ${routineName}${issueLabel ? ` -> ${issueLabel}` : ""}。`,
    });
  }
  if (kind === "automation_failed") {
    const reason = readString(payload.error);
    return localize({
      en: `Automation needs attention${reason ? ` — ${humanizeReason(reason)}` : ""}.`,
      zh: `自动化需要处理${reason ? ` — ${humanizeReason(reason)}` : ""}。`,
    });
  }
  if (kind === "claimed") return localize({ en: "Work started.", zh: "任务已开始。" });
  if (kind === "lease_released" || kind === "lease_expired") return localize({ en: "Work handoff cleared.", zh: "任务交接已清除。" });
  return localize({ en: "Activity recorded.", zh: "活动已记录。" });
}
