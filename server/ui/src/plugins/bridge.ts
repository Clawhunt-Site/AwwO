 ...
 *
 * Makes an HTTP POST to `${apiBase}/plugins/:pluginId/data/:key` and returns
 ...
 * `${apiBase}/plugins/:pluginId/actions/:key` and returns the worker result.
 *
...
export function usePluginStream<T = unknown>(
  ...
  useEffect(() => {
    ...
    const source = new EventSource(
      `${apiBase}/plugins/${encodeURIComponent(pluginId)}/bridge/stream/${encodeURIComponent(channel)}?${params.toString()}`,
