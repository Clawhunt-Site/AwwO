...
export function Layout() {
  ...
  const boardChrome = useBoardChrome();
  // The embedding host (super's Team panel) can request a horizontal top-nav
  // instead of the board's left sidebar. Desktop-only: mobile keeps its
  // off-canvas drawer + MobileBottomNav. Presentation only — same routes; the
  // host owns the surrounding chrome (back button, account/theme), so the
  // sidebar's account menu is dropped along with the rail in this mode.
  const useTopNav = boardChrome === "top-nav" && !isMobile;
  const {
    companies,
  ...
  return (
    <GeneralSettingsProvider value={{ keyboardShortcutsEnabled }}>
      ...
      >
        {localize({ en: "Skip to Main Content", zh: "跳到主要内容" })}
      </a>
      <WorktreeBanner />
      ...
      <div className={cn("min-h-0 flex-1", isMobile ? "w-full" : "flex overflow-clip")}>
        {isMobile && sidebarOpen && (
          <button
            ...
            aria-label={localize({ en: "Close sidebar", zh: "关闭侧边栏" })}
          />
        )}
