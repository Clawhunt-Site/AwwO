...
describe("SidebarNavItem", () => {
  ...
  it("surfaces the live count in Chinese when the board locale is Chinese", async () => {
    await i18n.changeLanguage("zh-CN");
    sidebarState.collapsed = true;
    render(<SidebarNavItem to="/dashboard" label="仪表盘" icon={Inbox} liveCount={3} />);

    expect(link().getAttribute("aria-label")).toBe("仪表盘, 3 运行中");
  });
});
