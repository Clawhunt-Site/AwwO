...
export function RunButton({
  ...
  const resolvedLabel = label ?? localize({ en: "Run now", zh: "立即运行" });
  return (
...
export function PauseResumeButton({
  ...
  if (isPaused) {
    return (
      <Button variant="outline" size={size} onClick={onResume} disabled={disabled}>
        <Play className="h-3.5 w-3.5 sm:mr-1" />
        <span className="hidden sm:inline">{localize({ en: "Resume", zh: "恢复" })}</span>
      </Button>
    );
  }
  ...
  return (
    <Button variant="outline" size={size} onClick={onPause} disabled={disabled}>
      ...
      <span className="hidden sm:inline">{localize({ en: "Pause", zh: "暂停" })}</span>
    </Button>
  );
}
...
export function ClearErrorButton({
  ...
  return (
    <Button
      ...
      aria-label={localize({
        en: "Clear error and return agent to idle",
        zh: "清除错误并让 Agent 回到空闲状态",
      })}
    >
      <CheckCircle2 className="h-3.5 w-3.5 sm:mr-1" />
      <span className="hidden sm:inline">{localize({ en: "Clear error", zh: "清除错误" })}</span>
    </Button>
  );
}
...
export function AgentActionButtons({
  ...
  const reportError = useCallback(
    (message: string) => {
      if (onActionError) {
        ...
        pushToast({ title: localize({ en: "Action failed", zh: "操作失败" }), body: message, tone: "error" });
      }
    },
    [localize, onActionError, pushToast],
  );
  ...
  const agentAction = useMutation({
    ...
    onError: (err) => {
      reportError(err instanceof Error ? err.message : localize({ en: "Action failed", zh: "操作失败" }));
    },
  });
  ...
  const duplicateAgent = useMutation({
    mutationFn: async () => {
      if (!resolvedCompanyId) {
        throw new Error(localize({ en: "Agent is not ready to duplicate", zh: "Agent 还不能复制" }));
      }
    ...
    onSuccess: async (createdAgent) => {
      ...
      pushToast({ title: localize({ en: "Agent duplicated", zh: "Agent 已复制" }), body: createdAgent.name, tone: "success" });
      navigate(`/agents/${agentRouteRef(createdAgent)}/dashboard`);
    },
    onError: (err) => {
      const message = err instanceof Error ? err.message : localize({ en: "Failed to duplicate agent", zh: "复制 Agent 失败" });
      onActionError?.(message);
      pushToast({ title: localize({ en: "Could not duplicate agent", zh: "无法复制 Agent" }), body: message, tone: "error" });
    },
  });
  ...
  const handleDuplicateAgent = useCallback(() => {
    ...
    const confirmed = window.confirm(localize({
      en: `Duplicate ${agent.name} as ${nextName}?`,
      zh: `将 ${agent.name} 复制为 ${nextName}？`,
    }));
    setMoreOpen(false);
    if (!confirmed) return;
    duplicateAgent.mutate();
  }, [agent.name, duplicateAgent, localize]);
  ...
  const resetTaskSession = useMutation({
    ...
    onError: (err) => {
      reportError(err instanceof Error ? err.message : localize({ en: "Failed to reset session", zh: "重置会话失败" }));
    },
  });
  ...
  const resolvedAssignLabel = assignLabel ?? localize({ en: "Assign Task", zh: "分配任务" });
  const resolvedRunLabel = runLabel ?? localize({ en: "Run now", zh: "立即运行" });
      ...
      <Popover open={moreOpen} onOpenChange={setMoreOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="ghost"
            size="icon-xs"
            aria-label={localize({ en: `Open actions for ${agent.name}`, zh: `打开 ${agent.name} 的操作菜单` })}
          >
        ...
        <PopoverContent className="w-44 p-1" align="end">
          ...
          >
            ...
            {localize({ en: "Duplicate Agent", zh: "复制 Agent" })}
          </button>
          <button
            className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
            onClick={() => {
          ...
          >
            <Copy className="h-3 w-3" />
            {localize({ en: "Copy Agent ID", zh: "复制 Agent ID" })}
          </button>
          <button
            className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
            onClick={() => {
          ...
          >
            ...
            {localize({ en: "Reset Sessions", zh: "重置会话" })}
          </button>
          <button
