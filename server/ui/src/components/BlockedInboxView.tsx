...
export function BlockedInboxView({
  ...
  if (error) {
    const message =
      error instanceof Error ? error.message : localize({ en: "Couldn't load the Blocked tab.", zh: "无法加载“已阻塞”标签。" });
    return (
      <div
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">
              {localize({ en: "Couldn't load the Blocked tab.", zh: "无法加载“已阻塞”标签。" })}
            </p>
            <p className="text-xs opacity-80">
              {localize({ en: "Other Inbox tabs still work.", zh: "其他 Inbox 标签仍可使用。" })} {message}
            </p>
          </div>
          <Button
            type="button"
          ...
          >
            {isFetching
              ? localize({ en: "Trying...", zh: "正在重试..." })
              : localize({ en: "Try again", zh: "重试" })}
          </Button>
        </div>
      </div>
    );
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">
            {localize({ en: "No work is stopped.", zh: "没有停止的工作。" })}
          </p>
          <p className="text-xs text-muted-foreground">
            {localize({
              en: "Tasks that need a decision, recovery, or external action will appear here.",
              zh: "需要决策、恢复或外部操作的任务会显示在这里。",
            })}
          </p>
        </div>
      </div>
  ...
  if (groups.length === 0) {
    return (
      <div className="space-y-3">
        ...
        >
          {localize({ en: "No stopped items match your search.", zh: "没有停止项匹配搜索。" })}
        </div>
      </div>
    );
  }
            ...
            return (
              <div key={group.variant} data-testid={`blocked-inbox-group-${group.variant}`}>
                <div className="px-3 sm:px-4">
                  <IssueGroupHeader
                    label={`${blockedVariantLabel(group.variant, language)} · ${group.rows.length}`}
                    collapsible
...
function BlockedInboxRow({
  ...
  const stoppedAge = formatStoppedAge(row.attention.stoppedSinceAt, Date.now(), language);
