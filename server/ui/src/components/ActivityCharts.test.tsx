...
import { PriorityChart, RunActivityChart, SuccessRateChart } from "./ActivityCharts";
...
describe("ActivityCharts", () => {
  ...
  it("renders dashboard chart empty states and tooltips in Chinese", async () => {
    await i18n.changeLanguage("zh-CN");

    render(<RunActivityChart activity={undefined} />);
    expect(container.textContent).toContain("暂无运行");

    render(<PriorityChart issues={[]} />);
    expect(container.textContent).toContain("暂无任务");

    render(<RunActivityChart runs={[createRun()]} />);
    expect(container.querySelector("[title='2026-04-20: 1 次运行']")).not.toBeNull();
  });
});
