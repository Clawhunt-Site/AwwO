...
function buildStatusOptions(localize: (copy: LocalizedText) => string): ReadonlyArray<{ value: string; label: string; color: string; description?: string }> {
  const palette = issueStatusText;
  return [
    {
      value: "backlog",
      label: localize({ en: "Backlog", zh: "待办池" }),
      color: palette.backlog ?? issueStatusTextDefault,
      description: localize({ en: "Parked - assignee will not be woken", zh: "已搁置 - 不会唤醒负责人" }),
    },
    {
      value: "todo",
      label: localize({ en: "Todo", zh: "待办" }),
      color: palette.todo ?? issueStatusTextDefault,
      description: localize({ en: "Executable - assignee will be woken", zh: "可执行 - 会唤醒负责人" }),
    },
    { value: "in_progress", label: localize({ en: "In Progress", zh: "进行中" }), color: palette.in_progress ?? issueStatusTextDefault },
    { value: "in_review", label: localize({ en: "In Review", zh: "审核中" }), color: palette.in_review ?? issueStatusTextDefault },
    { value: "done", label: localize({ en: "Done", zh: "已完成" }), color: palette.done ?? issueStatusTextDefault },
  ];
}

const priorities = [
  { value: "critical", label: { en: "Critical", zh: "紧急" }, icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault },
  { value: "high", label: { en: "High", zh: "高" }, icon: ArrowUp, color: priorityColor.high ?? priorityColorDefault },
  { value: "medium", label: { en: "Medium", zh: "中" }, icon: Minus, color: priorityColor.medium ?? priorityColorDefault },
  { value: "low", label: { en: "Low", zh: "低" }, icon: ArrowDown, color: priorityColor.low ?? priorityColorDefault },
];
...
const IssueDescriptionEditor = memo(function IssueDescriptionEditor({
  ...
  return (
    <MarkdownEditor
      ...
      placeholder={localize({ en: "Add description...", zh: "添加描述..." })}
      bordered={false}
      mentions={mentions}
...
export function NewIssueDialog() {
  ...
  const statuses = useMemo(() => buildStatusOptions(localize), [localize]);
      ...
      >
                    ...
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={localize({ en: "Remove attachment", zh: "移除附件" })}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
              ...
              >
                {currentPriority ? (
                  <>
                    ...
                    {localize(currentPriority.label)}
                  </>
                ) : (
                  <>
                    <Minus className="h-3 w-3 text-muted-foreground" />
                    {localize({ en: "Priority", zh: "优先级" })}
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start" data-testid="new-issue-more-menu">
              ...
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {localize({ en: "Start date", zh: "开始日期" })}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {localize({ en: "Due date", zh: "截止日期" })}
              </button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              {localize({
                en: "Assigning implies executable intent - leave status as Backlog only to deliberately park this. The assignee will not be woken until status moves to Todo or In Progress.",
                zh: "分配负责人表示任务具备执行意图；只有在明确搁置时才保持为待办池。状态移动到待办或进行中前，不会唤醒负责人。",
              })}
            </span>
          </div>
        ) : null}
        ...
        {currentAssigneeLowTrust ? (
          ...
          >
            ...
            <span className="leading-snug">
              {localize({
                en: "Low-trust review agent. It can only act inside its assigned review boundary; issue, project, or run policy defines the concrete scope.",
                zh: "低信任评审 Agent。它只能在被分配的评审边界内行动；具体范围由任务、项目或运行策略定义。",
              })}
            </span>
          </div>
        ) : null}
