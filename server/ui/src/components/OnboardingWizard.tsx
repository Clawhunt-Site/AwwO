...
export function OnboardingWizard() {
  ...
  const {
    ...
    // resolve against the local workspace host (environmentId = null).
  ...
  return (
    ...
    >
      <DialogPortal>
        ...
        {/* data-board-portal: inert marker so an embedding host (apps/web) can scope its
            CSS to this plain-div DialogPortal (no DialogContent → no data-slot). Lets the
            host neutralize bleed (e.g. its global --accent) into this onboarding surface
            without forking this component. No-op for the standalone board. */}
        <div data-board-portal="" className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" onKeyDown={handleKeyDown}>
          {/* Super-style contained dialog panel — replaces the legacy full-bleed
              takeover with a centered, bounded card over a dimmed backdrop. The inner
              split-pane (form | ascii art) and FrontDoor are unchanged; they simply flow
              inside this panel instead of the whole viewport. */}
          <div className="relative flex w-full max-w-5xl h-[88vh] max-h-[900px] overflow-hidden rounded-2xl border border-border bg-background shadow-2xl">
          {/* Close button */}
          <button
            onClick={handleClose}
