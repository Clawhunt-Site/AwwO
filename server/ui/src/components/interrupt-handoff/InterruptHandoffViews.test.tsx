...
describe("RunStatusBadge", () => {
  ...
  it("renders run status labels in Chinese when the board locale is Chinese", async () => {
    await i18n.changeLanguage("zh-CN");
    const host = mount(<RunStatusBadge status="succeeded" />);
    const badge = host.querySelector<HTMLElement>("[data-testid='run-status-badge']")!;
    expect(badge.textContent).toContain("成功");
    expect(badge.textContent).not.toContain("succeeded");
  });
});
