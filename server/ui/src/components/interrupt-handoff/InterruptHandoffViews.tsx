...
export function RunStatusBadge({
  ...
  const label = ({
    ...
    "error": localize({ en: "error", zh: "错误" }),
    "failed": localize({ en: "failed", zh: "失败" }),
    "interrupted": localize({ en: "interrupted", zh: "已中断" }),
    "pending": localize({ en: "pending", zh: "等待中" }),
    "queued": localize({ en: "queued", zh: "排队中" }),
    "running": localize({ en: "running", zh: "运行中" }),
    "succeeded": localize({ en: "succeeded", zh: "成功" }),
    "timed out": localize({ en: "timed out", zh: "超时" }),
  } as Record<string, string>)[p.label] ?? p.label;
  const srHint = p.srHint === "interrupted by board comment"
    ? localize({ en: "interrupted by board comment", zh: "由看板评论中断" })
    : p.srHint;
  return (
    <span
...
export function PauseAffectsSummaryView({
  ...
  const bucketCopy = (bucket: PauseAffectsSummary["buckets"][number]) => {
    const copy = {
      live_runs: {
        label: { en: bucket.label, zh: "实时 agent 运行" },
        detail: { en: bucket.detail, zh: "立即中断，恢复时重新排队" },
      },
      queued_wakes: {
        label: { en: bucket.label, zh: "排队唤醒" },
        detail: { en: bucket.detail, zh: "已暂停，恢复前不会开始" },
      },
      agent_owned: {
        label: { en: bucket.label, zh: "Agent 负责" },
        detail: { en: bucket.detail, zh: "已分配给 agent，但没有实时运行" },
      },
      human_owned: {
        label: { en: bucket.label, zh: "人工负责" },
        detail: { en: bucket.detail, zh: "由看板用户负责，暂停不会通知他们" },
      },
      static: {
        label: { en: bucket.label, zh: "静态" },
        detail: { en: bucket.detail, zh: "无负责人，没有待运行工作" },
      },
    }[bucket.key];
    return {
      label: localize(copy.label),
      detail: localize(copy.detail),
    };
  };
  return (
    <div
    ...
    >
      <div className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-muted-foreground">
        ...
        {localize({ en: "What this affects", zh: "影响范围" })}
      </div>
      {summary.nothingLive ? (
        <p role="status" className="text-xs text-muted-foreground" data-testid="pause-nothing-live">
          {localize({
            en: "Nothing live to pause — no agent run is in flight or queued. This records a hold so new work won't start until you resume.",
            zh: "没有实时运行需要暂停 — 当前没有 agent 运行中或排队中。系统会记录暂停，让新工作在恢复前不会开始。",
          })}
        </p>
      ) : null}
      {visibleBuckets.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">
          {localize({ en: "No tasks are affected.", zh: "没有受影响的任务。" })}
        </p>
      )}
    </div>
  );
