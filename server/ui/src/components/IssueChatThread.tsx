...
import { useLocalizedDurationWords, useLocalizedRelativeTime, useLocalizedText, type LocalizedText } from "@/i18n/localized";
import type {
  Agent,
...
function useLiveElapsed(startMs: number | null | undefined, active: boolean): string | null {
  ...
  const formatDuration = useLocalizedDurationWords();
  useEffect(() => {
...
function IssueChatFallbackThread({
  ...
  const commentDateLabel = useCommentDateLabel();
  return (
...
function formatCommentDateLabel(date: Date | string | undefined, relative: (date: Date | string) => string): string {
  if (!date) return "";
  const then = new Date(date).getTime();
  if (Date.now() - then < WEEK_MS) return relative(date);
  return formatShortDate(date);
}

function useCommentDateLabel(): (date: Date | string | undefined) => string {
  const relative = useLocalizedRelativeTime();
  return useCallback((date: Date | string | undefined) => formatCommentDateLabel(date, relative), [relative]);
}
...
function humanizeValue(value: string | null, localize?: (copy: LocalizedText) => string) {
  if (!value) return localize ? localize({ en: "None", zh: "无" }) : "None";
...
function formatInteractionActorLabel(args: {
  ...
  if (userId) {
    return userLabelMap?.get(userId)
      ...
      ?? (localize ? localize({ en: "Board", zh: "看板" }) : "Board");
  }
  return "Paperclip";
}
...
export function resolveIssueChatHumanAuthor(args: {
  ...
  const { authorName, authorUserId, currentUserId, userProfileMap, localize } = args;
  ...
  const resolvedAuthorName = profile?.label?.trim()
    ...
    || (authorUserId === "local-board"
      ? (localize ? localize({ en: "Board", zh: "看板" }) : "Board")
      : isCurrentUser
        ? (localize ? localize({ en: "You", zh: "你" }) : "You")
        : (localize ? localize({ en: "User", zh: "用户" }) : "User"));
...
function IssueChatChainOfThought({
  ...
  if (isActive) {
    headerVerb = localize({ en: "Working", zh: "运行中" });
    if (liveElapsed) headerSuffix = localize({ en: `for ${liveElapsed}`, zh: `已运行${liveElapsed}` });
  } else if (segmentTiming) {
    const durationMs = segmentTiming.endMs - segmentTiming.startMs;
    const durationText = formatDuration(durationMs);
    headerVerb = localize({ en: "Worked", zh: "已运行" });
    if (durationText) headerSuffix = localize({ en: `for ${durationText}`, zh: durationText });
  } else {
    headerVerb = localize({ en: "Worked", zh: "已运行" });
  }
...
function IssueChatUserMessage({
  ...
  const queueBadgeLabel = queueReason === "hold"
    ? localize({ en: "\u23f8 Deferred wake", zh: "\u23f8 延后唤醒" })
    : localize({ en: "Queued", zh: "排队中" });
  ...
  const authorAvatar = (
    <Avatar size="sm" className="shrink-0">
      {avatarUrl ? <AvatarImage src={resolveContentUrl(avatarUrl)} alt={resolvedAuthorName} /> : null}
  ...
  const messageBody = (
    <div className={cn("flex min-w-0 max-w-[85%] flex-col", isCurrentUser && "items-end")}>
      ...
      >
        ...
        {deleted ? (
          <div className="text-sm italic text-muted-foreground">{localize({ en: "Comment deleted", zh: "评论已删除" })}</div>
        ) : (
...
function IssueChatAssistantMessage({
  ...
  const waitingTextZh = typeof custom.waitingTextZh === "string" ? custom.waitingTextZh : waitingText;
  const currentStatusMessage =
    typeof custom.currentStatusMessage === "string" ? custom.currentStatusMessage.trim() : "";
  ...
  const chainOfThoughtLabelZh = typeof custom.chainOfThoughtLabelZh === "string" ? custom.chainOfThoughtLabelZh : chainOfThoughtLabel;
  const displayChainOfThoughtLabel = chainOfThoughtLabel
    ? localize({ en: chainOfThoughtLabel.toLowerCase(), zh: chainOfThoughtLabelZh ?? chainOfThoughtLabel })
    : "";
  const displayWaitingText = waitingText
    ? localize({ en: waitingText, zh: waitingTextZh })
    : "";
        ...
        <div className="min-w-0 flex-1">
          {isFoldable ? (
            ...
            >
              <span className="text-sm font-medium text-foreground">{authorName}</span>
              <SourceTrustBadge sourceTrust={sourceTrust} artifactLabel="comment" />
              <span className="text-xs text-muted-foreground/60">{displayChainOfThoughtLabel}</span>
          ...
          {deleted ? (
            <div className="rounded-sm bg-muted/40 px-3 py-2 text-sm italic text-muted-foreground">
              {localize({ en: "Comment deleted", zh: "评论已删除" })}
            </div>
          ) : !folded ? (
            <>
              <div className="space-y-3">
                <IssueChatAssistantParts message={message} hasCoT={hasCoT} />
                {message.content.length === 0 && displayWaitingText ? (
                  <div className="rounded-lg px-1 py-2">
                    <div className="flex min-w-0 items-center gap-2.5">
                      <span className="inline-flex items-center gap-2 text-sm font-medium text-foreground/80">
                        ...
                        <span className="shimmer-text">{displayWaitingText}</span>
                      </span>
                    </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose whether voted AI outputs can be shared for product diagnostics. This
  ...
  const rowContent = (
    <div className="min-w-0 flex-1">
      <div className={cn("flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs", isCurrentUser && "justify-end")}>
        ...
        <span className="text-muted-foreground">{localize({ en: "updated this task", zh: "更新了这个任务" })}</span>
        <a
          href={anchorId ? `#${anchorId}` : undefined}
        ...
        >
          ...
          {expanded
            ? localize({ en: "Hide confirmation", zh: "收起确认" })
            : localize({ en: "Expired confirmation", zh: "已过期确认" })}
        </button>
      </div>
      {expanded ? (
        ...
        <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">
            {custom.followUpRequested === true
              ? localize({ en: "requested follow-up", zh: "请求跟进" })
              : localize({ en: "updated this task", zh: "更新了这个任务" })}
          </span>
          <a
            href={anchorId ? `#${anchorId}` : undefined}
        ...
        {statusChange ? (
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70">
              {localize({ en: "Status", zh: "状态" })}
            </span>
            <span className="text-muted-foreground">{humanizeValue(statusChange.from, localize)}</span>
            <ArrowRight className="h-3 w-3 text-muted-foreground/70" />
            <span className="font-medium text-foreground">{humanizeValue(statusChange.to, localize)}</span>
          </div>
        ) : null}
    ...
    return (
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">{localize({ en: "run", zh: "运行" })}</span>
          <Link
  ...
  return (
    ...
    >
          ...
          {composerAttachments.map((attachment) => {
            ...
            const statusLabel =
              attachment.status === "uploading"
                ? localize({ en: "Uploading to task", zh: "正在上传到任务" })
                : attachment.status === "error"
                  ? attachment.error ?? localize({ en: "Upload failed", zh: "上传失败" })
                  : attachment.inline
                    ? localize({ en: "Inserted inline", zh: "已插入正文" })
                    : localize({ en: "Attached to task", zh: "已附加到任务" });
            return (
              <div
                key={attachment.id}
                className={cn(
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          {(onImageUpload || onAttachImage) ? (
            <>
              ...
              <Button
                ...
                disabled={attaching}
                title={localize({ en: "Attach file", zh: "附加文件" })}
              >
                <Paperclip className="h-4 w-4" />
              </Button>
            </>
            ...
            <Popover open={workModeMenuOpen} onOpenChange={setWorkModeMenuOpen}>
              ...
              >
                ...
                <div className="mt-1 border-t px-2 py-1.5 text-[10px] text-muted-foreground">
                  {localize({ en: "Cmd/Ctrl+. cycles modes", zh: "Cmd/Ctrl+. 切换模式" })}
                </div>
              </PopoverContent>
            </Popover>
          ) : null}
        ...
        {enableReassign && reassignOptions.length > 0 ? (
          <InlineEntitySelector
            ...
            noneLabel={localize({ en: "No assignee", zh: "未分配" })}
            ...
            renderTriggerValue={(option) => {
              if (!option) return <span className="text-muted-foreground">{localize({ en: "Assignee", zh: "负责人" })}</span>;
        ...
        <Button size="sm" disabled={!canSubmit} onClick={() => void handleSubmit()}>
          {submitting ? localize({ en: "Posting...", zh: "正在发布..." }) : localize({ en: "Send", zh: "发送" })}
        </Button>
      </div>
      ...
      <AlertDialog open={noAssigneeDialogOpen} onOpenChange={setNoAssigneeDialogOpen}>
        ...
        >
          <AlertDialogHeader>
            <AlertDialogTitle>{localize({ en: "No assignee selected", zh: "未选择负责人" })}</AlertDialogTitle>
            <AlertDialogDescription>
              {localize({
                en: "This comment will be posted without an assignee, so no agent will be woken to act on it. Go back to pick an assignee, or send anyway.",
                zh: "这条评论将不带负责人发布，因此不会唤醒任何 Agent 处理。返回选择负责人，或仍然发送。",
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {localize({ en: "Send anyway", zh: "仍然发送" })}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
