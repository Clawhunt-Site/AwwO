...
describe("SidebarCompanyMenu", () => {
  ...
  it("renders company switcher actions in Chinese when the board locale is Chinese", async () => {
    ...
    const trigger = container.querySelector('button[aria-label="打开 Acme Labs 公司切换器"]');
    expect(trigger).not.toBeNull();
    act(() => {
      trigger?.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, button: 0 }));
      trigger?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    ...
    expect(document.body.textContent).toContain("切换公司");
    expect(document.body.textContent).toContain("编辑");
    expect(document.body.textContent).toContain("创建新公司...");
