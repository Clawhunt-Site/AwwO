...
const severityCopy: Record<IssueBlockedInboxSeverity, LocalizedText> = {
...
export function BlockedReasonChip({
  ...
  const label = blockedVariantLabel(variant, language);
  ...
  return (
    <span
      ...
      aria-label={localize({
        en: `Reason: ${label}, severity ${severity}`,
        zh: `原因：${label}，严重程度：${severityCopy[severity] ? localize(severityCopy[severity]) : severity}`,
      })}
      className={cn(
