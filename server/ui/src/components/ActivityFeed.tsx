...
const FILTER_OPTIONS: Array<{ value: FilterValue; label: LocalizedText }> = [
  { value: "all", label: { en: "All", zh: "全部" } },
  { value: "in-progress", label: { en: "In Progress", zh: "进行中" } },
  { value: "for-review", label: { en: "In Review", zh: "审核中" } },
  { value: "completed", label: { en: "Done", zh: "已完成" } },
];
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? localize({ en: "System", zh: "系统" })
      : group.latestEvent.actorType === "user" ? localize({ en: "Board", zh: "看板" })
      : localize({ en: "Unknown", zh: "未知" }));
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-[#959596]">
            {localize({ en: `made ${group.events.length} updates to`, zh: `更新了 ${group.events.length} 次` })}
          </span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: localize({ en: "No agents set up yet. Add an agent to get started.", zh: "还没有设置 Agent。添加一个 Agent 开始使用。" }),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: localize({ en: "All agents are paused. Resume agents from the sidebar to see activity.", zh: "所有 Agent 都已暂停。请从侧边栏恢复 Agent 后查看活动。" }),
        showPulse: false,
      };
    }
    return {
      text: localize({ en: "Your agents are running - activity will appear here shortly.", zh: "Agent 正在运行，活动很快会显示在这里。" }),
      showPulse: true,
    };
  }, [agents, localize]);
        ...
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={localize({ en: "Group by task", zh: "按任务分组" })}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat"
                ? localize({ en: "Group by task", zh: "按任务分组" })
                : localize({ en: "Show flat", zh: "平铺显示" })}
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={localize({ en: "Filter by", zh: "筛选" })}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{localize({ en: "Filter by", zh: "筛选" })}</TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                {localize({ en: "Show all activity", zh: "显示全部活动" })}
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
