...
const goalLevelCopy: Record<string, LocalizedText> = {
  ...
  const inner = (
    <>
      ...
      <span className="text-xs text-muted-foreground capitalize">
        {localize(goalLevelCopy[goal.level] ?? { en: goal.level, zh: goal.level })}
      </span>
...
export function GoalTree({ goals, goalLink, onSelect }: GoalTreeProps) {
  ...
  if (goals.length === 0) {
    return <p className="text-sm text-muted-foreground">{localize({ en: "No goals.", zh: "没有目标。" })}</p>;
  }
