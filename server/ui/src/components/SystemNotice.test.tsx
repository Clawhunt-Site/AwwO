...
describe("SystemNotice", () => {
  it("renders the warning tone label and body in a single status container", () => {
    ...
    expect(status[0]?.getAttribute("aria-label")).toBe("Paperclip warning");
    expect(node.textContent).toContain(
      "Paperclip needs a disposition before this issue can continue.",
    );
  });

  it("uses Paperclip alert label for danger tone", () => {
    const node = render(
      <SystemNotice tone="danger" body="Recovery escalated to CTO." />,
    );

    const status = node.querySelector('[role="status"]');
    expect(status?.getAttribute("aria-label")).toBe("Paperclip alert");
  });

  it("uses neutral Paperclip notice label by default", () => {
    const node = render(
      <SystemNotice tone="neutral" body="Reassigned to ClaudeFixer." />,
    );

    const status = node.querySelector('[role="status"]');
    expect(status?.getAttribute("aria-label")).toBe("Paperclip notice");
  });
