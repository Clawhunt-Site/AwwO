...
const projectStatuses: Array<{ value: string; label: LocalizedText }> = [
...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(localize({ en: "Repo must use a valid GitHub or GitHub Enterprise repo URL.", zh: "仓库必须使用有效的 GitHub 或 GitHub Enterprise 仓库 URL。" }));
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{localize({ en: "New project", zh: "新建项目" })}</span>
          </div>
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{localize({ en: "Repo URL", zh: "仓库 URL" })}</label>
              <span className="text-xs text-muted-foreground/50">{localize({ en: "optional", zh: "可选" })}</span>
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {localize({
                    ...
                    zh: "关联 GitHub 仓库后，Agent 可以为该项目克隆、读取并推送代码。",
                  })}
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{localize({ en: "Local folder", zh: "本地文件夹" })}</label>
              <span className="text-xs text-muted-foreground/50">{localize({ en: "optional", zh: "可选" })}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {localize({
                    ...
                    zh: "设置本机绝对路径，供本地 Agent 为该项目读写文件。",
                  })}
                </TooltipContent>
              </Tooltip>
            </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
            ...
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {localize({ en: "All goals already selected.", zh: "所有目标都已选择。" })}
                </div>
              )}
            </PopoverContent>
          </Popover>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={localize({ en: "Target date", zh: "目标日期" })}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{localize({ en: "Failed to create project.", zh: "创建项目失败。" })}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending
              ...
              : localize({ en: "Create project", zh: "创建项目" })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
