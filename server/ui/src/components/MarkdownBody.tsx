...
function safeMarkdownUrlTransform(url: string): string {
  ...
  const transformed = parseMentionChipHref(url) ? url : defaultUrlTransform(url);
  // Central rewrite point for EVERY markdown link href and image src: a server-emitted
  // "/api/..." content URL (e.g. an inline attachment image, a `/api/attachments/.../content`
  // link) must target the Paperclip control plane when the board is embedded. No-op
  // standalone and for non-"/api" URLs (workspace/mention hrefs, external links).
  return resolveContentUrl(transformed);
}
  ...
  const components = useMemo<Components>(() => {
    ...
    if (resolveImageSrc || onImageClick) {
      map.img = ({ node: _node, src, alt, ...imgProps }) => {
        ...
        // The canonical src (post resolveImageSrc; still a raw "/api/..." when
        // server-emitted) is the IDENTITY handed to onImageClick — callers match it
        // against attachment contentPaths, which are raw "/api/...". Only the rendered
        // <img src> is made embedding-aware; rewriting the click identity would break
        // that gallery matching. resolveImageSrc can also mint a fresh "/api/..." that
        // bypasses urlTransform, so resolveContentUrl on the display src covers it.
        const clickSrc = resolved ?? src;
        const displaySrc = resolveContentUrl(clickSrc);
        return (
          <img
            {...imgProps}
            src={displaySrc}
