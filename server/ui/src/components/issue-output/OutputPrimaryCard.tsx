  ...
  return (
    <div className="overflow-hidden rounded-md border border-border bg-card">
      ...
      {meta && isVideoContentType(contentType) ? (
        <OutputVideoPlayer src={resolveContentUrl(meta.contentPath)} title={filename} />
      ) : meta && isImageContentType(contentType) ? (
        <a
          href={resolveContentUrl(meta.openPath)}
          target="_blank"
          rel="noreferrer"
          className="block aspect-video w-full overflow-hidden bg-black"
          aria-label={`Open ${filename}`}
        >
          <img src={resolveContentUrl(meta.contentPath)} alt={filename} className="h-full w-full object-contain" />
        </a>
      ) : (
        ...
        {meta ? (
          <div className={cn("flex shrink-0 items-center gap-2", "max-md:w-full")}>
            <Button asChild variant="outline" size="sm" className="max-md:flex-1">
              <a href={resolveContentUrl(meta.openPath)} target="_blank" rel="noreferrer">
                <ExternalLink className="h-4 w-4" />
                Open
              </a>
            </Button>
            <Button asChild size="sm" className="max-md:flex-1">
              <a href={resolveContentUrl(meta.downloadPath)} aria-label={`Download ${filename}`}>
