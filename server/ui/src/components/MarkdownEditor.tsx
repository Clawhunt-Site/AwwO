...
import { Boxes, CalendarClock, Hash, Plug, User } from "lucide-react";
...
export function findMentionMatch(
  ...
  if (
    trigger === "skill" &&
    /\s/.test(query) &&
    !query.toLowerCase().startsWith("routine:") &&
    !query.toLowerCase().startsWith("plugin:")
  ) {
    return null;
  }
...
function slashCommandLabel(option: SlashCommandOption): string {
  ...
  if (option.kind === "plugin") return `/plugin:${option.name}`;
  return `/${option.slug}`;
}
...
function isSlashCommandOption(option: AutocompleteOption): option is SlashCommandOption {
  return option.kind === "skill" || option.kind === "routine" || option.kind === "plugin";
}

function autocompleteMarkdown(option: AutocompleteOption): string {
  return isSlashCommandOption(option) ? slashCommandMarkdown(option) : mentionMarkdown(option);
}
...
function autocompleteOptionMatchesLink(option: AutocompleteOption, href: string): boolean {
  ...
  if (option.kind === "plugin") {
    return parsed.kind === "plugin" && parsed.pluginId === option.pluginId;
  }
...
export const MarkdownEditor = forwardRef<MarkdownEditorRef, MarkdownEditorProps>(function MarkdownEditor({
    ...
    if (imageHandler) {
      all.push(
        imagePlugin({
          ...
          // The rich editor renders image PREVIEWS through this handler. A stored
          // "/api/..." src (existing markdown, or a just-uploaded asset whose upload
          // handler returns a raw "/api/..." path) is made embedding-aware so the
          // preview loads from the Paperclip control plane when the board is embedded.
          // Display-only: the PERSISTED markdown keeps the raw "/api/..." src (the
          // node source is untouched), so nothing is rewritten on save. No-op standalone.
          imagePreviewHandler: (src) => Promise.resolve(resolveContentUrl(src)),
        }),
      );
    }
    return all;
  ...
  const decorateProjectMentions = useCallback(() => {
    ...
    for (const node of links) {
      ...
      if (parsed.kind === "user" || parsed.kind === "issue" || parsed.kind === "plugin") {
        applyMentionChipDecoration(link, parsed);
        continue;
      }
              ...
              >
                ...
                {option.kind === "issue" && option.issueIdentifier ? (
                  ...
                  <span className="truncate">
                    {isSlashCommandOption(option) ? slashCommandLabel(option) : option.name}
                  </span>
                )}
                {option.kind === "issue" && (
