...
function SidebarSectionHeader({
  ...
  const actionClassName = cn(
    "h-5 w-5 shrink-0 text-muted-foreground/60 transition-opacity hover:bg-muted/60 hover:text-foreground data-[state=open]:opacity-100 dark:hover:bg-muted/35",
    headerControlVisibilityClassName,
  );
      ...
      <DropdownMenuTrigger asChild>
        <button
          ...
          className={cn(
            ...
            "hover:bg-muted/60 focus-visible:bg-muted/60 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 dark:hover:bg-muted/35 dark:focus-visible:bg-muted/35",
            menuOpen && "bg-muted/60 dark:bg-muted/35",
          )}
          aria-label={menu?.ariaLabel ?? localize({ en: `${label} actions`, zh: `${label}操作` })}
        >
          {headerContent}
        </button>
      </DropdownMenuTrigger>
        ...
        {collapsible ? (
          <CollapsibleTrigger asChild>
            <button
              type="button"
              data-slot="icon-button"
              className="absolute -left-4 flex h-5 w-5 items-center justify-center rounded-sm outline-none transition-colors hover:bg-muted/60 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 dark:hover:bg-muted/35"
              aria-label={collapsible.open
                ? localize({ en: `Collapse ${label}`, zh: `收起 ${label}` })
                : localize({ en: `Expand ${label}`, zh: `展开 ${label}` })}
            >
