...
export function SidebarNavItem({
  ...
  const railAriaLabel = !rail
    ? undefined
    : hasLive
      ? `${label}, ${liveCount} ${localize({ en: "live", zh: "运行中" })}`
      : hasBadge
        ? `${label}, ${badge}${badgeLabel ? ` ${badgeLabel}` : ""}`
        : alert
          ? `${label}, ${localize({ en: "attention needed", zh: "需要注意" })}`
          : undefined;
  ...
  const link = (
    ...
    >
      <span className="relative shrink-0">
        ...
        {rail && !alert && hasLive && (
          <span
            className="absolute -right-0.5 -top-0.5 inline-flex h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_0_2px_hsl(var(--background))]"
            aria-hidden="true"
          />
        )}
      ...
      {!rail && hasLive && (
        <span className="ml-auto flex items-center gap-1.5">
          ...
          <span className="text-[11px] font-medium text-emerald-700 dark:text-emerald-300">
            {liveCount} {localize({ en: "live", zh: "运行中" })}
          </span>
        </span>
      )}
      {!rail && hasBadge && (
