...
describe("Sidebar", () => {
  ...
  async function renderSidebar({ embedded = false }: { embedded?: boolean } = {}) {
    ...
    flushSync(() => {
      root.render(
        <QueryClientProvider client={queryClient}>
          {embedded ? (
            <EmbeddedHostProvider value={{ onCreateCompany: vi.fn() }}>
              {sidebar}
  ...
  it("hides the duplicate company switcher in an embedded expanded sidebar", async () => {
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: false });
    const root = await renderSidebar({ embedded: true });

    expect(container.textContent).not.toContain("Company menu");
    expect(container.textContent).toContain("New Task");
    expect(container.querySelector('a[aria-label="Open search"]')).not.toBeNull();
  ...
  it("renders the Paperclip shell navigation in Chinese when the board locale is Chinese", async () => {
