...
const PROJECT_STATUSES: Array<{ value: string; label: LocalizedText }> = [
  { value: "backlog", label: { en: "Backlog", zh: "待规划" } },
  { value: "planned", label: { en: "Planned", zh: "已计划" } },
  { value: "in_progress", label: { en: "In Progress", zh: "进行中" } },
...
function ProjectStatusPicker({ status, onChange }: { status: string; onChange: (status: string) => void }) {
  ...
  const currentStatusLabel =
    PROJECT_STATUSES.find((projectStatus) => projectStatus.value === status)?.label;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {currentStatusLabel ? localize(currentStatusLabel) : status.replace("_", " ")}
        </button>
      </PopoverTrigger>
      ...
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          ...
          {isArchive
            ...
            : localize({ en: "Unarchiving...", zh: "正在取消归档..." })}
        </Button>
      ) : confirming ? (
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{localize({ en: "Archive project", zh: "归档项目" })}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{localize({ en: "Unarchive project", zh: "取消归档项目" })}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(localize({ en: "Select a company to create secrets", zh: "请选择公司后再创建密钥" }));
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(localize({ en: "Local folder must be a full absolute path.", zh: "本地文件夹必须是完整的绝对路径。" }));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(localize({ en: "Repo must use a valid GitHub or GitHub Enterprise repo URL.", zh: "代码仓库必须使用有效的 GitHub 或 GitHub Enterprise 仓库 URL。" }));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? localize({ en: "Clear local folder from this workspace?", zh: "要清除此工作区的本地文件夹吗？" })
        : localize({ en: "Delete this workspace local folder?", zh: "要删除此工作区本地文件夹吗？" }),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? localize({ en: "Clear repo from this workspace?", zh: "要清除此工作区的仓库吗？" })
        : localize({ en: "Delete this workspace repo?", zh: "要删除此工作区仓库吗？" }),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={localize({ en: "Name", zh: "名称" })} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={localize({ en: "Project name", zh: "项目名称" })}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={localize({ en: "Description", zh: "描述" })} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            ...
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || localize({ en: "No description", zh: "暂无描述" })}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={localize({ en: "Status", zh: "状态" })} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={localize({ en: "Lead", zh: "负责人" })}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={localize({ en: "Goals", zh: "目标" })} state={fieldState("goals")} />}
          alignStart
        ...
        <PropertyRow
          label={<FieldLabel label={localize({ en: "Env", zh: "环境变量" })} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {localize({
                en: "Applied to all runs for tasks in this project. Project values override agent env on key conflicts.",
                zh: "应用到此项目中所有任务运行。键名冲突时，项目值会覆盖 Agent 环境变量。",
              })}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={localize({ en: "Created", zh: "创建时间" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={localize({ en: "Updated", zh: "更新时间" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={localize({ en: "Target Date", zh: "目标日期" })} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{localize({ en: "Codebase", zh: "代码库" })}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={localize({ en: "Codebase help", zh: "代码库帮助" })}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {localize({
                  ...
                  zh: "仓库用于标识代码来源。本地文件夹是 Agent 默认写入代码的位置。",
                })}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{localize({ en: "Repo", zh: "仓库" })}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {localize({ en: "Change repo", zh: "更改仓库" })}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={localize({ en: "Clear repo", zh: "清除仓库" })}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{localize({ en: "Not set.", zh: "未设置。" })}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{localize({ en: "Local folder", zh: "本地文件夹" })}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{localize({ en: "Workspace-managed folder.", zh: "工作区托管文件夹。" })}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder
                      ? localize({ en: "Change local folder", zh: "更改本地文件夹" })
                      : localize({ en: "Set local folder", zh: "设置本地文件夹" })}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      variant="ghost"
                      ...
                      aria-label={localize({ en: "Clear local folder", zh: "清除本地文件夹" })}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {localize({
                  en: "Additional legacy workspace records exist on this project. The primary workspace is used as the codebase view.",
                  zh: "此项目还有额外的旧工作区记录。代码库视图会使用主工作区。",
                })}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? localize({ en: "No URL", zh: "无 URL" })
                        )}
                      </div>
                    </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{localize({ en: "Failed to save workspace.", zh: "保存工作区失败。" })}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{localize({ en: "Failed to delete workspace.", zh: "删除工作区失败。" })}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{localize({ en: "Failed to update workspace.", zh: "更新工作区失败。" })}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{localize({ en: "Execution Workspaces", zh: "执行工作区" })}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={localize({ en: "Execution workspaces help", zh: "执行工作区帮助" })}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {localize({
                      en: "Project-owned defaults for isolated task checkouts and execution workspace behavior.",
                      zh: "项目级默认值，用于隔离任务检出和执行工作区行为。",
                    })}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{localize({ en: "Enable isolated task checkouts", zh: "启用隔离任务检出" })}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {localize({
                        en: "Let tasks choose between the project's primary checkout and an isolated execution workspace.",
                        zh: "允许任务在项目主检出和隔离执行工作区之间选择。",
                      })}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled
                        ? localize({ en: "Enabled", zh: "已启用" })
                        : localize({ en: "Disabled", zh: "已停用" })}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{localize({ en: "New tasks default to isolated checkout", zh: "新任务默认使用隔离检出" })}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {localize({
                            en: "If disabled, new tasks stay on the project's primary checkout unless someone opts in.",
                            zh: "如果停用，新任务会留在项目主检出，除非手动选择隔离检出。",
                          })}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? localize({ en: "Hide advanced checkout settings", zh: "隐藏高级检出设置" })
                          : localize({ en: "Show advanced checkout settings", zh: "显示高级检出设置" })}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {localize({ en: "Host-managed implementation:", zh: "宿主管理实现：" })} <span className="text-foreground">Git worktree</span>
                        </div>
                        {showExecutionWorkspaceEnvironmentControl ? (
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{localize({ en: "Environment", zh: "环境" })}</span>
                            ...
                            >
                              <option value="">{localize({ en: "No environment", zh: "无环境" })}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{localize({ en: "Base ref", zh: "基础引用" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{localize({ en: "Branch template", zh: "分支模板" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{localize({ en: "Worktree parent dir", zh: "Worktree 父目录" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{localize({ en: "Provision command", zh: "准备命令" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{localize({ en: "Teardown command", zh: "清理命令" })}</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {localize({
                            en: "Provision runs inside the derived worktree before agent execution. Teardown is stored here for future cleanup flows.",
                            zh: "准备命令会在派生 worktree 内、Agent 执行前运行。清理命令会存储在这里，供后续清理流程使用。",
                          })}
                        </p>
                      </div>
                    ) : null}
