...
const FILTERS: Array<{ label: LocalizedText; value: FilterValue }> = [
  { label: { en: "All", zh: "全部" }, value: "all" },
  { label: { en: "In Progress", zh: "进行中" }, value: "in_progress" },
  { label: { en: "For Review", zh: "待审核" }, value: "for_review" },
  { label: { en: "Completed", zh: "已完成" }, value: "completed" },
];

const workProductTypeCopy: Record<string, LocalizedText> = {
  artifact: { en: "artifact", zh: "产物" },
  branch: { en: "branch", zh: "分支" },
  commit: { en: "commit", zh: "提交" },
  document: { en: "document", zh: "文档" },
  preview_url: { en: "preview url", zh: "预览链接" },
  pull_request: { en: "pull request", zh: "拉取请求" },
  runtime_service: { en: "runtime service", zh: "运行服务" },
};
...
function statusBadge(status: string): { label: LocalizedText; className: string } {
  switch (status) {
    case "active":
    case "draft":
      return { label: { en: "In Progress", zh: "进行中" }, className: "bg-blue-500/10 text-blue-600 dark:text-blue-400" };
    case "ready_for_review":
      return { label: { en: "For Review", zh: "待审核" }, className: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
    case "approved":
    case "merged":
      return { label: { en: "Completed", zh: "已完成" }, className: "bg-green-500/10 text-green-600 dark:text-green-400" };
    case "changes_requested":
      return { label: { en: "Changes Requested", zh: "需要修改" }, className: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
    case "failed":
      return { label: { en: "Failed", zh: "失败" }, className: "bg-red-500/10 text-red-600 dark:text-red-400" };
    default:
      return { label: { en: status, zh: status }, className: "bg-muted text-muted-foreground" };
  }
}
...
export function ArtifactsPanel({ taskId, isAgentWorking, openDocKey, openDocTitle, onClearOpenDoc, onApprove, onReject }: ArtifactsPanelProps) {
  ...
  const effectiveViewingDoc = openDocKey
    ? { key: openDocKey, title: openDocTitle ?? localize({ en: "Document", zh: "文档" }) }
    : viewingDoc;
  ...
  return (
    <div className="flex flex-col h-full" data-artifacts-panel>
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">{localize({ en: "Artifacts", zh: "产物" })}</h3>
      </div>
          ...
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {workProducts?.length === 0
                ? localize({ en: "Your team's deliverables and plans will appear here as they're produced.", zh: "团队生成的交付物和计划会显示在这里。" })
                : localize({ en: "No artifacts match this filter.", zh: "没有产物匹配此筛选。" })}
            </p>
          </div>
        ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[10px] text-muted-foreground capitalize">
                          {localize(workProductTypeCopy[wp.type] ?? { en: wp.type.replace(/_/g, " "), zh: wp.type.replace(/_/g, " ") })}
                        </span>
                        {showGenerating ? (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            {localize({ en: "Loading document...", zh: "正在加载文档..." })}
          </div>
        ) : error ? (
          <p className="text-sm text-muted-foreground">
            {localize({ en: "Document not available yet.", zh: "文档暂不可用。" })}
          </p>
        ) : doc?.body ? (
          ...
          <p className="text-sm text-muted-foreground">
            {localize({ en: "Document is empty.", zh: "文档为空。" })}
          </p>
        )}
      </div>
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-[11px] text-muted-foreground mb-2">
            {localize({ en: "This document needs your review.", zh: "此文档需要你审核。" })}
          </p>
        ...
        <div className="border-t border-green-500/30 bg-green-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[13px] font-medium text-green-700 dark:text-green-400">
              {localize({ en: "Approved - hire tasks created", zh: "已批准 - 招募任务已创建" })}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="border-t border-orange-500/30 bg-orange-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[13px] font-medium text-orange-700 dark:text-orange-400">
              {localize({ en: "Changes requested - CEO is revising", zh: "已请求修改 - CEO 正在修订" })}
            </p>
          </div>
        </div>
      )}
