...
export function ArtifactFileChip({
  ...
  const lineSuffix = workspaceFileRef.line
    ? localize({
        en: ` line ${workspaceFileRef.line}${workspaceFileRef.column ? ` column ${workspaceFileRef.column}` : ""}`,
        zh: ` 第 ${workspaceFileRef.line} 行${workspaceFileRef.column ? ` 第 ${workspaceFileRef.column} 列` : ""}`,
      })
    : "";
  const ariaLabel = canOpen
    ? localize({
        en: `Open ${workspaceFileRef.displayPath}${lineSuffix} in the file viewer`,
        zh: `在文件查看器中打开 ${workspaceFileRef.displayPath}${lineSuffix}`,
      })
    : localize({
        en: `Workspace file ${workspaceFileRef.displayPath}${lineSuffix}`,
        zh: `工作区文件 ${workspaceFileRef.displayPath}${lineSuffix}`,
      });
