...
function isOverlayDirty(o: AgentConfigOverlay): boolean {
  return (
    ...
    Object.keys(o.pluginTools).length > 0 ||
...
// claude_local's `--effort` domain (low/medium/high/xhigh/max), per Anthropic's
// output_config.effort tiers — keep in lockstep with adapter-effort.ts and the
// Python ClaudeCliBackend. (grok_local is NOT effort-capable, so it's excluded
// via showThinkingEffort below — it never reaches this table.)
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  type RecordOverlayGroup = "identity" | "adapterConfig" | "heartbeat" | "pluginTools" | "runtime";
  ...
  const pluginTools = !isCreate ? ((runtimeConfig.pluginTools ?? {}) as Record<string, unknown>) : {};
  ...
  // grok_local has no real reasoning-effort axis (no Grok model honors it — see
  // adapter-effort.ts / the Python GrokCliBackend), so don't render the picker.
  const showThinkingEffort =
    adapterType !== "gemini_local" && adapterType !== "cursor_cloud" && adapterType !== "grok_local";
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            <div className={cn(cards ? "p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              <ToggleField
                label="Plugin tools"
                hint="Always expose this company's installed plugin tools to the agent via the governed MCP bridge (local execution only). A run can also opt in for one run with a plugin:// mention."
                checked={eff("pluginTools", "enabled", pluginTools.enabled === true)}
                onChange={(v) => mark("pluginTools", "enabled", v)}
              />
            </div>
            <CollapsibleSection
