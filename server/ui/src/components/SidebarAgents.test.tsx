...
describe("SidebarAgents", () => {
  ...
  it("renders section actions in Chinese when the board locale is Chinese", async () => {
    await i18n.changeLanguage("zh-CN");
    await renderSidebarAgents();

    expect(container.textContent).toContain("Agent");
    const sectionMenuTrigger = container.querySelector('button[aria-label="Agent 分区操作"]');
    expect(sectionMenuTrigger).toBeTruthy();
    const newAgentButton = container.querySelector('button[aria-label="新建 agent"]');
    expect(newAgentButton).toBeTruthy();

    await act(async () => {
      sectionMenuTrigger?.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, button: 0 }));
      sectionMenuTrigger?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
    await flushReact();

    expect(document.body.textContent).toContain("浏览 agent");
    expect(document.body.textContent).toContain("置顶");
    expect(document.body.textContent).not.toContain("Browse agents");
  });
