...
describe("SidebarSection", () => {
  ...
  it("does not apply hover background classes to static section labels", async () => {
    ...
    expect(staticLabelControl?.getAttribute("class")).not.toContain("hover:bg-muted/60");
    expect(staticLabelControl?.getAttribute("class")).not.toContain("focus-visible:bg-muted/60");
  });
  ...
  it("keeps the header action outside the label menu hit area", async () => {
    ...
    expect(sectionMenuTrigger?.getAttribute("class")).toContain("hover:bg-muted/60");
