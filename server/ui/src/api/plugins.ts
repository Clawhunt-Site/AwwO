...
/** Trust class that admitted a global-store package's signature. */
export type GlobalPluginSignerClass = "root" | "local_dev" | "none";

/**
 * A verified plugin from SuperClaw's native global store (`~/.superclaw/plugins`),
 * returned by `GET /api/plugins/global`. Mirrors the server's `GlobalPluginEntry`
 * (`server/src/services/global-runtime-plugins.ts`). This is a READ-ONLY, signature-
 * verified inventory projection — it does not install or execute; acquiring/enabling
 * happens through SuperClaw's Capability Workshop / plugin gateway.
 */
export interface GlobalPluginEntry {
  /** Stable, namespaced key (never collides with a DB plugin row id). */
  key: string;
  /** The manifest plugin id (e.g. `acme.formatter`); matches a DB `pluginKey`. */
  pluginId: string;
  version: string;
  ...
  digest: string;
  signerClass: GlobalPluginSignerClass;
  /** Absolute on-disk package directory. */
  source: string;
  origin: "superclaw-global";
  /** Whether this plugin id is ALSO installed in the company's DB registry. */
...
export const pluginsApi = {
  ...
  /**
   * List the verified plugins in SuperClaw's native global store
   * (`~/.superclaw/plugins`) — the read-only, signature-verified local library.
   */
  listGlobal: () =>
    api.get<GlobalPluginEntry[]>("/plugins/global"),
