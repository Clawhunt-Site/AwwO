...
const DATA_DIR_OPTION_HELP =
  "SuperClaw data directory root (isolates state from ~/.paperclip)";

program
  .name("paperclipai")
  .description("SuperClaw CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program
  ...
  .option("--run", "Start SuperClaw immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your SuperClaw setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
const run = program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run SuperClaw")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the SuperClaw server API")
