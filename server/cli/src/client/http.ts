...
export class PaperclipApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-superclaw-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the SuperClaw API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the SuperClaw server is not running, the configured URL is wrong, or the request is being blocked before it reaches SuperClaw.",
    "",
    "Try:",
    "- Start SuperClaw with `pnpm dev` or `pnpm paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If SuperClaw is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
