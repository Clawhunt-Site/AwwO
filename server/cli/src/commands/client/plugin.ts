...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; SuperClaw watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}
...
export function formatTargetDiagnostics(diag: TargetDiagnostics): string {
  const lines = [pc.dim(`Target SuperClaw: ${pc.cyan(diag.apiBase)}`)];
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local SuperClaw plugin project")
  ...
  addCommonClientOptions(
    plugin
      ...
      .option(
        "--no-verify-target",
        "Skip the pre-install probe that reports which SuperClaw instance the plugin installs into",
      )
  ...
  addCommonClientOptions(
    plugin
      .command("target")
      .description(
        "Show which SuperClaw instance plugin commands will talk to.\n" +
