...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to SuperClaw config file")
    .option("-d, --data-dir <path>", "SuperClaw data directory root (isolates state from ~/.paperclip)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the SuperClaw API")
...
export function resolveCommandContext(
  ...
  const runId = options.runId?.trim() || (process.env.SUPERCLAW_RUN_ID?.trim() || process.env.PAPERCLIP_RUN_ID?.trim()) || undefined;
...
export function resolveApiBase(options: Pick<BaseClientOptions, "apiBase" | "config">, profile: ClientContextProfile = {}): string {
  return normalizeApiBase(
    ...
    (process.env.SUPERCLAW_API_URL?.trim() || process.env.PAPERCLIP_API_URL?.trim()) ||
    profile.apiBase ||
...
function resolveApiKey(
  ...
  const envValue = (process.env.SUPERCLAW_API_KEY?.trim() || process.env.PAPERCLIP_API_KEY?.trim());
