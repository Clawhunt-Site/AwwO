...
function listAvailableSkills(): AvailableSkill[] {
  ...
  if (paperclipSkillsDir) {
    ...
    } catch { /* skip SuperClaw skills directory */ }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your SuperClaw host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "SuperClaw is bound to loopback in authenticated/private mode.",
  ...
  return {
    ...
    onboarding: {
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the SuperClaw hostname is reachable and allowed via `pnpm paperclipai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this SuperClaw API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific SuperClaw skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which SuperClaw adapter type matches your runtime.

    Use adapterType only when there is a matching SuperClaw adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    OpenClaw Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "paperclipApiUrl": "https://superclaw-hostname-your-agent-can-reach:3100",
    ...
    - Set agentDefaultsPayload.apiBaseUrl to the Hermes gateway URL SuperClaw can reach.
    - Set agentDefaultsPayload.paperclipApiUrl to the SuperClaw base URL Hermes can reach.
    - Use hermes_local when SuperClaw should start Hermes on the SuperClaw host.
    - Use hermes_gateway when SuperClaw should call an already-running Hermes API server.
    - Hermes-originated SuperClaw API usage means Hermes calls SuperClaw with PAPERCLIP_API_URL and PAPERCLIP_API_KEY after approval/key claim. Do not confuse that with agentDefaultsPayload.apiBaseUrl, which points SuperClaw to Hermes.
    ...
    - Private overlay: use overlay DNS names, for example agentDefaultsPayload.apiBaseUrl = "http://hermes-host.tailnet-name.ts.net:8642" and agentDefaultsPayload.paperclipApiUrl = "http://superclaw-host.tailnet-name.ts.net:3100".
    - Docker: if SuperClaw runs in Docker and Hermes runs on the host, use agentDefaultsPayload.apiBaseUrl = "http://host.docker.internal:8642"; if both run in Compose, use the Hermes service name.
    - Reverse proxy/TLS: use HTTPS origins, for example agentDefaultsPayload.apiBaseUrl = "https://hermes-gateway.example" and agentDefaultsPayload.paperclipApiUrl = "https://superclaw.example".
    ...
    The board approves the join request in SuperClaw before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set PAPERCLIP_API_KEY and PAPERCLIP_API_URL for future SuperClaw API calls.
    ...
    ## Step 4: Install SuperClaw skill
    ...
    ${
      ...
      "Ensure SuperClaw is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested SuperClaw base URLs to try");
    ...
    appendBlock(`
      ...
      - then restart SuperClaw and retry onboarding.
    `);
  }
