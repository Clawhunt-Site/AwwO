...
import { unionGlobalRuntimePluginEntries } from "../services/global-runtime-plugins.js";
...
import { ToolParameterValidationError } from "../services/plugin-tool-schema.js";
import { resolveRunUniqueOwnedProjectId, projectBelongsToCompany } from "../services/plugin-run-project.js";
...
import { clearPluginProvenanceOnUninstall } from "../services/super-plugin-installer.js";
import {
    ...
    bundledPlugins.push({
      entry: {
        ...
        description: metadata.description
          ?? `Bundled SuperClaw plugin from ${path.relative(REPO_ROOT, packageRoot)}.`,
...
export function pluginRoutes(
  ...
  async function validateToolRunContextScope(runContext: ToolRunContext): Promise<string | null> {
    ...
    // Bind projectId to the RUN, not just the company. A JWT carries no
    // project_id, so a company-only check would let an agent target ANY project
    // in its company. Derive the run's project from the issue that OWNS the run
    // — i.e. the issue whose checkout/execution run IS this run. These links are
    // written host-side during claim/execution. NOTE: the link's *generation*
    // side can still be influenced by a self-wakeup's contextSnapshot.issueId —
    // that pre-existing heartbeat/claim hardening is tracked in design §7.2 as a
    // must-close gate before the bridge is wired (slice 3b). Fail closed when no
    // owning project resolves — never fall back to a company-only scope here.
    // Derive the run's owned project via the shared single-source resolver
    // (issue-owns-run links → exactly one distinct project), then require the
    // supplied projectId to equal it, then verify it belongs to the company.
    // This ordering (mismatch before the company query) matches the original
    // inline check exactly. The same resolver feeds the per-run plugin MCP
    // bridge context so both bind projectId identically. Fail closed throughout.
    const owned = await resolveRunUniqueOwnedProjectId(db, runContext.companyId, runContext.runId);
    if (owned.kind === "none") {
      return '"runContext.runId" has no derivable project; cannot bind "runContext.projectId"';
    }
    if (owned.kind === "ambiguous") {
      return '"runContext.runId" maps to multiple projects; "runContext.projectId" cannot be bound unambiguously';
    }
    if (owned.projectId !== runContext.projectId) {
      return '"runContext.projectId" does not belong to "runContext.runId"';
    }
    if (!(await projectBelongsToCompany(db, owned.projectId, runContext.companyId))) {
  ...
  /**
   * GET /api/plugins/global
   *
   * Read-only inventory of the VERIFIED plugins in SuperClaw's native global plugin
   * store (`~/.superclaw/plugins`), the plugin analog of the global skill union. Each
   * package is integrity- and signature-verified by the trust primitives, fail-closed
   * (an unverifiable/revoked/skill-origin package is dropped; a corrupt store surfaces
   * none). Entries are annotated with `alreadyInstalled` (whether the same plugin id is
   * also in the DB registry). This is a VISIBILITY/governance surface only — it does not
   * install or execute; that stays with the plugin install/gateway flow.
   *
   * Response: `GlobalPluginEntry[]`
   */
  router.get("/plugins/global", async (req, res) => {
    assertBoardOrgAccess(req);
    const installed = await registry.listInstalled();
    const installedIds = new Set(installed.map((plugin) => plugin.pluginKey));
    res.json(await unionGlobalRuntimePluginEntries(installedIds));
  });
  ...
  router.post("/plugins/tools/execute", async (req, res) => {
    ...
    // An agent caller on this run-bound execution path must authenticate with
    // the signed, run-bound agent JWT. The agent_key (API-key) actor takes its
    // runId from a caller-controllable header, so it cannot serve as a trusted
    // run binding for plugin tool calls — reject it (board/operator unaffected).
    if (req.actor.type === "agent" && req.actor.source !== "agent_jwt") {
      res.status(403).json({ error: "plugin tool execution requires a run-bound agent JWT" });
      return;
    }
    ...
    // Bind the runContext to the authenticated caller: an agent may only execute
    // under its OWN identity (blocker 1, agent-identity half). Self-consistency
    // above is necessary but not sufficient — without this an agent could pass a
    // foreign-but-consistent runContext and act as another agent in the company.
    const actorMismatch = runContextActorMismatch(req.actor, runContext);
    if (actorMismatch) {
      res.status(403).json({ error: actorMismatch });
      return;
    }

    // Existence is resolved by the dispatcher/runtime-router across BOTH stores —
    // super plugin tools are not in the in-memory tool registry, so a registry
    // pre-gate here would 404 them before they ever reach the governed router.
    try {
      ...
      if (err instanceof ToolParameterValidationError) {
        // Contract violation (bad params / unusable schema) → 400, not a 500.
        res.status(400).json({ error: message });
      } else if (message.includes("not running")) {
        // Worker process down → 502.
        res.status(502).json({ error: message });
      } else if (
        message.includes("no installed plugin") ||
        message.includes("does not declare tool") ||
        message.includes("is not registered") ||
        message.includes("invalid namespaced tool name")
      ) {
        // Unknown / undeclared tool (replaces the removed registry pre-gate) → 404.
        // NOTE: match only these precise dispatcher/router messages — a broad
        // "not found" substring would misclassify execution-env failures like an
        // external_mcp "launcher not found on PATH" (a 500, not a missing tool).
  ...
  router.delete("/plugins/:pluginId", async (req, res) => {
    ...
    try {
      // Clear workshop provenance BEFORE the native unload. Provenance is keyed by the
      // native plugin key, so a stale OFFICIAL row is the dangerous residue (a future
      // plugin REUSING this key would inherit it — the unified catalog joins the badge
      // by key, with no digest-drift guard on the JS path). Clearing first means a
      // clear-failure aborts the delete (the still-installed plugin temporarily loses
      // official — recoverable) rather than leaving an orphan-official after a
      // successful uninstall. Idempotent + no-op for non-workshop plugins.
      await clearPluginProvenanceOnUninstall(db, plugin.pluginKey);
