...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    // already access-controlled field. Resolve it ONLY when it will be exposed, so an
    // anonymous health check never forces the (memoized) git snapshot to compute.
    const serverInfo = exposeFullDetails ? (opts.serverInfo ?? getServerInfoSnapshot()) : undefined;
