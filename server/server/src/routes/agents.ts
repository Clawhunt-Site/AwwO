...
export function agentRoutes(
  ...
  // An agent self-wake is a generic on-demand ping, NOT an issue-scoped
  // authority grant. Caller-supplied scope (a wake reason, or a privilege-bearing
  // payload field) from an agent must be refused — otherwise a self-wake could
  // shape its own run's issue/project binding (design §7.2 trust-chain gate),
  // which the claim-side verified-comment check then trusts. Board/user/system
  // callers are unaffected (they legitimately drive scoped wakes).
  const agentSelfWakeCarriesScope = (req: Request): boolean => {
    if (req.actor.type !== "agent") return false;
    const body = (req.body ?? {}) as { reason?: unknown; payload?: unknown; source?: unknown };
    const hasReason =
      body.reason != null && String(body.reason).trim().length > 0;
    // A self-ping is on-demand only; timer/assignment/automation sources select
    // different heartbeat scheduling policy and are not the agent's to choose.
    const hasNonOnDemandSource =
      body.source != null && String(body.source).trim().length > 0 && body.source !== "on_demand";
    const payload = body.payload;
    const hasScopedPayload =
      payload != null &&
      typeof payload === "object" &&
      !Array.isArray(payload) &&
      // resumeFromRunId is privilege-bearing too: heartbeat copies the
      // referenced run's issueId/taskId into the new run context, which an agent
      // could use to indirectly shape its run's issue/project binding.
      ["issueId", "taskId", "commentId", "projectId", "executionStage", "resumeFromRunId"].some(
        (k) => (payload as Record<string, unknown>)[k] != null,
      );
    return hasReason || hasScopedPayload || hasNonOnDemandSource;
  };
  ...
  const handleWakeupRoute = async (
    ...
    if (req.actor.type === "agent") {
      ...
      if (agentSelfWakeCarriesScope(req)) {
        res.status(403).json({ error: "agent self-wake cannot carry issue/comment/project scope" });
        return;
      }
    } else {
      await assertBoardCanManageAgentsForCompany(req, agent.companyId);
