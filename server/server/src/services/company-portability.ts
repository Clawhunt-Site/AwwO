...
import {
  isVendoredEngineRepo,
  isVendoredEngineSkillKey,
  VENDORED_ENGINE_OWNER,
  VENDORED_ENGINE_REPO,
  VENDORED_ENGINE_SKILL_KEY_PREFIX,
} from "@paperclipai/shared/skill-key-namespaces";
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; SuperClaw will import the routine trigger without those limits.`);
  }
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  ...
  if (asString(metadata?.sourceKind) === "paperclip_bundled") {
    ...
    return {
      kind: "github-dir",
      repo: `${VENDORED_ENGINE_OWNER}/${VENDORED_ENGINE_REPO}`,
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/${VENDORED_ENGINE_OWNER}/${VENDORED_ENGINE_REPO}/tree/master/skills/${skill.slug}`,
    };
  }
...
function buildManifestFromPackageFiles(
  ...
  for (const skillPath of skillPaths) {
    ...
    if (sourceKind === "github-dir" || sourceKind === "github-file") {
      ...
      const normalizedSourceKind = isVendoredEngineRepo(owner, repoName) && isVendoredEngineSkillKey(canonicalKey)
        ? "paperclip_bundled"
        : "github";
      sourceType = "github";
