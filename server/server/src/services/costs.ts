...
export function costService(db: Db, budgetHooks: BudgetServiceHooks = {}) {
  ...
  return {
    createEvent: async (companyId: string, data: Omit<typeof costEvents.$inferInsert, "companyId">) => {
      ...
      // The optional FK references must belong to the same company, mirroring
      // financeService.createEvent. Without this, an agent-key caller could attach
      // its cost event to ANOTHER company's issue/project/goal/run — a cross-tenant
      // NO ACTION reference that leaks linkage and blocks that company's deletion.
