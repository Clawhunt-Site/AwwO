...
// Reason marker for the pause hold that a user-initiated chat Stop places on
// the chat-backed issue. It durably parks the issue against the periodic
// stranded-issue recovery (which cannot otherwise tell "user said stop" from
// "execution path lost") and is auto-released by the user's next turn.
export const CHAT_STOP_HOLD_REASON = "chat_stop";
...
export async function isVerifiedIssueCommentInteractionWake(
  dbOrTx: Pick<Db, "select">,
  input: {
    companyId: string;
    issueId: string;
...
// This verifier checks a comment-interaction wake against the real issue comment
// (existence on the issue) + the wake requester's actor matching the comment
// author. Named for what it verifies (comment interaction); the original
// tree-control name is kept as an alias for existing tree-hold callers.
export const isVerifiedIssueTreeControlInteractionWake = isVerifiedIssueCommentInteractionWake;
...
export function issueTreeControlService(db: Db) {
  ...
  // Chat-stop park helpers. Multiple active pause holds can share a root and
  // `getActivePauseHoldGate` collapses them to one (newest wins), so both the
  // idempotency probe and the release MUST query the holds table directly —
  // gate-based probing under-creates the park when a manual hold shadows it,
  // and gate-based release strands an older chat-stop hold behind a newer one.
  async function hasActiveChatStopHold(companyId: string, issueId: string) {
    return db
      ...
      .where(
        and(
          ...
          eq(issueTreeHolds.status, "active"),
          eq(issueTreeHolds.mode, "pause"),
          eq(issueTreeHolds.reason, CHAT_STOP_HOLD_REASON),
        ),
      )
      .limit(1)
  ...
  async function releaseChatStopHolds(
    companyId: string,
    issueId: string,
    input: { reason: string; actor: ActorInput },
  ) {
    const holds = await db
      .select({ id: issueTreeHolds.id })
      .from(issueTreeHolds)
    ...
    for (const hold of holds) {
      try {
        await releaseHold(companyId, issueId, hold.id, {
          reason: input.reason,
          actor: input.actor,
        });
        released += 1;
      } catch {
        // A concurrent turn released this hold first; the remaining holds (and
        // the caller's gate re-read) still decide the final state.
      }
    }
    return released;
  }
  ...
  return {
    ...
    getHold,
    hasActiveChatStopHold,
    listHolds,
    getActivePauseHoldGate,
    releaseChatStopHolds,
    releaseHold,
