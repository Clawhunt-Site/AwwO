...
/**
 * Validate the typed `pluginTools` opt-in at the SINGLE persistence boundary, so
 * every write path — HTTP routes, plugin-managed-agent declarations, and
 * company-as-code import — is held to the same contract. Without this, a
 * misspelled key (`enable`) or non-boolean would slip past the non-route paths
 * and silently never enable the bridge. Only the `pluginTools` sub-field is
 * checked (not the whole runtimeConfig) so other catchall keys are untouched.
 */
export function assertPluginToolsConfigValid(runtimeConfig: unknown): void {
  if (isPlainRecord(runtimeConfig) && Object.prototype.hasOwnProperty.call(runtimeConfig, "pluginTools")) {
    agentPluginToolsConfigSchema.parse(runtimeConfig.pluginTools);
  }
}

function normalizeRuntimeConfigForNewAgent(runtimeConfig: unknown): Record<string, unknown> {
  assertPluginToolsConfigValid(runtimeConfig);
...
export function agentService(db: Db) {
  ...
  async function updateAgent(
    ...
    if (Object.prototype.hasOwnProperty.call(normalizedPatch, "runtimeConfig")) {
      assertPluginToolsConfigValid(normalizedPatch.runtimeConfig);
    }
