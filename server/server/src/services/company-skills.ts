...
import { isVendoredEngineRepo, isVendoredEngineSkillKey, VENDORED_ENGINE_SKILL_KEY_PREFIX } from "@paperclipai/shared/skill-key-namespaces";
...
function assertImportedSkillKeyAllowed(skill: ImportedSkill) {
  if (!isVendoredEngineSkillKey(skill.key)) return;
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  const sourceKind = asString(metadata?.sourceKind);
  if (sourceKind === "paperclip_bundled") return;
  throw unprocessable(
    `Reserved SuperClaw skill key "${skill.key}" cannot be imported from unbundled sources.`,
    {
      skillKey: skill.key,
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (isVendoredEngineSkillKey(key)) return slug;
...
function deriveCanonicalSkillKey(
  ...
  if (sourceKind === "paperclip_bundled") {
    return `${VENDORED_ENGINE_SKILL_KEY_PREFIX}${slug}`;
  }
...
function deriveImportedSkillSource(
  ...
  if (kind === "github-dir" || kind === "github-file") {
    ...
    if (repo && owner && repoName) {
      const sourceKind = isVendoredEngineRepo(owner, repoName) && isVendoredEngineSkillKey(canonicalKey)
        ? "paperclip_bundled"
        : "github";
      return {
        sourceType: "github",
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but SuperClaw cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but SuperClaw cannot find a valid local runtime source for it.`;
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "paperclip_bundled") {
    return {
      editable: false,
      editableReason: "Bundled SuperClaw skills are read-only.",
      sourceLabel: "SuperClaw bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "SuperClaw workspace",
        sourceBadge: "paperclip",
...
export function companySkillService(db: Db) {
  ...
  async function installFromCatalog(
    ...
    const values = {
      ...
      authorName: storeMetadata.authorName ?? existingByKey?.authorName ?? "SuperClaw",
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        ...
        && incomingKind === "github"
        && isVendoredEngineRepo(incomingOwner, incomingRepo)
      ) {
        out.push(existing);
        continue;
      }
