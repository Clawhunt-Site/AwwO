...
// Wake sources of the automatic stranded-issue recovery loop itself. Comments
// created by runs from this lineage are the loop's own output and must not
// count as "recent visible progress" for the GGU-809 exemption above —
// otherwise each requeued continuation's reply re-arms the next requeue and
// the escalation valve never fires (self-feeding livelock).
const AUTOMATIC_STRANDED_RECOVERY_SOURCES = new Set<string>([
...
function buildLivenessEscalationDescription(finding: IssueLivenessFinding) {
  ...
  return [
    "The workspace detected a harness-level issue graph liveness incident.",
    "",
    "## Source",
    "",
...
function buildLivenessOriginalIssueComment(finding: IssueLivenessFinding, escalation: typeof issues.$inferSelect) {
  return [
    "The workspace detected a harness-level liveness incident in this issue's dependency graph.",
    "",
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  //
  // Comments created by the automatic stranded-recovery lineage itself do NOT
  // count: a recovery continuation that only replies "acknowledged" would
  // otherwise feed the very exemption that keeps re-arming it — each requeued
  // run posts one more comment, that comment suppresses the escalation valve,
  // and the loop pings the thread forever (livelock). Attachments and comments
  // from any other run (user turns, timer heartbeats, normal continuations) or
  // from no run at all still count, so batch workflows with real output keep
  // their exemption.
  async function hasRecentVisibleProgress(
    ...
    const [comments, attachment] = await Promise.all([
      db
        .select({ id: issueComments.id, createdByRunId: issueComments.createdByRunId })
        .from(issueComments)
        .where(
          and(
    ...
    // Comments without run provenance (user/operator/agent-outside-a-run) are
    // real engagement, never recovery-loop output.
    if (comments.some((comment) => !comment.createdByRunId)) return true;

    const runIds = [
      ...new Set(comments.map((comment) => comment.createdByRunId).filter((id): id is string => Boolean(id))),
    ];
    const runs = await db
    ...
    const recoveryLineageRunIds = new Set(
      runs
        .filter((run) =>
          AUTOMATIC_STRANDED_RECOVERY_SOURCES.has(
            readNonEmptyString(parseObject(run.contextSnapshot).source) ?? "",
          ),
        )
        .map((run) => run.id),
    );
    // A comment from an unknown run id (FK set null races, purged runs) keeps
    // the current exempting behaviour rather than escalating on ambiguity.
    return comments.some(
      (comment) => !comment.createdByRunId || !recoveryLineageRunIds.has(comment.createdByRunId),
    );
  }
    ...
    for (const candidate of candidates) {
      ...
      await issuesSvc.addComment(
        ...
        [
          ...
          "",
          `The workspace found this issue is blocking ${blockingLinks} but had no assignee, so no heartbeat could pick it up.`,
          "",
  ...
  function buildStaleRunEvaluationDescription(input: {
    ...
    const blockers = input.evidence.blockers.length > 0
      ...
      : "- none detected";
    return [
      `The workspace detected ${input.level} output silence on an active heartbeat run.`,
      "",
      "## Run",
      "",
  ...
  async function ensureSourceIssueCommentedForStaleEvaluation(input: {
    ...
    await issuesSvc.addComment(input.sourceIssue.id, [
      "The workspace detected critical output silence on this issue's active run.",
      "",
  ...
  function buildStrandedIssueRecoveryDescription(input: {
    ...
    if (input.recoveryCause === SUCCESSFUL_RUN_MISSING_STATE_REASON) {
      ...
      return [
        "The workspace exhausted the bounded corrective handoff for a successful run that still has no valid issue disposition.",
        "",
    ...
    return [
      "The workspace exhausted automatic recovery for an assigned issue and created this explicit recovery task.",
      "",
      "## Source",
      "",
      `- Source issue: ${sourceIssue}`,
  ...
  function buildRecoveryIssueInPlaceEscalationComment(input: {
    ...
    return [
      "The workspace stopped automatic stranded-work recovery for this recovery issue.",
      "",
  ...
  async function resolveContinuationWaitingOnReview(issue: typeof issues.$inferSelect) {
    ...
    await issuesSvc.addComment(
      ...
      `This task is waiting on ${waitingOn} to finish. ` +
        ...
        "The workspace turned that into a normal dependency wait instead of flagging it as stuck.)",
      {},
      { authorType: "system" },
    );
    await logActivity(db, {
  ...
  async function escalateStrandedAssignedIssue(input: {
    ...
    const recoveryLine = recoveryAction.ownerAgentId
      ...
      : [
        "",
        `- Recovery action: \`${recoveryAction.id}\``,
        "- Recovery owner: board escalation, because the workspace could not find an invokable manager, creator, or executive owner with budget available.",
      ...
      if (issue.status === "todo") {
        ...
        if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "The workspace automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
        ...
        if (isRepeatedProductiveContinuationRecovery(successfulRun)) {
          ...
          if (!exempted) {
            const updated = await escalateStrandedAssignedIssue({
              ...
              comment:
                "The workspace automatically retried continuation for this assigned `in_progress` issue and the retry " +
      ...
      if (isUnsuccessfulTerminalIssueRun(latestRun)) {
        ...
        if (classification.kind === "non_retryable") {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "The workspace detected a non-retryable failure on this issue's continuation run " +
        ...
        if (didAutomaticRecoveryFail(latestRun, "issue_continuation_needed")) {
          ...
          if (consecutive >= classification.maxAttempts) {
            ...
            const updated = await escalateStrandedAssignedIssue({
              ...
              latestRun,
              comment:
                "The workspace automatically retried continuation for this assigned `in_progress` issue after its live " +
