...
// SuperClaw coexist: chat sessions are persisted as issues with this origin kind
// (mirrors chat-compat's CHAT_SESSION_ORIGIN_KIND — defined locally to avoid an
// issues<->chat-compat import cycle). They are conversations, not board work
// items, so they are excluded from the user/board-facing issue lists + counts
// (same treatment as plugin-operation issues). The chat surface lists them via
// its own direct queries; an explicit originKind="chat" filter still returns them
// (the eq() filter then pins the kind, so the exclusion is skipped).
const CHAT_SESSION_ORIGIN_KIND = "chat";
function nonChatSessionIssueCondition() {
  return sql<boolean>`${issues.originKind} IS DISTINCT FROM ${CHAT_SESSION_ORIGIN_KIND}`;
}
...
async function blockedInboxIssueConditions(
  ...
  if (filters?.originKind !== CHAT_SESSION_ORIGIN_KIND) conditions.push(nonChatSessionIssueCondition());
  if (filters?.labelId) {
...
export function issueService(db: Db) {
  ...
  return {
    ...
    list: async (companyId: string, filters?: IssueFilters) => {
      ...
      if (filters?.originKind !== CHAT_SESSION_ORIGIN_KIND) {
        conditions.push(nonChatSessionIssueCondition());
      }
      if (filters?.labelId) {
