...
import { executeTool as routeToolThroughRuntime } from "./plugin-runtime-router.js";
import {
  callSuperPluginTool,
  openSuperMcpSession,
  type NormalizedToolResult,
} from "./super-mcp-runner.js";
import { assertValidToolParameters } from "./plugin-tool-schema.js";
import { logger } from "../middleware/logger.js";

// Super tool calls run as a stateless MCP subprocess; mirror the admin super
// surface's per-call deadline (super-workshop.ts) so timeout behaviour is uniform.
const SUPER_TOOL_TIMEOUT_MS = 60_000;

/** Adapt a super-runtime NormalizedToolResult into the dispatcher's ToolResult. */
function normalizedToToolResult(n: NormalizedToolResult): ToolResult {
  return {
    content: n.text.length > 0 ? n.text : undefined,
    data: n.structured ?? undefined,
    error: n.isError ? (n.text.length > 0 ? n.text : "tool reported an error") : undefined,
  };
}
...
export function createPluginToolDispatcher(
  ...
  return {
    ...
    async executeTool(
      ...
      const parsed = registry.parseNamespacedName(namespacedName);
      if (!parsed) {
        throw new Error(`invalid namespaced tool name: ${namespacedName}`);
      }
      const { pluginId, toolName } = parsed;

      // Without a DB handle we cannot consult the super store or its provenance
      // gate; only paperclip JS tools (in-memory registry) can exist, so fall
      // back to the native registry path — still enforcing the declared schema.
      if (!db) {
        const tool = registry.getToolByPlugin(pluginId, toolName);
        // Existence first: a typo'd tool must read as unknown (→404 at the route),
        // not as a schema violation (→400) or a worker-down error (→502).
        if (!tool) {
          throw new Error(`does not declare tool: ${namespacedName}`);
        }
        assertValidToolParameters(tool.parametersSchema, parameters, namespacedName);
        return registry.executeTool(namespacedName, parameters, runContext);
      }

      const pluginRegistry = pluginRegistryService(db);

      // Route through the governed runtime-router (blocker 3): super provenance +
      // digest-drift + dual-store conflict are enforced there and never bypassed.
      // Schema validation (blocker 4) is co-located with each runtime's declared
      // schema source — super tools are NOT in the in-memory tool registry, so we
      // validate paperclip against registry.parametersSchema and super against the
      // super record's tools[].inputSchema.
      const routed = await routeToolThroughRuntime(db, pluginId, toolName, parameters, {
        callSuperTool: (record, tool, input) => {
          const decl = record.tools.find((t) => t.name === tool);
          assertValidToolParameters(decl?.inputSchema, input, namespacedName);
          return callSuperPluginTool(tool, input, {
            openSession: () => openSuperMcpSession(record),
            timeoutMs: SUPER_TOOL_TIMEOUT_MS,
          });
        },
        dispatchPaperclipTool: async (pluginKey, tool, input) => {
          const decl = registry.getToolByPlugin(pluginKey, tool);
          // Existence first (see no-db branch): unknown tool → 404, never a 400
          // schema error or a 502 worker error.
          if (!decl) {
            throw new Error(`does not declare tool: ${pluginKey}:${tool}`);
          }
          assertValidToolParameters(decl.parametersSchema, input, namespacedName);
          const r = await registry.executeTool(
            registry.buildNamespacedName(pluginKey, tool),
            input,
            runContext,
          );
          return r.result;
        },
        isPaperclipPlugin: async (key) => (await pluginRegistry.getByKey(key)) !== null,
      });

      const result: ToolResult =
        routed.kind === "super"
          ? normalizedToToolResult(routed.result as NormalizedToolResult)
          : (routed.result as ToolResult);
