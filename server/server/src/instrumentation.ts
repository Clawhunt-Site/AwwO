...
export function shutdownInstrumentation(): Promise<void> {
  shutdownPromise ??= (async () => {
    ...
    try {
      ...
      console.error(`${LOG_PREFIX} OpenTelemetry shutdown failed`, err);
    }
  })();
...
export function resolveProtocol(): {
  ...
  switch (raw) {
    ...
    default:
      // eslint-disable-next-line no-console
      console.warn(
        `${LOG_PREFIX} Unknown OTEL_EXPORTER_OTLP_PROTOCOL=${raw}; falling back to grpc. ` +
...
async function bootstrapOtel(endpoint: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        `${LOG_PREFIX} OpenTelemetry SDK failed to start; continuing without tracing`,
        err,
      );
      return;
    }
    ...
    // eslint-disable-next-line no-console
    console.warn(
      `${LOG_PREFIX} OTEL_EXPORTER_OTLP_ENDPOINT is set but the @opentelemetry/* ` +
