...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    const runIdHeader = (req.header("x-superclaw-run-id") || req.header("x-paperclip-run-id"));
    ...
    if (!key) {
      ...
      req.actor = {
        ...
        keyId: undefined,
        // Agent-JWT run binding comes from the SIGNED claim only — the
        // x-*-run-id header is caller-controllable, so honoring it here would let
        // a holder of a valid agent/company run-JWT retarget another run of the
        // same agent (defeating the runContext<->actor binding in
        // runContextActorMismatch). createLocalAgentJwt always carries run_id, so
        // the header is never needed for this path; it stays for non-JWT actors.
...
export async function resolveCloudTenantActor(db: Db, req: Request): Promise<Express.Request["actor"] | null> {
  ...
  const companyName = paperclipCompanyId || `${stackId} SuperClaw`;
  const now = new Date();
  ...
  await db
    ...
    .values({
      id: companyId,
      name: companyName,
      description: `Provisioned by SuperClaw Cloud for stack ${stackId}.`,
      status: "active",
