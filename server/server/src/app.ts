...
import { chatCompatStubsRoutes } from "./routes/chat-stubs.js";
...
import { superWorkshopRoutes } from "./routes/super-workshop.js";
...
export async function createApp(
  ...
  // Capability Workshop static contracts — mounted BEFORE pluginRoutes so
  // `/plugins/github-catalog` resolves here rather than `/plugins/:pluginId`.
  api.use(workshopContractRoutes());
  api.use("/companies", companyRoutes(db, opts.storageService));
  api.use(llmRoutes(db));
  api.use(companySkillRoutes(db));
  api.use(teamsCatalogRoutes(db));
  // Before agentRoutes: the legacy composer-inventory paths (/agents/probe,
  // /agents/:backend/models) must win over the native /api/agents/:id lookup.
  api.use(chatRuntimeRoutes({ deploymentMode: opts.deploymentMode }));
  ...
  api.use(chatRoutes(db, { deploymentMode: opts.deploymentMode, pluginWorkerManager: workerManager }));
  api.use(chatCompatStubsRoutes(db, { deploymentMode: opts.deploymentMode }));
  ...
  // Add-only super-workshop surface (dual plugin runtime: import + unified catalog +
  // super tool execute + uninstall). Does NOT modify pluginRoutes above.
  api.use(superWorkshopRoutes(db, { logger, storageService: opts.storageService }));
  ...
  // Legacy /v1/skills lives at the app root (not under /api).
  app.use(chatSkillsRoutes(db, { deploymentMode: opts.deploymentMode }));
  app.use("/api", api);
  ...
  if (opts.uiMode === "static") {
    ...
    if (uiDist) {
      ...
      console.warn(`${LOG_PREFIX} UI dist not found; running in API-only mode`);
    }
  }
