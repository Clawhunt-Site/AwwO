...
const mockUnionGlobalPlugins = vi.hoisted(() => vi.fn());
...
vi.mock("../services/global-runtime-plugins.js", () => ({
  unionGlobalRuntimePluginEntries: mockUnionGlobalPlugins,
}));
...
function createSelectQueueDb(rows: Array<Array<Record<string, unknown>>>) {
  return {
    select: vi.fn(() => ({
      from: vi.fn(() => ({
        // `where()` shifts one queued result and returns a value usable BOTH as
        // `.where().limit()` (older single-row lookups) AND as a bare awaited
        // `.where()` (the multi-row owning-issues query, which has no limit).
        where: vi.fn(() => {
          const result = Promise.resolve(rows.shift() ?? []);
          return {
            limit: vi.fn(() => result),
            then: (...args: Parameters<Promise<unknown>["then"]>) => result.then(...args),
            catch: (...args: Parameters<Promise<unknown>["catch"]>) => result.catch(...args),
            finally: (...args: Parameters<Promise<unknown>["finally"]>) => result.finally(...args),
          };
        }),
      })),
    })),
...
describe.sequential("plugin install and upgrade authz", () => {
  ...
  it("GET /plugins/global returns the verified global union, deduped against installed ids", async () => {
    mockRegistry.listInstalled.mockResolvedValue([
      { pluginKey: "acme.installed" },
      { pluginKey: "acme.other" },
    ]);
    const projected = [
      { key: "superclaw-global:acme.installed", pluginId: "acme.installed", origin: "superclaw-global", alreadyInstalled: true },
      { key: "superclaw-global:acme.fresh", pluginId: "acme.fresh", origin: "superclaw-global", alreadyInstalled: false },
    ];
    mockUnionGlobalPlugins.mockResolvedValue(projected);

    const { app } = await createApp(boardActor());
    const res = await request(app).get("/api/plugins/global");

    expect(res.status).toBe(200);
    expect(res.body).toEqual(projected);
    // The route hands the union the set of installed plugin keys so it can annotate.
    expect(mockUnionGlobalPlugins).toHaveBeenCalledTimes(1);
    const passedIds = mockUnionGlobalPlugins.mock.calls[0][0] as Set<string>;
    expect(passedIds).toBeInstanceOf(Set);
    expect([...passedIds].sort()).toEqual(["acme.installed", "acme.other"]);
  });
...
describe.sequential("plugin tool and bridge authz", () => {
  ...
  it("rejects tool execution when any runContext reference is outside the company scope", async () => {
    const cases: Array<[string, Array<Array<Record<string, unknown>>>]> = [
      ...
      [
        "projectId not matching the run's owning project",
        [
          [{ companyId: companyA }],
          [{ companyId: companyA, agentId: agentA }],
          // the issue that owns this run (checkout/execution) points elsewhere;
          // company-scope would pass, run-binding must not. Mismatch is checked
          // before the company query, so no projects row is consumed here.
          [{ projectId: "00000000-0000-4000-8000-0000000000ff" }],
        ],
      ],
      [
        "projectId not derivable from the run (fail-closed)",
        [
          [{ companyId: companyA }],
          [{ companyId: companyA, agentId: agentA }],
          [], // no issue owns this run -> no derivable project -> reject
        ],
      ],
    ];
  ...
  it("allows tool execution when agent, run, and project all belong to runContext.companyId", async () => {
    ...
    const { app } = await createApp(boardActor(), {}, {
      db: createSelectQueueDb([
        [{ companyId: companyA }],
        [{ companyId: companyA, agentId: agentA }],
        [{ projectId: projectA }], // issue that owns the run -> project A
        [{ companyId: companyA }], // project A ∈ companyA
      ]),
      toolDeps: {
        toolDispatcher: {
          listToolsForAgent: vi.fn(),
    ...
    const { app } = await createApp(
      ...
      {},
      {
        db: createSelectQueueDb([
          [{ companyId: companyA }],
          [{ companyId: companyA, agentId: agentA }],
          [{ projectId: projectA }], // owning issue's project
          [{ companyId: companyA }], // project A ∈ companyA
        ]),
        toolDeps: {
          toolDispatcher: {
  ...
  it("rejects an agent executing under another agent's SELF-CONSISTENT runContext in the same company", async () => {
    // The foreign runContext is fully self-consistent in companyA, so
    // validateToolRunContextScope PASSES — only the actor-identity binding
    // (blocker 1) rejects it. This is the cross-agent impersonation the DB
    // scope check alone cannot catch.
    const otherAgent = "88888888-8888-4888-8888-888888888888";
    const executeTool = vi.fn();
    const { app } = await createApp(
      agentActor(), // authenticated as agentA
      {},
      {
        db: createSelectQueueDb([
          [{ companyId: companyA }], // otherAgent ∈ companyA
          [{ companyId: companyA, agentId: otherAgent }], // run ∈ otherAgent/companyA
          [{ projectId: projectA }], // owning issue's project (scope PASSES); only the actor binding rejects
          [{ companyId: companyA }], // project A ∈ companyA
        ]),
        toolDeps: {
          toolDispatcher: {
  ...
  it("rejects when the run owns issues across MORE THAN ONE project (ambiguous, fail-closed)", async () => {
    const executeTool = vi.fn();
    const { app } = await createApp(agentActor(), {}, {
      db: createSelectQueueDb([
        [{ companyId: companyA }],
        [{ companyId: companyA, agentId: agentA }],
        // run owns two issues in different projects -> cannot bind unambiguously
        [{ projectId: projectA }, { projectId: "00000000-0000-4000-8000-0000000000ee" }],
      ]),
      toolDeps: {
        toolDispatcher: {
          listToolsForAgent: vi.fn(),
    ...
    expect(res.status).toBe(403);
    expect(res.body.error).toMatch(/multiple projects/);
    expect(executeTool).not.toHaveBeenCalled();
  });

  it("rejects when the run's owning project belongs to a DIFFERENT company (FK has no company composite)", async () => {
    const executeTool = vi.fn();
    const { app } = await createApp(agentActor(), {}, {
      db: createSelectQueueDb([
        [{ companyId: companyA }],
        [{ companyId: companyA, agentId: agentA }],
        [{ projectId: projectA }], // owning issue references projectA...
        [{ companyId: companyB }], // ...but projectA actually belongs to companyB
      ]),
      toolDeps: {
        toolDispatcher: {
          listToolsForAgent: vi.fn(),
    ...
    expect(res.status).toBe(403);
    expect(res.body.error).toMatch(/does not belong to "runContext\.companyId"/);
    expect(executeTool).not.toHaveBeenCalled();
  });

  it("rejects an agent_key (API-key) caller on the run-bound tool-execute path", async () => {
    // Only the signed, run-bound agent JWT may drive plugin tools; agent_key's
    // runId comes from a caller-controllable header, so it is rejected outright.
    const executeTool = vi.fn();
    const { app } = await createApp(
      { type: "agent", agentId: agentA, companyId: companyA, runId: runA, source: "agent_key" },
      {},
      {
        toolDeps: {
          toolDispatcher: { listToolsForAgent: vi.fn(), getTool: vi.fn(), executeTool },
        },
      },
    );
    ...
    expect(res.status).toBe(403);
    expect(res.body.error).toMatch(/run-bound agent JWT/);
    expect(executeTool).not.toHaveBeenCalled();
  });
});
