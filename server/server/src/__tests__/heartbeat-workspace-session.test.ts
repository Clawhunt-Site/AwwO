...
describe("operationalIssueCommentTarget", () => {
  it("posts to a normal issue run but never to a chat-origin run (no System turns in chat)", () => {
    expect(operationalIssueCommentTarget({ issueId: "issue-1", isChatOrigin: false })).toBe("issue-1");
    expect(operationalIssueCommentTarget({ issueId: "issue-1", isChatOrigin: true })).toBeNull();
    expect(operationalIssueCommentTarget({ issueId: null, isChatOrigin: false })).toBeNull();
    expect(operationalIssueCommentTarget({ issueId: null, isChatOrigin: true })).toBeNull();
  });
});

describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits workspace warnings on stderr with the superclaw prefix (diagnostics, never chat text)", () => {
    // stderr matters: the chat stream projects ONLY stdout into the visible
    // transcript, so stdout here leaked fallback-workspace warnings into chat
    // on plain-text runtimes.
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stderr",
      chunk: "[superclaw] Using fallback workspace\n",
    });
  });
});
