...
describe("server adapter registry", () => {
  ...
  it("isBuiltinAdapterActive tracks provenance across register/pause/unpause/unregister", () => {
    // Baseline: the real built-in is active; non-built-in / unknown types are never
    // "built-in active".
    expect(isBuiltinAdapterActive("claude_local")).toBe(true);
    expect(isBuiltinAdapterActive("external_test")).toBe(false);
    expect(isBuiltinAdapterActive("definitely_not_a_type")).toBe(false);

    const override: ServerAdapterModule = { ...externalAdapter, type: "claude_local" };
    // External adapter overrides the built-in type → external active, NOT built-in.
    registerServerAdapter(override);
    expect(isBuiltinAdapterActive("claude_local")).toBe(false);

    // Pause the override → built-in restored as active.
    setOverridePaused("claude_local", true);
    expect(isBuiltinAdapterActive("claude_local")).toBe(true);

    // Unpause → external override active again.
    setOverridePaused("claude_local", false);
    expect(isBuiltinAdapterActive("claude_local")).toBe(false);

    // Unregister the override → built-in restored. Must NOT false-negative even
    // though unregister leaves builtinFallbacks populated (identity-based check).
    unregisterServerAdapter("claude_local");
    expect(isBuiltinAdapterActive("claude_local")).toBe(true);
  });
