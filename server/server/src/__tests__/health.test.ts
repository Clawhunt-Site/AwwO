...
describe("GET /health", () => {
  ...
  it("omits serverInfo on an anonymous authenticated health check even when none is injected", async () => {
    // Gating regression guard: the redacted path must NOT resolve serverInfo (no injected value,
    // and it must not fall back to the git snapshot), so the field is simply absent.
    const db = {
      execute: vi.fn().mockResolvedValue([{ "?column?": 1 }]),
      select: vi.fn(() => ({ from: vi.fn(() => ({ where: vi.fn().mockResolvedValue([{ count: 1 }]) })) })),
    } as unknown as Db;
    const app = express();
    app.use((req, _res, next) => {
      (req as any).actor = { type: "none", source: "none" };
    ...
    app.use(
      ...
      healthRoutes(db, {
        ...
        authReady: true,
        companyDeletionEnabled: false,
        // serverInfo intentionally NOT injected.
      }),
    );
