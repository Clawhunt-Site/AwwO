...
const ORIGINAL_SUPERCLAW_API_URL = process.env.SUPERCLAW_API_URL;
...
describe("startServer PAPERCLIP_API_URL handling", () => {
  beforeEach(() => {
    ...
    // startServer now reads + writes the SuperClaw alias too (de-brand dual-read);
    // a value written back by a prior startServer() call would otherwise shadow
    // the per-test PAPERCLIP_API_URL, so reset it symmetrically.
    delete process.env.SUPERCLAW_API_URL;
  });
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_SUPERCLAW_API_URL === undefined) delete process.env.SUPERCLAW_API_URL;
    else process.env.SUPERCLAW_API_URL = ORIGINAL_SUPERCLAW_API_URL;
