...
describe("runtime API discovery", () => {
  ...
  it("emits the loopback bind host over http even when the public origin is https (no TLS-error candidate)", () => {
    // Regression (real codex e2e): with an https public origin, the loopback bind-host
    // candidate was `https://127.0.0.1:<port>` — unreachable against the plain-http local
    // listener (curl TLS error), so the chat manager skills couldn't reach the local API.
    ...
    expect(candidates).toContain("http://127.0.0.1:8793");
    expect(candidates).not.toContain("https://127.0.0.1:8793");
  });
