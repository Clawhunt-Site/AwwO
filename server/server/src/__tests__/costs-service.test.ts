...
describeEmbeddedPostgres("cost and finance aggregate overflow handling", () => {
  ...
  it("rejects cost events whose FK references belong to another company", async () => {
    // Mirrors financeService's ownership checks. Without them, an agent-key caller
    // could attach a cost event to ANOTHER company's issue/project/goal/run —
    // a cross-tenant NO ACTION reference that also blocks that company's deletion.
    ...
    for (const [companyId, name, prefix] of [
      [companyA, "Company A", "AAA"],
      [companyB, "Company B", "BBB"],
    ] as const) {
      await db.insert(companies).values({ id: companyId, name, issuePrefix: prefix });
    }
    ...
    await db.insert(issues).values({ id: issueB, companyId: companyB, title: "B task" });
    await db.insert(projects).values({ id: projectB, companyId: companyB, name: "B project" });
    const goalB = randomUUID();
    await db.insert(goals).values({ id: goalB, companyId: companyB, title: "B goal" });
    ...
    await expect(costs.createEvent(companyA, { ...base, heartbeatRunId: runB })).rejects.toThrow(
      /Heartbeat run does not belong to company/,
    );
    await expect(costs.createEvent(companyA, { ...base, issueId: issueB })).rejects.toThrow(
      /Issue does not belong to company/,
    );
    await expect(costs.createEvent(companyA, { ...base, projectId: projectB })).rejects.toThrow(
      /Project does not belong to company/,
    );
    await expect(costs.createEvent(companyA, { ...base, goalId: goalB })).rejects.toThrow(
      /Goal does not belong to company/,
    );
    expect(await db.select().from(costEvents)).toHaveLength(0);

    // Same-company references stay accepted.
    ...
    const created = await costs.createEvent(companyA, { ...base, heartbeatRunId: runA });
    expect(created.heartbeatRunId).toBe(runA);
  });
});
