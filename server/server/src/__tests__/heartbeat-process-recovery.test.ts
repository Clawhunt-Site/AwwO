...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("does not exempt escalation when the only recent comment is the recovery run's own reply (livelock guard)", async () => {
    const { companyId, agentId, issueId, runId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "succeeded",
      retryReason: "issue_continuation_needed",
    ...
    // The only "visible progress" is the requeued continuation's own
    // acknowledgment. Counting it would re-arm the next requeue forever
    // (each retry posts one more comment that exempts the next escalation),
    // so the valve must fire instead.
    await db.insert(issueComments).values({
      companyId,
      issueId,
      authorAgentId: agentId,
  ...
  it("still exempts escalation when a recent comment comes from a non-recovery run (GGU-809 preserved)", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "succeeded",
      retryReason: "issue_continuation_needed",
    ...
    // A comment written by an ordinary (non-recovery-lineage) run — e.g. a
    // user-triggered turn — is real engagement and keeps the exemption. The
    // run is older than the fixture's latest run so it does not become the
    // reconciler's `latestRun`.
    const engagementRunId = randomUUID();
    await db.insert(heartbeatRuns).values({
      id: engagementRunId,
      companyId,
      agentId,
      ...
      status: "succeeded",
      contextSnapshot: { issueId, taskId: issueId, wakeReason: "issue_comment" },
    ...
    await db.insert(issueComments).values({
      ...
      issueId,
      authorAgentId: agentId,
      createdByRunId: engagementRunId,
      body: "frame 02/08 generated, attaching shortly",
    });
    const heartbeat = heartbeatService(db);
