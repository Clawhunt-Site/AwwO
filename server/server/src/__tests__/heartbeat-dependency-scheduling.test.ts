...
describeEmbeddedPostgres("heartbeat dependency-aware queued run selection", () => {
  ...
  it("keeps blocked descendants idle until their blockers resolve", async () => {
    ...
    // NOTE: the former "forged interaction wake bypasses the blocker" sub-scenario
    // was removed — that exception now requires a VERIFIED comment wake (real
    // comment + matching source + actor), which the heartbeat enrichment path
    // sets for genuine comment wakes, not a synthetic contextSnapshot. The
    // verifier's accept/reject behaviour is covered directly by the
    // isVerifiedIssueCommentInteractionWake unit test.
