...
describe("agent live run routes", () => {
  ...
  it("rejects an agent self-wake that carries a privilege-bearing payload (issue scope)", async () => {
    mockHeartbeatService.wakeup.mockClear();
    const res = await requestApp(
      await createApp({}, agentSelfActor),
      (baseUrl) => request(baseUrl)
        .post(`/api/agents/${routeAgentId}/heartbeat/invoke?companyId=company-1`)
  ...
  it("rejects an agent self-wake that carries a scoped reason", async () => {
    mockHeartbeatService.wakeup.mockClear();
    const res = await requestApp(
      await createApp({}, agentSelfActor),
      (baseUrl) => request(baseUrl)
        .post(`/api/agents/${routeAgentId}/wakeup?companyId=company-1`)
        .send({ source: "on_demand", reason: "issue_commented" }),
    );
    expect(res.status, JSON.stringify(res.body)).toBe(403);
    expect(mockHeartbeatService.wakeup).not.toHaveBeenCalled();
  });

  it("rejects an agent self-wake that carries payload.resumeFromRunId (indirect issue scope)", async () => {
    mockHeartbeatService.wakeup.mockClear();
    const res = await requestApp(
      await createApp({}, agentSelfActor),
      (baseUrl) => request(baseUrl)
        .post(`/api/agents/${routeAgentId}/heartbeat/invoke?companyId=company-1`)
        .send({ payload: { resumeFromRunId: "run-prev" } }),
    );
    expect(res.status, JSON.stringify(res.body)).toBe(403);
    expect(mockHeartbeatService.wakeup).not.toHaveBeenCalled();
  });

  it("rejects an agent self-wake that selects a non-on_demand source", async () => {
    mockHeartbeatService.wakeup.mockClear();
    const res = await requestApp(
      await createApp({}, agentSelfActor),
      (baseUrl) => request(baseUrl)
  ...
  it("allows a generic agent self-ping (no reason / no scoped payload)", async () => {
    mockHeartbeatService.wakeup.mockClear();
    const res = await requestApp(
      await createApp({}, agentSelfActor),
      (baseUrl) => request(baseUrl)
