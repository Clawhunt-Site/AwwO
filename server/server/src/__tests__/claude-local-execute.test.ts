...
if (capturePath) {
  fs.writeFileSync(capturePath, JSON.stringify({
    paperclipApiKey: process.env.PAPERCLIP_API_KEY || null,
    superclawApiKey: process.env.SUPERCLAW_API_KEY || null,
    paperclipApiUrl: process.env.PAPERCLIP_API_URL || null,
    superclawApiUrl: process.env.SUPERCLAW_API_URL || null,
  }), "utf8");
}
console.log(JSON.stringify({ type: "result", session_id: "22222222-2222-4222-8222-222222222222", result: "ok", usage: { input_tokens: 1, cache_read_input_tokens: 0, output_tokens: 1 } }));
`;
  await fs.writeFile(commandPath, script, "utf8");
  await fs.chmod(commandPath, 0o755);
}
...
describe("claude execute", () => {
  ...
  it("mirrors an explicit SUPERCLAW_API_KEY onto PAPERCLIP_API_KEY in the child env (de-brand compat)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-claude-exec-mirror-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root, {
      commandWriter: writeEnvCaptureClaudeCommand,
    });
    try {
      await execute({
        runId: "run-mirror",
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          // User configures only the NEW name; authToken is present but injection
          // is skipped because hasExplicitApiKey is true.
          env: { PAPERCLIP_TEST_CAPTURE_PATH: capturePath, SUPERCLAW_API_KEY: "sc-only-key" },
          promptTemplate: "Do work.",
        },
        context: {},
        authToken: "tok",
      ...
      const captured = JSON.parse(await fs.readFile(capturePath, "utf-8"));
      // The explicit new-name key must be honored AND mirrored onto the legacy
      // name, so prompt gates / remote bridge token that still read
      // PAPERCLIP_API_KEY keep working when a user only sets SUPERCLAW_API_KEY.
      expect(captured.superclawApiKey).toBe("sc-only-key");
      expect(captured.paperclipApiKey).toBe("sc-only-key");
    } finally {
      restore();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("reuses a stable Paperclip-managed Claude prompt bundle across equivalent runs", async () => {
    ...
    try {
      ...
      expect(capture2.prompt).toContain("## Resume Delta");
