...
/**
 * Per-agent persistent opt-in for the plugin-tool MCP bridge (the "always-on"
 * half of the bridge opt-in; the run-scoped `plugin://` mention is the instant
 * half). The heartbeat gate reads `pluginTools.enabled === true`; making it a
 * typed first-class field (rather than relying on the runtimeConfig catchall)
 * means a misspelled key is rejected instead of silently never enabling tools.
 */
export const agentPluginToolsConfigSchema = z.object({
  enabled: z.boolean().optional(),
}).strict();
...
export const agentRuntimeConfigSchema = z.object({
  ...
  pluginTools: agentPluginToolsConfigSchema.optional(),
}).catchall(z.unknown());
