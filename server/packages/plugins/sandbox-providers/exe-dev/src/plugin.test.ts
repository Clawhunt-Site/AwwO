...
describe("exe.dev sandbox provider plugin", () => {
  ...
  it("normalizes config and emits SSH guidance warnings", async () => {
    ...
    expect(result).toEqual({
      ok: true,
      warnings: [
        "The SuperClaw host must have SSH access to the created exe.dev VM, and its SSH key must be registered with exe.dev. The API token only covers provisioning.",
  ...
  it("surfaces exe.dev SSH onboarding guidance during lease acquisition", async () => {
    ...
    await expect(plugin.definition.onEnvironmentAcquireLease?.({
      ...
      "the SuperClaw host SSH key is not registered with exe.dev",
    );
  ...
  it("returns exe.dev SSH onboarding guidance for command execution failures", async () => {
    ...
    expect(String(result?.stderr ?? "")).toContain("the SuperClaw host SSH key is not registered with exe.dev");
  ...
  it("returns onboarding guidance when probe hits exe.dev SSH registration", async () => {
    ...
    expect(String(result?.metadata?.error ?? "")).toContain("the SuperClaw host SSH key is not registered with exe.dev");
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'paperclip-probe'");
  });
