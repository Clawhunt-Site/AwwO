...
export async function startAdapterExecutionTargetPaperclipBridge(input: {
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side runtime API token.");
  }
  ...
  return {
    env: {
      PAPERCLIP_API_URL: server.baseUrl, SUPERCLAW_API_URL: server.baseUrl,
      PAPERCLIP_API_KEY: bridgeToken, SUPERCLAW_API_KEY: bridgeToken,
