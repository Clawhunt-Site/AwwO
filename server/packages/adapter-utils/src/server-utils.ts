...
export const WATCHDOG_DEFAULT_MANDATE = [
  ...
  "- If you discover a SuperClaw product or platform bug while reviewing the stopped subtree, create a linked engineering follow-up outside the watched source tree using the server-provided watchdog discovery route instead of making it a source child.",
...
> {
  return {
    ...
    originLabel: "Managed by SuperClaw",
    readOnly: false,
  };
}
...
type PaperclipWakePayload = {
  reason: string | null;
  /**
   * SuperClaw chat-origin marker. When true, this is a plain chat turn (not a
   * Paperclip issue/ticket): the renderer emits ONLY the conversation transcript —
   * no "Paperclip Wake Payload" header, no issue/company execution contract, no
   * issue framing. The transcript (comments) is the cross-model-switch continuity
   * carrier, so it is preserved. company issues leave this false and keep the full
   * Paperclip wake prompt unchanged.
   */
  chat: boolean;
...
export function normalizePaperclipWakePayload(value: unknown): PaperclipWakePayload | null {
  ...
  return {
    ...
    chat: asBoolean(payload.chat, false),
...
/**
 * Lean chat transcript renderer for SuperClaw chat turns. Emits ONLY the
 * conversation (role-labelled prior turns) — the cross-model-switch continuity
 * carrier — with NO Paperclip wake/issue/execution-contract scaffolding. Mirrors
 * super's chat_prompt.py task-context (history block). Returns "" when there is no
 * transcript to replay (e.g. a live native session carries continuity itself).
 */
function renderChatTranscriptPrompt(normalized: PaperclipWakePayload): string {
  if (normalized.comments.length === 0) return "";
  const lines: string[] = [
    "Conversation so far (oldest first; this is your memory of this session — rely on it):",
    "",
  ];
    ...
    const role =
      comment.authorType === "user"
        ...
        : comment.authorType === "agent"
          ? "Assistant"
          : comment.authorType
            ? comment.authorType.charAt(0).toUpperCase() + comment.authorType.slice(1)
            : "Unknown";
    lines.push(`${role}: ${comment.body}`);
    if (comment.bodyTruncated) lines.push("[message truncated]");
  }
  if (normalized.missingCount > 0) {
    lines.push("", `[${normalized.missingCount} earlier message(s) omitted]`);
  }
  return lines.join("\n").trim();
}
...
export function renderPaperclipWakePrompt(
  ...
  // SuperClaw chat turns get a LEAN transcript only — no "Paperclip Wake Payload"
  // header, no issue/company execution contract, no issue framing. This mirrors
  // super's chat_prompt.py (conversation history + current turn). The transcript is
  // also the cross-model-switch continuity carrier (replayed comments), so it is
  // preserved verbatim; the issue/ticket fields on the payload are simply not
  // rendered (which also sidesteps any issue-summary null/fallback concern).
  if (normalized.chat) {
    return renderChatTranscriptPrompt(normalized);
  }
  ...
  const lines = resumedSession
      ? [
        "## Resume Delta",
        "",
        "You are resuming an existing session.",
...
export function buildPaperclipEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const apiUrl =
    ...
    (process.env.SUPERCLAW_API_URL || process.env.PAPERCLIP_API_URL) ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.PAPERCLIP_API_URL = apiUrl;
  vars.SUPERCLAW_API_URL = apiUrl;
  return vars;
}
...
export function refreshPaperclipWorkspaceEnvForExecution(input: {
  ...
  mirrorApiEnvAliases(input.env);

  return shapedWorkspaceEnv;
}

/**
 * De-brand compat: keep the SuperClaw and Paperclip API env aliases in sync so
 * explicit config under either name is honored under both. Call this after an
 * adapter has finished applying envConfig + any host-token injection, so the
 * child process always sees both `SUPERCLAW_*` and `PAPERCLIP_*` whenever either
 * is set. The agent curl examples use `${SUPERCLAW_*:-$PAPERCLIP_*}`, but prompt
 * gates and the remote bridge token still read `PAPERCLIP_*` directly, so the
 * legacy name must stay populated even when a user only configures the new one.
 */
export function mirrorApiEnvAliases(env: Record<string, string>): void {
  // RUNTIME_API_URL / RUNTIME_API_CANDIDATES_JSON are mirrored so SuperClaw-owned
  // consumers (the chat manager skills) can read a `SUPERCLAW_RUNTIME_*` name instead
  // of the branded `PAPERCLIP_RUNTIME_*` — the value is whichever name is populated.
  //
  // The runtime API locator is a PROCESS-GLOBAL the host sets once at startup
  // (`process.env.PAPERCLIP_RUNTIME_API_URL` / `_CANDIDATES_JSON`, index.ts), NOT part of
  // the per-adapter `env` object. A bare `env[...]` lookup never finds it, so
  // `SUPERCLAW_RUNTIME_*` was never created and the manager skills' `_resolve_api` failed
  // with `api_base_unset` (the agent only inherited the branded `PAPERCLIP_*` at spawn).
  // So for those two suffixes, fall back to `process.env` for the source. The
  // agent-specific suffixes (API_URL/API_KEY/RUN_ID) stay env-only — they must never be
  // sourced from the host's global env.
  const PROCESS_GLOBAL_SUFFIXES = new Set(["RUNTIME_API_URL", "RUNTIME_API_CANDIDATES_JSON"]);
  const populated = (v: unknown): v is string => typeof v === "string" && v.trim().length > 0;
  for (const suffix of ["API_URL", "API_KEY", "RUN_ID", "RUNTIME_API_URL", "RUNTIME_API_CANDIDATES_JSON"]) {
    let sc: string | undefined = env[`SUPERCLAW_${suffix}`];
    let pc: string | undefined = env[`PAPERCLIP_${suffix}`];
    // Fall back to the host process.env ONLY when the adapter env carries NEITHER alias —
    // otherwise a stale host-global value could clobber a correct adapter-local one (and
    // agent-specific suffixes must never be sourced from the host global at all).
    if (PROCESS_GLOBAL_SUFFIXES.has(suffix) && !populated(sc) && !populated(pc)) {
      sc = process.env[`SUPERCLAW_${suffix}`];
      pc = process.env[`PAPERCLIP_${suffix}`];
    }
    const val = populated(sc) ? sc : populated(pc) ? pc : undefined;
    if (val !== undefined) {
      env[`SUPERCLAW_${suffix}`] = val;
      env[`PAPERCLIP_${suffix}`] = val;
    }
  }
}
...
export function sanitizeInheritedPaperclipEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    ...
    // Keep the runtime API candidate list too — the chat manager skills resolve a
    // reachable loopback base from it (mirrored to SUPERCLAW_RUNTIME_API_CANDIDATES_JSON).
...
export function buildRuntimeMountedSkillSnapshot(
  ...
  const {
    ...
    desiredSkills,
    configuredDetail,
    missingDetail = "Cannot find this skill in the local runtime skills directory.",
    mode = "ephemeral",
    externalInstalled,
    externalLocationLabel,
    externalDetail = "Installed outside SuperClaw management.",
    skillsHome,
  } = options;
        ...
        ? configured
          ...
          : resolveSkillDetail(
              options.unsupportedDetail
                ?? "Desired state is stored in SuperClaw only; this adapter cannot apply skills at runtime.",
              available,
            )
        : null,
      ...buildManagedSkillOrigin(),
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the runtime skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    entries.push({
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Cannot find this skill in the local runtime skills directory.",
      origin: "external_unknown",
      originLabel: "External or unavailable",
      readOnly: false,
    });
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for runtime skill materialization lock at ${lockDir}`);
      }
...
export async function materializePaperclipSkillCopy(
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("Runtime skills must be directories.");
  }
