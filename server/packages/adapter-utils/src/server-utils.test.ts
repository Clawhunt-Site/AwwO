...
describe("adapter skill snapshots", () => {
  ...
  it("can surface read-only external skills for runtime-mounted adapters", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      externalDetail: "Installed outside SuperClaw management in the Claude skills home.",
    });
  ...
  it("reports persistent adapter installed, stale, external, and missing states", () => {
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      state: "external",
      managed: false,
      detail: "Installed outside SuperClaw management.",
    }));
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      key: "missing-skill",
      state: "missing",
...
describe("renderPaperclipWakePrompt", () => {
  ...
  it("adds the execution contract to scoped wake prompts", () => {
    ...
    expect(prompt).toContain("## Wake Payload");
  ...
  it("chat:true renders a LEAN transcript only — no Paperclip wake/issue/contract scaffolding", () => {
    const payload = {
      chat: true,
      reason: "issue_commented",
      // chat issues carry the same shape, but none of it must surface in chat mode.
      issue: { id: "issue-1", identifier: "PAP-1", title: "my chat", status: "in_progress", workMode: "standard" },
      commentIds: ["c1", "c2"],
      latestCommentId: "c2",
      commentWindow: { requestedCount: 2, includedCount: 2, missingCount: 0 },
      comments: [
        { id: "c1", body: "what is 2+2?", author: { type: "user", id: "board-user-1" }, createdAt: "2026-06-29T00:00:00.000Z" },
        { id: "c2", body: "It is 4.", author: { type: "agent", id: "agent-1" }, createdAt: "2026-06-29T00:00:01.000Z" },
      ],
      fallbackFetchNeeded: false,
    };

    const prompt = renderPaperclipWakePrompt(payload);
    // The conversation (continuity carrier) is preserved, role-labelled.
    expect(prompt).toContain("Conversation so far");
    expect(prompt).toContain("User: what is 2+2?");
    expect(prompt).toContain("Assistant: It is 4.");
    // NONE of the Paperclip issue/company scaffolding leaks into a chat prompt.
    ...
    expect(prompt).not.toContain("child issues");
  });

  it("chat omitted (company issue) keeps the FULL Paperclip wake stack unchanged", () => {
    const payload = {
      reason: "issue_commented",
      issue: { id: "issue-1", identifier: "PAP-1", title: "deliver feature", status: "in_progress", workMode: "standard" },
      commentIds: ["c1"],
      latestCommentId: "c1",
      commentWindow: { requestedCount: 1, includedCount: 1, missingCount: 0 },
      comments: [{ id: "c1", body: "ship it", author: { type: "user", id: "board-user-1" }, createdAt: "2026-06-29T00:00:00.000Z" }],
      fallbackFetchNeeded: false,
    };
    const prompt = renderPaperclipWakePrompt(payload);
    expect(prompt).toContain("## Paperclip Wake Payload");
    expect(prompt).toContain("Execution contract: take concrete action in this heartbeat");
    expect(prompt).toContain("- issue: PAP-1 deliver feature");
  });
});
