...
import { ensureRelayIsolatedClaudeConfigDir, prepareClaudeConfigSeed } from "./claude-config.js";
...
describe("ensureRelayIsolatedClaudeConfigDir long-term memory", () => {
  const cleanupDirs: string[] = [];
  ...
  function relayEnv(root: string): NodeJS.ProcessEnv {
    return {
      HOME: root,
      PAPERCLIP_HOME: path.join(root, "paperclip-home"),
      PAPERCLIP_INSTANCE_ID: "test-instance",
  ...
  it("seeds a persistent memory.md and embeds it into the loaded CLAUDE.md (persona kept, no credentials)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "relay-isolated-mem-"));
    cleanupDirs.push(root);
    const dir = await ensureRelayIsolatedClaudeConfigDir(relayEnv(root));
    const memoryPath = path.join(dir, "memory.md");
    await expect(fs.readFile(memoryPath, "utf8")).resolves.toContain("long-term memory");
    const claudeMd = await fs.readFile(path.join(dir, "CLAUDE.md"), "utf8");
    expect(claudeMd).toContain("You are **SuperClaw**"); // persona preserved
    expect(claudeMd).toContain("## Long-term memory"); // memory section present
    expect(claudeMd).toContain("<memory>"); // memory embedded into loaded context
    expect(claudeMd).toContain(memoryPath); // agent told where to append
    // Credential-free isolation must remain (relay key is the only auth).
    await expect(fs.access(path.join(dir, ".credentials.json"))).rejects.toMatchObject({ code: "ENOENT" });
  });

  it("preserves accumulated memory across runs and re-embeds it into CLAUDE.md", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "relay-isolated-mem-"));
    cleanupDirs.push(root);
    const env = relayEnv(root);
    const dir = await ensureRelayIsolatedClaudeConfigDir(env);
    const memoryPath = path.join(dir, "memory.md");
    // Simulate the agent appending a durable memory entry between sessions.
    await fs.appendFile(memoryPath, "\n- 2026-07-02 user prefers concise replies\n", "utf8");
    // A later session must NOT wipe the memory file, and must re-embed it.
    const dir2 = await ensureRelayIsolatedClaudeConfigDir(env);
    expect(dir2).toBe(dir);
    await expect(fs.readFile(memoryPath, "utf8")).resolves.toContain("user prefers concise replies");
    await expect(fs.readFile(path.join(dir, "CLAUDE.md"), "utf8")).resolves.toContain(
