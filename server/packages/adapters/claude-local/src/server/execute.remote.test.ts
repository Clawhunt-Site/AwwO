...
// ---- Execution-layer coverage for kernel model-default materialization ----
// Locks the WIRED behavior end-to-end (not just the resolveConfiguredModel unit):
// a blank config.model must reach the CLI as `--model claude-opus-4-8`, while
// Bedrock auth is left untouched. Reuses the mocked runChildProcess to read argv.
async function runClaudeArgs(
  cleanupDirs: string[],
  configExtra: Record<string, unknown>,
): Promise<string[]> {
  const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-claude-model-"));
  cleanupDirs.push(rootDir);
  const workspaceDir = path.join(rootDir, "workspace");
  const instructionsPath = path.join(rootDir, "instructions.md");
  await mkdir(workspaceDir, { recursive: true });
  await writeFile(instructionsPath, "Use the workspace.\n", "utf8");
  await execute({
    runId: "run-model",
    agent: {
      id: "agent-1",
    ...
    config: { command: "claude", instructionsFilePath: instructionsPath, ...configExtra },
    context: {
      paperclipWorkspace: {
        cwd: workspaceDir,
        source: "project_primary",
        ...
        repoUrl: "https://github.com/paperclipai/paperclip.git",
        repoRef: "main",
        branchName: "feature/model-default",
        worktreePath: workspaceDir,
      },
    },
    executionTransport: {
...
describe("claude model default materialization (execution layer)", () => {
  const cleanupDirs: string[] = [];
  ...
  it("passes --model claude-opus-4-8 when config.model is blank (non-Bedrock)", async () => {
    const args = await runClaudeArgs(cleanupDirs, {});
    const i = args.indexOf("--model");
    expect(i).toBeGreaterThanOrEqual(0);
    expect(args[i + 1]).toBe("claude-opus-4-8");
  });

  it("does NOT pass --model for a blank model under Bedrock auth", async () => {
    const args = await runClaudeArgs(cleanupDirs, { env: { CLAUDE_CODE_USE_BEDROCK: "1" } });
    expect(args).not.toContain("--model");
  });

  it("passes an explicit Bedrock-native id through under Bedrock auth", async () => {
    const bedrockId = "us.anthropic.claude-opus-4-8-v1";
    const args = await runClaudeArgs(cleanupDirs, {
      model: bedrockId,
    ...
    const i = args.indexOf("--model");
    expect(i).toBeGreaterThanOrEqual(0);
    expect(args[i + 1]).toBe(bedrockId);
  });
});
