...
/**
 * Resolve the concrete model to run from a run config's FINAL merged `model`.
 *
 * `configuredModel` here is already the merged result of base agent config +
 * modelProfile + issue/chat override. When it is blank, the kernel runs the
 * adapter's contract baked default (`models[0]` — the SAME id `/api/chat-runtime`
 * exposes to the composer as `default_model`) instead of omitting `--model` and
 * letting the local `claude` CLI fall back to whatever `~/.claude/settings.json`
 * configures (which may be a smaller model like haiku). This keeps "the default
 * the UI shows" == "the model actually executed" across chat/company/run, and
 * makes it independent of per-machine local CLI config. A non-blank value (an
 * explicit selection OR a modelProfile like "cheap") is returned unchanged, so
 * this NEVER overrides a real choice. Callers under Bedrock auth must skip this
 * (Anthropic ids are invalid on Bedrock; the CLI uses its Bedrock model).
 */
export function resolveConfiguredModel(configuredModel: string): string {
  const explicit = configuredModel.trim();
  if (explicit) return explicit;
  return (DIRECT_MODELS[0]?.id ?? "").trim();
}
