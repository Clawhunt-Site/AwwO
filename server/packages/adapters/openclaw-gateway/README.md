...
5. stream `event agent` frames into SuperClaw logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (SuperClaw `runId`)
...
Like every SuperClaw adapter, this one must treat the local execution-workspace
