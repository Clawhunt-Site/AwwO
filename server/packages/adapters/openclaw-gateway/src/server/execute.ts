...
function buildPaperclipEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  ...
  const paperclipEnv: Record<string, string> = {
    ...
    PAPERCLIP_RUN_ID: ctx.runId, SUPERCLAW_RUN_ID: ctx.runId,
  };

  if (paperclipApiUrlOverride) {
    paperclipEnv.SUPERCLAW_API_URL = paperclipEnv.PAPERCLIP_API_URL = paperclipApiUrlOverride;
  }
...
function buildWakeText(
  ...
  const apiBaseHint = paperclipEnv.PAPERCLIP_API_URL ?? "<set SUPERCLAW_API_URL (or legacy PAPERCLIP_API_URL)>";

  const lines = [
    "Wake event for a cloud adapter.",
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer ${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY} on every API call.",
    "- Use X-SuperClaw-Run-Id: ${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID} on every mutating API call.",
