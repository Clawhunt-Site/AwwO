...
import { firstGeminiErrorLine } from "./utils.js";
...
function renderApiAccessNote(env: Record<string, string>): string {
  if ((!hasNonEmptyEnvValue(env, "SUPERCLAW_API_URL") && !hasNonEmptyEnvValue(env, "PAPERCLIP_API_URL")) || (!hasNonEmptyEnvValue(env, "SUPERCLAW_API_KEY") && !hasNonEmptyEnvValue(env, "PAPERCLIP_API_KEY"))) return "";
  return [
    "API access note:",
    "Use run_shell_command with curl to make API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer \${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY}\\" \\"\${SUPERCLAW_API_URL:-$PAPERCLIP_API_URL}/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer \${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY}\\" -H 'Content-Type: application/json' -H \\"X-SuperClaw-Run-Id: \${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID}\\" -d '{...}' \\"\${SUPERCLAW_API_URL:-$PAPERCLIP_API_URL}/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const toResult = (
    ...
    const stderrLine = firstGeminiErrorLine(attempt.proc.stderr);
