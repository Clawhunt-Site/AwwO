...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
  'You are "{{agent.name}}", an AI agent employee in a SuperClaw-managed company.',
  "",
  "Runtime identity:",
  "- Agent ID: {{agent.id}}",
  ...
  "",
  "API guidance:",
  "- Use `curl` from the terminal for API calls; browser/web extraction tools may not reach localhost.",
  "- Use `${SUPERCLAW_API_URL:-$PAPERCLIP_API_URL}`, `${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY}`, and `${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID}`; do not hard-code local ports or copy secrets into comments.",
  "- Displayed command logs may redact secrets; rely on environment variables instead of printed token values.",
  "- Include `-H \"Authorization: Bearer ${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY}\"` on API requests.",
  "- Include `-H \"X-SuperClaw-Run-Id: ${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID}\"` on mutating issue requests.",
  ...
  "    -H \"Authorization: Bearer ${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY}\" \\",
  "    -H \"X-SuperClaw-Run-Id: ${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID}\" \\",
...
export function buildPrompt(
  ...
  let paperclipApiUrl =
    cfgString(config.superclawApiUrl) || cfgString(config.paperclipApiUrl) ||
...
export async function execute(
  ...
  if (ctx.runId) env.SUPERCLAW_RUN_ID = env.PAPERCLIP_RUN_ID = ctx.runId;

  // BUG FIX: Inject authToken as PAPERCLIP_API_KEY (matches adapter-claude-local behavior)
  if ((ctx as any).authToken) env.SUPERCLAW_API_KEY = env.PAPERCLIP_API_KEY = (ctx as any).authToken;
