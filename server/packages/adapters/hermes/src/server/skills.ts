...
async function buildSkillEntry(
  ...
  return {
    ...
    readOnly: true, // Hermes manages its own skills — SuperClaw can't toggle them
...
async function buildHermesSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(
      `Desired skill "${desiredSkill}" is not available in SuperClaw or Hermes skills.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Cannot find this skill in SuperClaw or ~/.hermes/skills/.",
    });
  }
