...
test("renders resume deltas instead of full scoped-wake boilerplate when continuing a session", () => {
  ...
  expect(prompt).toContain("## Resume Delta");
  expect(prompt).toContain("You are resuming an existing session.");
...
test("renders safe SuperClaw API examples (bash fallback to legacy PAPERCLIP_*) from environment variables with multiline update preservation", () => {
  const prompt = buildPrompt(baseContext(), {
    paperclipApiUrl: "http://paperclip.local/api",
  });

  expect(prompt).toContain("Use `${SUPERCLAW_API_URL:-$PAPERCLIP_API_URL}`");
  expect(prompt).toContain("Displayed command logs may redact secrets");
  expect(prompt).toContain('-H "Authorization: Bearer ${SUPERCLAW_API_KEY:-$PAPERCLIP_API_KEY}"');
  expect(prompt).toContain('-H "X-SuperClaw-Run-Id: ${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID}"');
...
test("preserves custom prompt templates while exposing runtime and wake variables", () => {
  ...
  expect(prompt).toContain("wakePrompt=## Wake Payload");
  expect(prompt).toContain("task=Task context:");
