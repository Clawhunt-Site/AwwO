...
function buildInput(ctx: AdapterExecutionContext, paperclipApiUrl: string | null): string {
  ...
  const lines = [
    `You are ${ctx.agent.name}, an AI agent employee in a SuperClaw-managed company.`,
    "",
    "Runtime identity:",
    `- Agent ID: ${ctx.agent.id}`,
    `- Company ID: ${ctx.agent.companyId}`,
    `- Run ID: ${ctx.runId}`,
    ...(paperclipApiUrl ? [`- Runtime API URL: ${paperclipApiUrl}`] : []),
    ...
    "- Use X-SuperClaw-Run-Id on mutating API requests when an API key is available.",
    "",
    wakePrompt,
...
function buildRunBody(ctx: AdapterExecutionContext, sessionKey: string | null): Record<string, unknown> {
  const paperclipApiUrl = (nonEmpty(ctx.config.superclawApiUrl) ?? nonEmpty(ctx.config.paperclipApiUrl));
  ...
  const instructions =
    ...
    "Follow the wake instructions exactly. Do not expose secrets in logs, comments, or final output.";
  return {
    ...payloadTemplate,
    input,
    instructions,
