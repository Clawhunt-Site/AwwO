/**
 * Hermes Agent adapter for SuperClaw.
 *
 * Runs Hermes Agent (https://github.com/NousResearch/hermes-agent)
 * as a managed employee in a SuperClaw company. Hermes Agent is a
 ...
 *
 * Hermes supports any model via any provider. The SuperClaw UI should
...
/**
 * Documentation shown in the SuperClaw UI when configuring a Hermes agent.
 */
...
This adapter package also ships a Hermes-facing SuperClaw task bridge skill:
\`paperclip-task-bridge\`. Use it when a user starts in Hermes and asks Hermes
to create, comment on, update, or list SuperClaw tasks.
...
- \`PAPERCLIP_BRIDGE_API_KEY\` - SuperClaw agent API key created with \`scope.kind = "task_bridge"\`
...
- \`hermes_local\` means SuperClaw shells out to local \`hermes chat\`.
- \`hermes_gateway\` means the workspace wakes remote Hermes through Hermes's API server.
- \`paperclip-task-bridge\` means Hermes calls SuperClaw's REST API to manage tasks.

Create task bridge keys with a parent issue or project boundary. Do not expose
normal claimed SuperClaw agent API keys to internet-facing Hermes chat/webhook
...
/**
 * External adapter plugin entrypoint expected by SuperClaw's adapter manager.
 */
