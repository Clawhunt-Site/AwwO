# SuperClaw Adapters for Hermes Agent

A [SuperClaw](https://paperclip.ing) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a SuperClaw company.
...
Choose `hermes_local` when SuperClaw and Hermes run on the same trusted host
and SuperClaw should start `hermes chat` for each heartbeat. Choose
...
- **Skills integration** — Scans both SuperClaw-managed and Hermes-native skills (`~/.hermes/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Hermes stdout is parsed into typed `TranscriptEntry` objects so SuperClaw renders proper tool cards with status icons and expand/collapse
...
This package ships with SuperClaw core as the built-in `hermes_local` and
`hermes_gateway` adapters. No Adapter manager installation is required for
normal SuperClaw use.
...
SuperClaw. The external package can override either built-in Hermes adapter
while it is enabled:
...
SuperClaw core imports these same package entrypoints for built-in adapter
registration.

### 2. Create a local Hermes agent in SuperClaw

In the SuperClaw UI or via API, create an agent with adapter type `hermes_local`:
...
This mode shells out to the local `hermes` CLI. SuperClaw injects runtime
environment variables and captures stdout/stderr from the child process.

### 3. Create a Hermes gateway agent in SuperClaw
...
for existing plugin installs. New installs and built-in SuperClaw registrations
should use `@paperclipai/hermes-paperclip-adapter`; the adapter type remains
`hermes_gateway`.

### Runtime API guidance

Hermes receives SuperClaw runtime identity through environment variables:
...
`Authorization: Bearer $PAPERCLIP_API_KEY` on Runtime API requests and
...
### Hermes-originated SuperClaw tasks

The package includes a Hermes skill/helper for the reverse direction: a user
starts in Hermes and asks Hermes to create or update SuperClaw work. This is not
the same as SuperClaw waking Hermes through `hermes_local` or `hermes_gateway`.

Configure SuperClaw access in Hermes env/profile secrets, not prompt text:
...
Create issues in SuperClaw and assign them to your Hermes agent. On each heartbeat, Hermes will:

1. Receive the task instructions
2. Use its full tool suite to complete the work
3. Report results back to SuperClaw
...
| `paperclipApiUrl` | string | `http://127.0.0.1:3100/api` | Runtime API base URL |
...
| `{{paperclipApiUrl}}` | Runtime API base URL |
...
6. **Reports** results back to SuperClaw with cost, usage, and session state
...
- **SuperClaw-managed skills** — bundled with the adapter, togglable from the UI
...
SuperClaw UI can display both managed and native skills in one view.
...
- [SuperClaw](https://github.com/paperclipai/paperclip) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [SuperClaw Docs](https://paperclip.ing/docs) — SuperClaw documentation
