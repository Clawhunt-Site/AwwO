...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[paperclip] Using SuperClaw-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function prepareClaudeSkillRuntime(input: {
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "SuperClaw has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} runtime skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}
...
async function writeAgentWrapper(input: {
  ...
  const script = [
    ...
    "  mkdir -p \"$stderr_dir\"",
    "  exec 2> >(tee -a \"$stderr_dir/${SUPERCLAW_RUN_ID:-$PAPERCLIP_RUN_ID}.log\" >&2)",
    "fi",
...
async function buildRuntime(input: {
  ...
  const env: Record<string, string> = { ...buildPaperclipEnv(agent), PAPERCLIP_RUN_ID: runId, SUPERCLAW_RUN_ID: runId };
  const wakeTaskId =
  ...
  if (!hasExplicitApiKey && authToken) env.SUPERCLAW_API_KEY = env.PAPERCLIP_API_KEY = authToken;
  ...
  if (acpxAgent === "claude") {
    ...
    skillCommandNotes.push(
      `Wrote SuperClaw-managed Claude settings to ${paperclipClaudeSettings.filePath} (defaultMode=${paperclipClaudeSettings.defaultMode}${
    ...
    if (desired.length > 0) {
      skillCommandNotes.push("Selected runtime skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in SuperClaw with ${prepared.mode} session mode.`,
