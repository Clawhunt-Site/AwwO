...
function unsupportedDetail(): string {
  return "Desired state is stored in SuperClaw only; custom ACP commands need an explicit skill integration contract before runtime sync is available.";
}
...
async function buildAcpxSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const warnings: string[] = supported
    ? []
    : [
        "Custom ACP commands do not expose a runtime skill integration contract yet; selected skills are tracked only.",
      ];
