...
Use when:
- You want SuperClaw to run Cursor Agent CLI locally as the agent runtime
...
- SuperClaw auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- SuperClaw auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
