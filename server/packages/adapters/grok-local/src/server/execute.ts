...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    "API access note:",
    "Use shell commands with curl to make API requests when needed.",
    "Include X-SuperClaw-Run-Id on mutating requests.",
    "",
    "",
  ].join("\n");
}
  ...
  try {
    ...
    const commandNotes = (() => {
      ...
      if (stagedAssets.stagedSkillsCount > 0) {
        notes.push(`Staged ${stagedAssets.stagedSkillsCount} runtime skill(s) into .claude/skills for native Grok discovery.`);
      }
      return notes;
    })();
