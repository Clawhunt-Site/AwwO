...
Use when:
- You want SuperClaw to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across SuperClaw heartbeats
- You want SuperClaw to keep task state while Cursor handles remote code execution
...
Notes:
- SuperClaw reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each SuperClaw heartbeat maps to a Cursor run on that durable agent.
- SuperClaw injects PAPERCLIP_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- SuperClaw remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
