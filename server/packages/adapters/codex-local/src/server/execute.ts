...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Wire the plugin MCP bridge into config.toml for LOCAL execution only, when
  // the host planted a complete server-derived run context. The bridge env (api
  // url + the per-run JWT) is sourced the same way the run env is built below;
  // prepareCodexRuntimeConfig writes it under a managed `[mcp_servers.superclaw]`
  // block (excising inherited mcp_servers) and cleanup() removes the JWT after.
  const pluginBridgeSpec = (() => {
    if (executionTargetIsRemote) return null;
    const runContext = readPluginBridgeRunContext(config as Record<string, unknown>);
    if (!runContext) return null;
    const bridgeEnv: Record<string, string> = {
      ...buildPaperclipEnv(agent),
      ...(authToken ? { SUPERCLAW_API_KEY: authToken, PAPERCLIP_API_KEY: authToken } : {}),
    };
    return buildPluginBridgeMcpServerSpec({
      runContext,
      agentEnv: bridgeEnv,
      agentId: agent.id,
  ...
  try {
    ...
    // Local execution always runs in a runtime-resolved workspace (a project
    // workspace, a prior session workspace, or the managed per-agent default) —
    // all trusted by construction. Codex's git-repo trust gate would otherwise
    // reject a non-git managed workspace (e.g. a chat session's default home),
    // so local execution gets the same --skip-git-repo-check bypass as sandbox.
    // SSH-remote execution is unaffected (it keeps the check).
    const executionTargetIsLocal = !executionTargetIsRemote;
    ...
    const repoAgentsNote =
      "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; SuperClaw does not currently suppress that discovery.";
    ...
    if (executionTargetIsSandbox || executionTargetIsLocal) {
      commandNotes.push(
        "Added --skip-git-repo-check: managed local and sandbox workspaces are trusted by construction, so Codex's git-repo trust gate (which would otherwise reject a non-git managed workspace such as a chat session home) is bypassed. SSH-remote execution keeps the check.",
      );
    }
    ...
    const runAttempt = async (resumeSessionId: string | null) => {
      const execArgs = buildCodexExecArgs(
        ...
        {
          resumeSessionId,
          skipGitRepoCheck: executionTargetIsSandbox || executionTargetIsLocal,
        },
      );
