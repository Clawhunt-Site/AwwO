...
export function initAccessibility(root = document) {
  ...
  // Native modality makes the page inert, but Tab can still reach browser UI.
  // Delegate so app-owned showModal() dialogs share the same boundary handling.
  root.addEventListener('keydown', (event) => {
    ...
    const tabbable = [...dialog.querySelectorAll('a[href], button, input, select, textarea, summary, [tabindex]')]
      .filter(el => el.tabIndex >= 0 && !el.matches(':disabled') && !el.closest('[hidden], [inert]')
        && el.getClientRects().length && !['hidden', 'collapse'].includes(doc.defaultView.getComputedStyle(el).visibility))
    ...
    if (!first) {
      ...
      event.preventDefault();
      last.focus();
    } else if (!event.shiftKey && (doc.activeElement === last || doc.activeElement === dialog)) {
      event.preventDefault();
      first.focus();
    }
  ...
  root.addEventListener('click', (event) => {
    ...
    if (trigger.hasAttribute('data-dialog-open')) {
      ...
      dialog.showModal(); // Native inert background and Escape; Tab boundaries handled above.
      return;
    }
