...
> **本分支（`dev/server-refactor`）是 Node server 再平台化的长生命周期集成主干，按 `docs/git-workflow-constitution.md` §十一「本地循环」工作流运行。** 下方所有「开发 / 测试 / 验收 / 提交 / PR」流程**以本节为准**：本节与下方任何**工作流类**铁律冲突时，**本分支上本节优先**。**产品治理类铁律不受影响、照常严格遵守**——runtime model/effort 联动选择器、Pay-Switch / fail-closed 治理、旧 Python 渠道按需彻底清除（见下方铁律）等。
>
> 〔正常 main 推进流程的**完整版** `CLAUDE.md` / `AGENTS.md` 已备份在 `main-md/`；末次合回 `main` 时用它恢复正常流程。〕

**⚠️ `dev/server-refactor` 是 merge-only 集成主干——绝不在它上面直接开发 / 直接 checkout 改业务文件 / 直接提交功能代码。** 所有功能 / 代码 / 重构 / 测试一律在独立 worktree 的 `feat/<slug>` 子分支上做，再 merge 回主干；主干只接收【子功能 / 回滚分支（`feat/<slug>` / `revert/<slug>`）merge + `origin/main` 吸收 + 本工作流文档维护】三类写入（§十一 第 1 条）。

**本分支开发循环（每次改动）：**
1. 子功能从 `dev/server-refactor` 切独立 worktree + `feat/<slug>`（一会话一树一分支，worktree 隔离铁律照旧）。
2. **只测相关**：跑改动那块「相关 + 可能相关」的 lint / test / build / smoke，并补齐相关 test case。**开发期不跑全量 `ci.yml`**（"不跑全量" ≠ "不跑测试"）。
3. **验收降为 Codex 单顾问**：只跑 `codex-cli-advisor --model gpt-5.5` 对抗式验收，**本分支不强制 Gemini**——此条**覆盖**下方「开发完成后必须经 Codex + Gemini 验收」铁律的双顾问要求。
4. **合回主干按 §十一 第 3 条 5 步**：先更新主干到最新 → `git merge --no-commit --no-ff` 解决所有冲突 → 在解冲突结果上跑相关测试全绿 → Codex 无阻断 → 才 `commit`（失败先 `git merge --abort` 回第 1 步重走）。
5. **本地原子提交即可，开发期不强制推远端**；`origin/dev/server-refactor` 仅作可选备份（**任何推远端、含备份 push，仍属远端状态变更，需业主批准**，见 `AGENTS.md` Approval Gates）。

**留到 PR 时由 Leon 统一做（满血重门，不豁免）：** 推远端 / 全量 `ci.yml` / 吸收最新 `origin/main` / 分段可 review PR（vendor 先单独落地 + 完整性门）/ **Codex + Gemini 双顾问** / §五 merge 门禁 / 合回 `main`。下方「远端 CI 已暂停—本地全量是推 PR 前唯一门」「CI 工程纪律」等铁律的**全量 / 裁判要求，在本分支只在此 PR 时点触发**，开发期不触发。

**命名去 Paperclip（§十一 第 8 条）：** 我们**新造**的命名（标识符 / 产品 / 模块 / 用户文案 / 文档叙述 / commit / 分支）**不得用任何形式的 "paperclip"**（大小写 / 变体 / `paperclipai` 均算），一律用 `SuperClaw` / 中性词。提交前 `git diff | grep -iE '^\+.*paperclip'`，**每处新增命中须落在例外内、否则改名**。**例外（事实性 / 被迫，非品牌传播）**：① vendored `server/` 上游树逐字保留（tree-equality）；② vendor 溯源（`paperclipai/paperclip` 归因）；③ **对接上游既有标识符的必需引用**（如 `paperclipRuntimeServices`、`config.paperclipRuntimeSkills`）；④ 本规则 / 治理文档对该词的必要提及。

**红线不豁免：** 永不在本地 `main` commit、永不 `git push origin main`、**主干 merge-only（绝不在 `dev/server-refactor` 直接开发 / 提交功能代码）**、worktree 隔离、禁 GitHub 分支保护、共享分支永不 force —— 全部照旧。
...
> **一句话总结：永远不要为本仓库（`team-to-be-found/SuperClaw`）的 `main` 开启 GitHub 分支保护（branch protection / required status checks / require branches up to date / required reviews 等）。漂移问题一律靠开发纪律解决，不引入平台级强制门。**
...
> **⚠️ `dev/server-refactor` 分支例外（见顶部「本分支工作模式」）：本分支开发期验收**降为 Codex（gpt-5.5）单顾问**，不强制 Gemini；双顾问要求在**末次由 Leon 推 PR 合 main 时**才完整恢复。**
...
> **⚠️ `dev/server-refactor` 分支例外（见顶部「本分支工作模式」）：本分支开发期为纯本地循环、只测相关，**不逐改跑全量 `ci.yml`**；本条"本地全量"门在本分支只在**末次由 Leon 推 PR 时**触发。**
...
- 它是**私有仓**，每次真跑都吃组织 `team-to-be-found` 那个**多仓共享的 Free 2000 分钟/月**池子；本月已被高频 CI 跑爆，spending limit 卡在 `$0` → 新 job 在启动前就被计费门拦下（"3 秒 fail"、无日志、`billable=0`）。见 [[ci-actions-minute-budget]]。
...
## 铁律：vendored 上游（`server/` = Paperclip）维护 —— 最小化并审计本地改动，主干 merge-only 永不重写历史

> **一句话总结：`server/` 是 bare-prefix vendored 上游 Paperclip，把它当可信底座维护：①本地改动优先级 宿主覆盖层(H) > 上游扩展点 add-only(E) > 上游核心就地改(C，最后手段 + 审计 + 语义评审)；②升级官方 = 在隔离 worktree 用「三方合并」把"新上游纯基线"并入"我们的 delta"，再经 merge 进集成主干，主干永不 rebase/force-push；③本地改动漂移由 content-addressed 账本 + 一行命令当场审计，不靠手维护账本；④交付门复用 `docs/git-workflow-constitution.md` §十一。这条由"vendored fork 持续跟踪上游"的业界最佳实践沉淀，经 Codex(GPT-5.5) + Gemini 3.1 Pro 五轮对抗式验收通过（阻断 10→0）。**

治理"如何在持续同步 Paperclip 官方更新的同时保住我们自己的改动"，新增 / 修改 `server/` 时都要核对。**完整 re-vendor SOP、override 账本格式、bootstrap 前置见 `docs/git-workflow-constitution.md` §十二**（本节是其必守摘要）。

### 1. 改动分层（落点优先级 H>E>C；G 仅治理元数据）
- **H 宿主 / 覆盖层**（`apps/web`，**在 `server/` 之外**）：主题 / 品牌 / 隐藏 / 布局 / host locale → 首选，零上游改动。
- **E 上游扩展点**：add-only 可，但登记须**引用上游证据**（exported registry API / 文档化扩展点 / 上游契约）；**无证据降 C**，不得自封 E。
- **C 上游核心**（`server/server` 的 routes/services/trust/app.ts/registry、`server/ui` 既有组件 / 页面、**及任何位于 `server/` 内的运行时 / 产品行为代码 —— 含我们新增的 `i18n/localized.ts`、`EmbeddedHostContext.tsx` 之类**）：任何改动（**含新增文件**）= liability，须登记 override + 语义评审；触治理 / fail-closed 须双顾问。**add-only 不等于合规。**
- **G 治理元数据**（**仅** `VENDOR_STATE.json` / `vendor-overrides.allow` / 生成的 `VENDOR_PATCHES.md`）：我们的治理产物、不含运行时逻辑 → 豁免"禁改上游"、不参与上游三方合并。**G 绝不含任何运行时代码**（运行时代码一律 C，除非移出 `server/` 才算 H）。

### 2. re-vendor 核心机制（详 §十二）
- 纯上游基线在 `vendor/paperclip` 分支（bootstrap 后**只接受 fast-forward 纯导入提交**，代码在 `server/` 前缀下；绝不 orphan/recreate/reset/force —— orphan 会破坏 merge-base 门）。
- 升级 = 隔离 worktree 内 `git merge vendor/paperclip` **三方合并**（base=旧上游、ours=我们 delta、theirs=新上游），**绝不 `rebase --onto`/force-push 共享主干**（普通 rebase 会重写共享历史、展平 merge commit，违 [[no-github-branch-protection]] 的"绝不 force-push"）。
- 两道 (A) 硬命令门（当场可跑）：① `git merge-base HEAD vendor/paperclip` == `VENDOR_STATE.currentBaselineCommit`（保证三方基线正确）；② `git rev-parse <upstreamSha>^{tree}` == `git rev-parse vendor/paperclip:server`（保证纯导入内容真是上游 root tree，非仅血缘）。
- C 层 override content-addressed 防漂移：`patch_digest = git diff --binary --no-color --no-ext-diff --no-textconv <baseline_sha> -- <path> | shasum -a 256`；digest 变即重审；PR 贴真实 `git diff` 供肉眼确认语义（**人不靠肉眼算 hash**）。

### 3. 红线
- `vendor/paperclip` 只 fast-forward；主干 merge-only，**绝不 rebase/force-push 共享主干**（承接 [[no-github-branch-protection]]、[[parallel-worktree-isolation-hazard]]）。
- **绝不删除仍存在于新基线的上游文件**；上游自身删 / 改名照单吸收。
- **G 绝不含运行时代码**；`server/` 内运行时 / 行为代码一律 C。
- 触后端治理 / fail-closed 的 C 层改动**绝不**无双顾问 + 无 override（含 digest）就进。
- "脚本失败"类自动门（待建）未落地前以 (A) 一行命令 + 贴 diff 兜底，**不冒充已生效**。

### 4. 生效前置 + 现状
- 本铁律的硬门依赖初始事实源，**生效前必须先做一次 bootstrap 提交**（建 `vendor/paperclip`、写 `VENDOR_STATE.json` 钉死 `f3f50e2`/`90a2efe4`、把现存 68 新增 + 191 就地改逐一 E/C/G 分类并登记 grandfathered override），详 §十二 §0.1。
- 现状（2026-06-29）：`server/` 相对纯上游基线已分叉 **259 文件 / +18152 / −2030**（68 新增，含 42 在 `server/server`；191 就地改，186 个 `.ts/.tsx` 多为 i18n 就地替换）。本铁律**冻结增量、存量渐迁**，不要求立即清零。
