...
type Runtime = { engine: string; available: boolean; configured: boolean; reason?: string; models: Array<{ id: string; name?: string; label?: string; provider?: string }> };
export function RuntimeSettings({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    api<Runtime>('/runtime', { signal: controller.signal }).then(value => { if (!controller.signal.aborted) setRuntime(value); }).catch(cause => { if (!controller.signal.aborted) setError(cause); });
  ...
  return <div className="saas-dialog-backdrop"><section role="dialog" aria-modal="true" aria-labelledby="saas-runtime-title" className="saas-card"><h2 id="saas-runtime-title">{t('工作区运行设置', 'Workspace runtime settings')}</h2>
    {error ? <p role="alert" className="saas-error">{saasErrorMessage(error, locale)}</p> : !runtime ? <p role="status">{t('正在读取运行服务…', 'Loading runtime…')}</p> : <><dl className="saas-runtime-values"><dt>{t('执行引擎', 'Engine')}</dt><dd>{runtime.engine}</dd><dt>{t('服务状态', 'Service status')}</dt><dd>{runtime.available ? t('配置就绪', 'Configured') : t('尚未就绪', 'Unavailable')}</dd><dt>{t('可选模型', 'Available models')}</dt><dd>{runtime.models.map(model => model.label || model.name || model.id).join(', ') || t('暂无', 'None')}</dd></dl>
