...
describe('scoped execution', () => {
  ...
  it.each([
    ...
    // Keep the cause on the scoped dependent without repainting an unexecuted upstream.
    expect(statuses.A).toBeUndefined();
    expect(statuses.B).toMatchObject({ state: 'blocked', detail: expect.stringContaining('节点-A') });
