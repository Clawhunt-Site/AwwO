...
describe('ssh-keys', () => {
  ...
  it('generate produces a valid OpenSSH ed25519 public key with a SHA256 fingerprint', async () => {
    ...
    // The private key file exists with mode 0600 on POSIX. Windows does not
    // project ACLs into POSIX permission bits, so Node reports 0666 there even
    // when writeFile was asked to create the key with 0600.
