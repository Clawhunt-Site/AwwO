...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|clawwork] [--self|--extensions|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Short forms:
  ...
  ${APP_NAME} update clawwork       Update ${APP_NAME} only (self also works)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "pi" || source === "clawwork";
			if (sourceIsSelf) {
