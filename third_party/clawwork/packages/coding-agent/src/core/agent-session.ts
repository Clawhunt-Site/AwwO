...
import { assertGovernanceBarrier } from "./governance-barrier.ts";
...
export class AgentSession {
	...
	private _buildRuntime(options: {
		...
		// SuperClaw forge-proof governance barrier: when a mandatory governance
		// extension is required (SUPERCLAW_REQUIRE_GOVERNANCE), refuse to finish
		// building the runtime — and therefore to run any prompt / model / tool —
		// unless that exact extension loaded and registered its tool_call gate. An
		// agent cannot forge an in-process handler registration, so this holds where
		// a file/env handshake would not. No-op when the env var is unset.
		assertGovernanceBarrier(extensionsResult.extensions);
