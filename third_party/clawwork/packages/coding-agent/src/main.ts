...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode =
		args.includes("--offline") || isTruthyEnvFlag(process.env.CLAWWORK_OFFLINE ?? process.env.PI_OFFLINE);
	if (offlineMode) {
		process.env.CLAWWORK_OFFLINE = "1";
		process.env.PI_OFFLINE = "1";
		process.env.CLAWWORK_SKIP_VERSION_CHECK = "1";
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.CLAWWORK_STARTUP_BENCHMARK ?? process.env.PI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: CLAWWORK_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
