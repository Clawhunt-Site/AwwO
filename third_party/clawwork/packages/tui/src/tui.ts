...
export class TUI extends Container {
	...
	private showHardwareCursor = (process.env.CLAWWORK_HARDWARE_CURSOR ?? process.env.PI_HARDWARE_CURSOR) === "1";
	private clearOnShrink = (process.env.CLAWWORK_CLEAR_ON_SHRINK ?? process.env.PI_CLEAR_ON_SHRINK) === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = (process.env.CLAWWORK_DEBUG_REDRAW ?? process.env.PI_DEBUG_REDRAW) === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".clawwork", "agent", "clawwork-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".clawwork", "agent", "clawwork-crash.log");
				const crashData = [
		...
		if ((process.env.CLAWWORK_TUI_DEBUG ?? process.env.PI_TUI_DEBUG) === "1") {
