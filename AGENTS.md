...
> **本分支（`dev/server-refactor`）是 Node server 再平台化的长生命周期集成主干，按 `docs/git-workflow-constitution.md` §十一「本地循环」工作流运行。** 凡本节与下方任何「分支拓扑 / promotion flow / CI Policy / 验收」流程冲突，**本分支以本节为准**；环境配置（dev/staging/production 三档、secrets、env 选择器）、worktree 隔离、Approval Gates、桌面打包 intent 门等**非冲突内容照常遵守**。
>
> 〔正常 main 推进流程的**完整版** `AGENTS.md` / `CLAUDE.md` 已备份在 `main-md/`；末次合回 `main` 时用它恢复。〕

- **基准 = 本地 `dev/server-refactor`**（不是 `origin/main`、不是 `dev/roadmap`）。子功能 `git worktree add <path> -b feat/<slug> dev/server-refactor`，一会话一树一分支。
- **主干 merge-only**：**绝不在 `dev/server-refactor` 上直接开发 / 直接 checkout 改业务文件 / 直接提交功能代码**；它只接收【子功能 / 回滚分支（`feat/<slug>` / `revert/<slug>`）merge + `origin/main` 吸收 + 本工作流文档维护】三类写入。所有功能 / 代码 / 重构 / 测试都在 `feat/<slug>` 子 worktree 上做、再 merge 回（§十一 第 1 条）。
- **promotion flow 改为**：`feat/<slug>` → 本地 merge 回 `dev/server-refactor`（按 §十一 第 3 条 5 步：先更新主干 → `git merge --no-commit --no-ff` 解冲突 → 相关测试全绿 → Codex 无阻断 → 才 `commit`）。**不经 `dev/roadmap`、开发期不发 PR**。
- **验收 = Codex 单顾问**（`codex-cli-advisor --model gpt-5.5`）；本分支**不强制 Gemini**（覆盖 `CLAUDE.md`「Codex + Gemini 双顾问」铁律）。
- **测试 = 只测相关**：跑改动那块「相关 + 可能相关」的 lint / test / build / smoke + 补相关 test case；**开发期不跑全量 `ci.yml`**（覆盖下方 CI Policy / "远端 CI 已暂停" 的全量要求——它们在本分支只在 PR 时点触发）。
- **远端 / 全量 / 合 `main` = 全部留到 PR 时由 Leon 统一做**（满血重门：吸收 `origin/main` + 全量 `ci.yml` + 分段可 review PR〔vendor 先单独落地 + 完整性门〕+ **Codex + Gemini 双顾问** + §五 merge 门禁）。开发期**不强制推远端**；`origin/dev/server-refactor` 仅作可选备份。
- **命名去 Paperclip（§十一 第 8 条）**：我们**新造**的命名（标识符 / 产品 / 模块 / 文案 / 文档叙述 / commit / 分支）**不得用任何形式的 "paperclip"**（大小写 / 变体均算），用 `SuperClaw` / 中性词。提交前 `git diff | grep -iE '^\+.*paperclip'`，**每处新增命中须落在例外内、否则改名**。**例外（事实性 / 被迫，非品牌传播）** = ① vendored `server/` 上游树逐字保留（tree-equality）；② vendor 溯源（`paperclipai/paperclip` 归因）；③ 对接上游既有标识符的必需引用（如 `paperclipRuntimeServices`）；④ 本规则 / 治理文档的必要提及。
- **红线不豁免**：永不本地 `main` commit、永不 `git push origin main`、**主干 merge-only（绝不在 `dev/server-refactor` 直接开发 / 提交功能代码）**、worktree 隔离、禁 GitHub 分支保护、共享分支永不 force、远端状态变更需业主批准（Approval Gates 照旧）。
